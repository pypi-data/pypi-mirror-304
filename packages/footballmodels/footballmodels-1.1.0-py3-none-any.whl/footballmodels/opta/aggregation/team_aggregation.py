...
def possession_operations(data):
    ...
    if len(data[data["event_type"] == EventType.Pass]) == 0:
        valid = False
...
def get_team_aggregation_2(data, possession_data):
    ...
    grouped_data = grouped_data[grouped_data["valid"] == True]
    grouped_data = grouped_data.groupby(
        ["season", "competition", "matchId", "possession_owner"]
    ).agg(
        {
            "start_possession_distance": "mean",
            "valid": "count",
            "end_possession_distance": "mean",
    ...
    grouped_data = pd.merge(
        left=gruoped_data, right=circulation(data), left_index=True, right_index=True
    )
    return grouped_data
