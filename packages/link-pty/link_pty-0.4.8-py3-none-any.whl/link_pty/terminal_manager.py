        ...
        while True:
            try:
                ...
                if len(self.buf) > 0 and self.ws.open:
                    terminal_index, output = self.buf.pop(0)
                    await self.ws.send(json.dumps({"operation": "RECEIVE_SERVEROUTPUT", "terminal_index": terminal_index, "data": output}))
            except Exception as e:
                print(f"Error while reading pty output: {e}")
        ...
        try:
            async for message in ws:
                ...
                if operation == "ASSIGN_KEY":
                    ...
                    # 使用 asyncio 创建任务
                    asyncio.create_task(self.read_and_save_buf(master_fd, len(TerminalManager.terminals) - 1))
