...
class NERImplementation(evaluate.Metric):
    ...
    def _compute(self, predictions, references):
        post_processor , _ = fetch_artifact("processors.to_span_label_pairs")
        
        processed_predictions = [ post_processor.process(pred) for pred in predictions]
        processed_references = [ post_processor.process(ref) for ref in references]
        metric = NER(n_resamples=None)
        results = self.__apply_metric(metric,processed_predictions,processed_references)
    ...
    def __apply_metric(self, metric, predictions, references):
