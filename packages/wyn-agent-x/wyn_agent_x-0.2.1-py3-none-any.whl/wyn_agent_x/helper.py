...
def find_in_event_stream(key: str, event_stream: List[Dict[str, Any]], current_api_name: str) -> Optional[str]:
    """
    Helper function to find a piece of information in the event stream for the current API context.
    Searches in reverse order for the most recent occurrence of a matching key that hasn't been used in a successful API call.
    ...
    for event in reversed(event_stream):
        # Stop the search if an API call with a 'success' status is found for the current API context
        if event.get("event") == "api_call" and event.get("api_name") == current_api_name and event.get("response", {}).get("status") == "success":
            break
