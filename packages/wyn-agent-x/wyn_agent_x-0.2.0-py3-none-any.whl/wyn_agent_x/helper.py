...
def find_in_event_stream(key: str, event_stream: List[Dict[str, Any]]) -> Optional[str]:
    ...
    Stops when a 'success' status is encountered, meaning that information is already used.
    """
    # Iterate through the event stream in reverse
    for event in reversed(event_stream):
        # Stop the search if an API call with a 'success' status is found
        if event.get("event") == "api_call" and event.get("response", {}).get("status") == "success":
            break
