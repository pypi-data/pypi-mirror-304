...
from fastapi_users_with_username import FastAPIUsers, schemas
...
async def test_app_client(
    ...
    fastapi_users_with_username = FastAPIUsers[UserModel, IDType](
        get_user_manager, [mock_authentication]
    )

    app = FastAPI()
    app.include_router(fastapi_users_with_username.get_register_router(User, UserCreate))
    app.include_router(fastapi_users_with_username.get_reset_password_router())
    app.include_router(fastapi_users_with_username.get_auth_router(mock_authentication))
    app.include_router(
        fastapi_users_with_username.get_oauth_router(oauth_client, mock_authentication, secret)
    )
    app.include_router(
        fastapi_users_with_username.get_oauth_associate_router(oauth_client, User, secret)
    )
    ...
    app.include_router(
        fastapi_users_with_username.get_users_router(User, UserUpdate), prefix="/users"
    )
    app.include_router(fastapi_users_with_username.get_verify_router(User))

    @app.get("/current-user", response_model=User)
    def current_user(user: UserModel = Depends(fastapi_users_with_username.current_user())):
        return user

    @app.get("/current-active-user", response_model=User)
    def current_active_user(
        user: UserModel = Depends(fastapi_users_with_username.current_user(active=True)),
    ):
        return user

    @app.get("/current-verified-user", response_model=User)
    def current_verified_user(
        user: UserModel = Depends(fastapi_users_with_username.current_user(verified=True)),
    ):
        return user
    ...
    def current_superuser(
        user: UserModel = Depends(
            fastapi_users_with_username.current_user(active=True, superuser=True)
        ),
    ):
        return user
    ...
    def current_verified_superuser(
        user: UserModel = Depends(
            fastapi_users_with_username.current_user(active=True, verified=True, superuser=True)
        ),
    ):
        return user
    ...
    def optional_current_user(
        user: Optional[UserModel] = Depends(fastapi_users_with_username.current_user(optional=True)),
    ):
    ...
    def optional_current_active_user(
        user: Optional[UserModel] = Depends(
            fastapi_users_with_username.current_user(optional=True, active=True)
        ),
    ):
        return schemas.model_validate(User, user) if user else None
    ...
    def optional_current_verified_user(
        user: Optional[UserModel] = Depends(
            fastapi_users_with_username.current_user(optional=True, verified=True)
        ),
    ):
        return schemas.model_validate(User, user) if user else None
    ...
    def optional_current_superuser(
        user: Optional[UserModel] = Depends(
            fastapi_users_with_username.current_user(optional=True, active=True, superuser=True)
        ),
    ):
        return schemas.model_validate(User, user) if user else None
    ...
    def optional_current_verified_superuser(
        user: Optional[UserModel] = Depends(
            fastapi_users_with_username.current_user(
...
@pytest.mark.fastapi_users_with_username
@pytest.mark.asyncio
