...
class P115Client:
    ...
    def download_url_app(
        ...
        **request_kwargs, 
        ...
        :payload:
            - pickcode: str 💡 如果 `app` 为 "chrome"，可以接受多个，多个用逗号 "," 隔开
            - pick_code: str 💡 同上，这两个参数，只需传一个
        """
        if app == "chrome":
        ...
        if isinstance(payload, str):
            payload = {"pickcode": payload, "pick_code": payload}
        elif "pickcode" in payload:
            payload = {**payload, "pick_code": payload["pickcode"]}
        elif "pick_code" in payload:
            payload = {**payload, "pickcode": payload["pick_code"]}
