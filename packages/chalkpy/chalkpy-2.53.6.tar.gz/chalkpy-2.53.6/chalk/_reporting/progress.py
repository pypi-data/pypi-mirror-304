    ...
    def poll_report(
        ...
        while datetime.now() < must_receive_next_report_by and datetime.now() < completion_deadline:
            ...
            if datetime.now() >= completion_deadline:
                raise TimeoutError(
                    f"Timed out waiting ({completion_timeout}) for completion of operation with id '{self.operation_id}'"
                )
            else:
                raise TimeoutError(
                    f"Timed out waiting ({timeout}) for next status report for operation with id '{self.operation_id}'"
                )
