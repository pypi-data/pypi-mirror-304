...
class Catsat(KaitaiStruct):
    ...
    class Adcs0BcnFast(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.ctrl_refq = [None] * (4)
            for i in range(4):
                self.ctrl_refq[i] = self._io.read_f4be()

            self.ctrl_errq = [None] * (4)
            for i in range(4):
                self.ctrl_errq[i] = self._io.read_f4be()

            self.ctrl_m = [None] * (3)
            for i in range(3):
                self.ctrl_m[i] = self._io.read_f4be()

            self.ctrl_mwspeed = [None] * (4)
            for i in range(4):
                self.ctrl_mwspeed[i] = self._io.read_f4be()

            self.ctrl_euleroff = [None] * (3)
            for i in range(3):
                self.ctrl_euleroff[i] = self._io.read_f4be()

            self.ctrl_btorque = [None] * (3)
            for i in range(3):
                self.ctrl_btorque[i] = self._io.read_f4be()
    ...
    class Adcs7BcnMed(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.ctrl_errrate = [None] * (3)
            for i in range(3):
                self.ctrl_errrate[i] = self._io.read_f4be()

            self.ctrl_m = [None] * (3)
            for i in range(3):
                self.ctrl_m[i] = self._io.read_f4be()

            self.ctrl_mwtorque = [None] * (4)
            for i in range(4):
                self.ctrl_mwtorque[i] = self._io.read_f4be()

            self.ctrl_mwspeed = [None] * (4)
            for i in range(4):
                self.ctrl_mwspeed[i] = self._io.read_f4be()

            self.ctrl_mwmoment = [None] * (4)
            for i in range(4):
                self.ctrl_mwmoment[i] = self._io.read_f4be()

            self.ctrl_refrate = [None] * (3)
            for i in range(3):
                self.ctrl_refrate[i] = self._io.read_f4be()
            ...
            self.ctrl_bmoment = [None] * (3)
            for i in range(3):
                self.ctrl_bmoment[i] = self._io.read_f4be()
