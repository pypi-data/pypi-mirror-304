...
class Experiment:
    ...
    def add_models_for_training(self, models, model_configs, compute_alias=""):
        ...
        Example
        -------
        >>> model = ModelArch(session, model_key="resnet50", is_autoML=True, tuning_type="grid", ...)
        >>> model_config = {"action_config": {}, "model_config": {}, ...}
        >>> resp, err, msg = experiment.add_models_for_training(model, model_config, "GPU")
        >>> if err:
        >>>     print(f"Error: {err}")
        >>> else:
    ...
    def list_models(self):
        ...
        tuple
            ...
            - models (list): A list of Model instances, each containing the name of a model.
            - status_list (list): A list of status strings corresponding to each model.
        ...
        Example
        -------
        >>> models, status_list, response, err, msg = experiment.list_experiment_models()
        >>> if err:
        >>>     print(f"Error: {err}")
        >>> else:
    ...
    def get_best_model(self):
        """
        Get the best model associated with the current experiment based on test score.

        This method returns the best model stored during the list_experiment_models call.

        Returns
        -------
        tuple
            A tuple containing three elements:
            - best_model (Model or None): The Model instance with the highest test score, or None if no models found.
            - error_message (str or None): Error message if an error occurred, None otherwise.
            - status_message (str): A status message indicating success or failure.
        ...
        >>>     print(f"Error: {err}")
        >>> elif best_model:
        >>>     print(f"Best Model: {best_model.name}, Test Score: {test_score}")
        >>> else:
