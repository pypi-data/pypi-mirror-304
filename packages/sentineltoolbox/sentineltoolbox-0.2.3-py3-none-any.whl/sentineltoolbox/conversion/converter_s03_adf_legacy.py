...
def convert_sl_adf_json(adf_type: str, input_path: Path) -> dict[Any, Any]:
    json_data = {}
    ...
    if adf_type == "SL1PP":
        ...
        for xml_file in input_path.glob("*.xml"):
            ...
            with xml_file.open() as xml_file_fp:
                ...
                for child in root:
                    tag = child.tag.lower()
                    if tag == "marine_branch_switches":
                        continue
                    elif tag == "land_branch_switches":
                        tag = "switches"
                    json_data[tag] = {}
                    for param in child:
                        p = {k.lower(): v for k, v in param.attrib.items() if k != "name"}
...
CONVERT_ADFS = {
    ...
    "SL1PP": convert_sl_adf_json,
    "SLADJ": convert_sl_adf_json,
    # SL2
