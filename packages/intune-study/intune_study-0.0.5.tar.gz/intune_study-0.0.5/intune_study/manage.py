...
def create_compliance_policy(tenant_id, client_id, client_secret):
    ...
    if response.status_code == 201:
        print(f"**Device Compliance Policy created successfully.**\n\nResponse:\n```json\n{json.dumps(response.json(), indent=4)}\n```")
    else:
        print(f"**Failed to create device compliance policy:** {response.status_code}\n\nError:\n```json\n{response.text}\n```")
...
def list_managed_devices(tenant_id, client_id, client_secret):
    ...
    if response.status_code == 200:
        print(f"**Managed Devices:**\n\n```json\n{json.dumps(response.json(), indent=4)}\n```")
    else:
        print(f"**Failed to list managed devices:** {response.status_code}\n\nError:\n```json\n{response.text}\n```")
