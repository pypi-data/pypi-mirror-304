"""
    QApp Platform Project cuda_quantum_handler_factory.py Copyright © CITYNOW Co. Ltd. All rights reserved.
"""
...
class CudaQuantumHandlerFactory(HandlerFactory):

    @staticmethod
    def create_handler(event, circuit_preparation_fn, post_processing_fn) -> Handler:
        logger.info("[CudaQuantumHandlerFactory] create_handler()")

        request_data = event.json()

        logger.debug("[CudaQuantumHandlerFactory] Create InvocationHandler")
