...
class BoxLayout(Layout, QBoxLayout):
    ...
    def addWidget(self,
                  children: QLayout | QWidget | Stretch | Spacing,
                  alignment=Qt.AlignmentFlag(0)) -> None:
        if isinstance(children, QLayout):
            return self.addLayout(children)
        if isinstance(children, Stretch):
            return self.addStretch(children)
        if isinstance(children, Spacing):
            return self.addSpacing(children)

        if isinstance(children, WidgetMix):
            alignment = children.self_align
        super().addWidget(children, alignment=alignment)
