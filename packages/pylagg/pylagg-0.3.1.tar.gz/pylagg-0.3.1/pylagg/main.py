...
def cgr(
    ...
    if size > 100 or size < 1:
        raise Exception("Please use a number between 1 and 100 for scale size.\nIf you need help, type 'lagg --help'.")
    if kmer <= 0:
    ...
    if input and not((input.rfind(".txt") != -1)):
        raise Exception("Your input is not a supported file type. Supported types are '.txt'. Please convert to a supported file type and try again.\nIf you need help, type 'lagg --help'.")
        ...
        with open(counts_file_path) as f:
            ...
            if(input.rfind(".txt") != -1):
                if size != 100:
                    ...
                    cgr_g.count_file_to_image_file(f, output_path + "/" + inputname + ".png")
