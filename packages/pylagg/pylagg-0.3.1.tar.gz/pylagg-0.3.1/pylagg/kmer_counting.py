...
def handle_config_jellyfish(config: dict, list_of_input_file_paths: List[str] = []):
    ...
    if 'jellyfish_args' in config:
        ...
        for key, value in jellyfish_args.items():
            ...
            if key not in config_keys_valid_options:
                ...
                raise Exception(f"ERROR: Invalid key: '{key}' was found in the 'jellyfish_args' section of the config file.")
        ...
        if 'kmer_length' not in args:
            raise Exception("ERROR: Kmer length is required. Please provide a value for this key in your config file.")

        if (args['kmer_length'] < 1):
            raise Exception("ERROR: Kmer length must be a positive integer.")

    else:
        raise Exception("ERROR: No 'jellyfish_args' section found in the config file.")
    ...
    if args is not None:
        ...
        raise Exception("ERROR: The 'jellyfish_args' section of the config file is empty.")
