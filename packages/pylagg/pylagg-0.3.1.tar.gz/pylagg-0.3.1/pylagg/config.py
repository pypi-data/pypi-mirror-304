...
from .cgr_step import config_count_file_to_image_file
...
def load_toml_file(config_file: str) -> dict:
    ...
    if not os.path.isfile(config_file):
        raise Exception(
            "ERROR: Config file not detected. Please ensure you have given the correct path."
        )
    ...
    try:
        ...
        if type(e) is toml.TomlDecodeError:
            raise Exception(
                "ERROR: Invalid TOML file. Please provide a valid TOML file."
            )
        else:
            raise Exception("ERROR: An error occurred while reading the config file.")
