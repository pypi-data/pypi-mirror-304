...
class FjqAuth(ABC, AuthBase):
    ...
    def update_access_token_in_credential_file(self, parent_lock: BaseFileLock = None) -> None:
        cred_json = self._read_credential_file()
    ...
    def _read_credential_file(self, parent_lock: BaseFileLock = None) -> Dict[str, str]:
        try:
            lock = parent_lock
            if lock is None:
                lock = FileLock(FjqAuth.CREDENTIALS_FILE_LOCK_PATH)
    ...
    def _load_credentials(self, parent_lock: BaseFileLock = None) -> None:
...
class FjqAuthBoto3(FjqAuth):
    ...
    def refresh_auth(self, force: bool = False) -> None:
        ...
        try:
            ...
            with lock:
                if not force:
                    # Because other threads or processes may have updated the credential file, it should reload the file.
                ...
                self.update_access_token_in_credential_file(lock)
