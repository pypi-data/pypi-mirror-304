...
def Spatial_Clustering(region, category):
    ...
    # 분석데이터 읽어오기
    with importlib.resources.open_text('sskie.data', "data/" + region + "_all_store.txt") as f:
        df_all_stores = pd.read_csv(f, sep=',', engine='python', encoding='UTF-8')

    with importlib.resources.open_text('sskie.data', "data/" + region + "_" + category + "_store.txt") as f:
