...
class SpatialAnalysis:
    ...
    def find_apt(self):
        ...
        with importlib.resources.open_text('sskie.data', "건물_서울_5179.geojson") as f:
            gdf_building = gpd.read_file(f, encoding='cp949')

        with importlib.resources.open_text('sskie.data', "지하철역_수도권_5179.geojson") as f:
            gdf_subway = gpd.read_file(f, encoding='cp949')

        with importlib.resources.open_text('sskie.data', "실거래가_서울.txt") as f:
            df_hs_p = pd.read_csv(f, sep='|', encoding='cp949', low_memory=False)
