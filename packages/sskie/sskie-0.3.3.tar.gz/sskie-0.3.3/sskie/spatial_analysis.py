...
def Spatial_Clustering(region, category):
    ...
    # 분석데이터 읽어오기
    with importlib.resources.open_text('SKIE.data', "Data/" + region + "_모든업종_점포정보.txt") as f:
        df_all_stores = pd.read_csv(f, sep=',', engine='python', encoding='UTF-8')

    with importlib.resources.open_text('SKIE.data', "Data/" + region + "_" + category + "_점포정보.txt") as f:
