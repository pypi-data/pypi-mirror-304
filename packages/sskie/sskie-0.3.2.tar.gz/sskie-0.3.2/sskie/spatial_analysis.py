...
def Spatial_Clustering(region, category):
    ...
    os.chdir('E:/R&D_공간지(국토부)/공간지식추론엔진 함수/4월_마포구 음식업 특화 장소/')
    ...
    # 분석데이터 읽어오기
    df_all_stores = pd.read_csv("Data/" + region + "_모든업종_점포정보.txt", sep=',', engine='python', encoding='UTF-8')
    df_food_stores = pd.read_csv("Data/" + region + "_" + category + "_점포정보.txt", sep=',', engine='python',
                                 encoding='UTF-8')
    ...
    r.to_html('Result/마포구 음식업 특화 장소_test.html')
