...
class Trades:
    ...
    def limit(
        ...
        for i, row in trades.iterrows():
            ...
            for end in range(start + 1, row[EXIT]):
                if row[TYPE] == LONG:
                    take_profit = take and (bid[end] / ask[start] > take)
                    stop_loss = stop and (bid[end] / ask[start] < stop)

                else:
                    take_profit = take and (bid[start] / ask[end] > take)
                    stop_loss = stop and (bid[start] / ask[end] < stop)
    ...
    def returns(
        ...
        returns[flip_short] = (
                bid.iloc[trades[ENTRY]].values / ask.iloc[trades[EXIT]].values
        )[flip_short]
...
class Plot:
    ...
    def returns_signals(returns: pd.Series, index: np.ndarray | pd.Series = None) -> None:
        ...
        if curve is not None:
            plt.plot(index[returns.index], curve, alpha=1, lw=2.5, c="cyan")
