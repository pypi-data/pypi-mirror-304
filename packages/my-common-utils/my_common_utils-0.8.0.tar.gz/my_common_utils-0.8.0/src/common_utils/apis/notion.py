...
class NotionClient:
    ...
    def get_page_content_markdown(self, page_id: str) -> str:
        ...
        for block in blocks['results']:
            try:
                ...
                match block['type']:
                    ...
                    case 'quote':
                        text = ''.join(
                            [text_part['text']['content'] for text_part in block['quote']['rich_text']])
                        text = f"> {text}"
                    case 'code':
...
if __name__ == "__main__":
    ...
    first_page = notion.get_page_content_markdown(df["id"].iloc[0])
