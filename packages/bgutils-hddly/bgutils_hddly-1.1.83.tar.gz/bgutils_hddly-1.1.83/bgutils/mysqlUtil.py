...
class mysqlUtil:
    ...
    def sftp_file_ins(self, filename, url, stud, fdesc, fsize, duration, ftype, pid, surl):
        try:
            ...
            strins = f"insert into sftp_files (filename, url, stud, fdesc, fsize, duration, ftype, pid, surl)"
            strins += f" values('{filename}','{url}','{self.escape(stud)}',"
            strins += f" '{self.escape(fdesc)}',{fsize},{duration},'{ftype}','{pid}','{self.escape(surl)}')"
            connection = self.engine.connect()
            connection.execute(strins)
