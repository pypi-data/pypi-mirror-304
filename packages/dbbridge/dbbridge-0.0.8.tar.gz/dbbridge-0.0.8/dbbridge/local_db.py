...
class LocalDBConnection:
    ...
    def __init__(self, host, user, password, database, region, port=3306):
        if not all([host, user, password, database, region]):
            raise ValueError("Host, user, password, database, and region are required for local database connection.")
    ...
    def connect_to_database(self):
        ...
        :raises pymysql.OperationalError: If the connection cannot be established.
        """
        try:
            ...
            raise pymysql.OperationalError(f"Failed to connect to the database: {e}")
