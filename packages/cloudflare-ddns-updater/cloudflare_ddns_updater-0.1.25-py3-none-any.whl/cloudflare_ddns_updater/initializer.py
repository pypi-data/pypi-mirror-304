...
def toggle_cron_job(toggle):
    ...
    for job in cron:
        if job.comment == cron_comment:
            ...
            cron.write()
            if job.is_enabled():
                print(f"{job.comment} cron job is functioning")
            else:
                print(f"{job.comment} cron job is stopped. "
...
def cleanup():
    ...
    print("Removed '/etc/ip_updater' Directory and 'cloudflare_config.json' ")
    try:
        os.remove('/var/log/ip_update.log')
        print("Removed '/var/log/ip_update.log' ")
    except:
        pass
    try:
        os.remove('/tmp/counter.txt')
        print("Removed '/tmp/counter.txt'")
    except:
        pass
    try:
        os.remove('/tmp/current_ip.txt')
        print("Removed '/tmp/current_ip.txt'")
    except:
        pass
    delete_cron_job()
...
def run_setup():
    ...
    try:
        ...
        print(f"Error occurred during retrieval of zone id.\n"
              f"Make sure that your token has the needed permissions.\n{e}")
        sys.exit()
