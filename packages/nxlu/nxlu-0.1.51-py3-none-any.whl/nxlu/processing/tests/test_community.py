...
def mock_sentence_transformer():
    ...
    class MockTransformer:
        ...
        def encode(self, text, convert_to_numpy=True, *args, **kwargs):
            if isinstance(text, str):
                return np.array([0.1, 0.2, 0.3])
            return np.array([[0.1, 0.2, 0.3]] * len(text))

    with patch("nxlu.processing.community.SentenceTransformer") as mock_st:
        mock_st.return_value = MockTransformer()
        yield mock_st
