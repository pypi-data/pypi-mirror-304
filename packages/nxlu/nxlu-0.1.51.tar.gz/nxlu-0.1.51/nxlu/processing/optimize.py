...
def filter_path_algorithms(algorithms: list[str], query: str | None) -> list[str]:
    ...
    for algo in algorithms:
        if algo in REQUIRES_SOURCE_TARGET:
            # check if algorithm requires both source and target
            requires_both = algo in {
                "shortest_path",
                "dijkstra_path",
                "bellman_ford_path",
            ...
            if requires_both:
                if has_source and has_target:
                    ...
                    logger.info(
                        f"Excluding algorithm '{algo}' due to missing source and/or "
                        f"target."
                    )
                ...
                logger.info(
                    f"Excluding algorithm '{algo}' due to missing source or target."
                )
        else:
            filtered_algorithms.append(algo)
...
class AlgorithmNominator:
    ...
    def select_algorithms(
        self, graph_properties: GraphProperties, query: str | None
    ) -> list[str]:
        ...
        with self._algorithms_lock:
            ...
            if query:
                selected_algorithms = filter_path_algorithms(selected_algorithms, query)
...
class AlgorithmElector:
    ...
    def apply_elected_algorithms(
        self, graph: nx.Graph, algorithms: list[str], query: str | None
    ) -> dict[str, Any]:
    ...
    def _extract_algorithm_parameters(
        query: str | None, algorithm_name: str
    ) -> dict[str, Any]:
