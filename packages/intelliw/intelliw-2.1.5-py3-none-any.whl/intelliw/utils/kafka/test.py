...
class MyConsumer:
    ...
    def handle_message(self, k, v):
        ...
        print(f"Received message with key: {k}, {otherStyleTime}")


    def run(self):
        # client = test_get_client_with_args('consumer1')
        client = get_client(
            group_id="mm.test.doc.8.bge",
            consumer_config={'auto.offset.reset': 'earliest',
                             'enable.auto.commit': True,
                             'max.poll.interval.ms': 8000000,
                             'heartbeat.interval.ms': 3000,
        ...
        # client.consume_messages(topics=["iuap_aip_search_topic_file_abstract"], handle_func=self.handle_message)
        client.consume_messages_with_threads(topics=['iuap_aip_service_topic_file_process'], handle_func=self.handle_message, num_threads=50)
