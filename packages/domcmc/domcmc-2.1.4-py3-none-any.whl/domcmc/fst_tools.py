...
def get_data(file_name:     Optional[str]=None,
    ...
    # keep track of what file the data comes from
    origin_file = file_to_read
    ...
    if (var is not None) and pres_from_var :
        ...
        estimated_pressure = press['phPa']


    #handle yin-yang grid in one record
    # optionally output lat/lon and pressure information
    if (var is not None):

        # file where data was taken from
        var['origin_file'] = origin_file
	    ...
	    #single grid -> not yin-yang
            if latlon == True :
                ...
                var['lat'] = ll_dict['lat']
                var['lon'] = ll_dict['lon']
            if estimated_pressure is not None:
                var['pressure'] = estimated_pressure
	    ...
	    #In this case, the outdict is modified to output data on the two grids
            ...
            if yy_values.ndim == 2:
                ...
                if estimated_pressure is not None:
                    var['yin']['pressure']  = copy.deepcopy(estimated_pressure[:,:half_ny])
                    var['yang']['pressure'] = copy.deepcopy(estimated_pressure[:,half_ny:])
