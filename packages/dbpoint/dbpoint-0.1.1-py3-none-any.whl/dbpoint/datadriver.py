...
logging = get_custom_logger('driver')
...
class DataDriverGeneric(object):
    ...
    def run(self, sql: str, do_return: bool = True, **kwargs):
        ...
        try:
            ...
            cur: Cursor = self.conn.cursor()
            cur.execute(sql)
            #logging.debug("execution instructed")

            if do_return:
                # cur.description: tuple[str, DBAPITypeCode, int | None, int | None, int | None, int | None, bool | None]
                if not self.analyze_column_meta(cur.description):
                    logging.debug("during analyze emptyness happenes")
                    return []
                
                if extra_return_style == 'number':
                    #logging.debug("numbered columns instructed")
                    result_set = cur.fetchall()

                if extra_return_style == 'name':
                    #logging.debug("named columns instructed")
                ...
                if extra_return_style == 'both':
                    #logging.debug("dualistic columns instructed")
                    columns_named_spaced = self.extract_name_string(' ')
            ...
            if extra_on_error_rollback:
                self.conn.rollback()
                logging.debug("rollback happenes")
            if extra_on_error_disconnect:
                self.disconnect()
                logging.debug("disconnect happenes")
            raise ex
        finally:      
        ...
        #logging.debug(f"commands executed is now {self.run_count} (and {self.run_count_after_last_disconnect} after last connect)")

        if extra_on_success_commit:
            #logging.debug("commit happenes")
            self.conn.commit()
        
        if do_return:
            #logging.debug("return data happenes")
            return result_set
        else:
            #logging.debug("just return happenes")
            return []
