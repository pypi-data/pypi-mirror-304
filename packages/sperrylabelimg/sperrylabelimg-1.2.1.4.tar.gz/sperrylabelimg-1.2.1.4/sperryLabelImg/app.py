...
# CONFIGURABLES
move_image_icon_path = proj_dir / "resources/folder-move.svg"
...
additional_button_names = {
    ...
    "resetDirs": "Reset Directories",
}

# REGEX PATTERNS
img_name_pattern = r'.*\d{3}_DF\d{6}_\d{3}_\d_\d+.*\.png'
pulse_count_pattern = r'.*\d{3}_DF\d{6}_\d{3}_\d_(\d+).*\.png'
rail_pattern = r'.*\d{3}_DF\d{6}_\d{3}_(\d)_\d+.*\.png'
data_source_pattern = r'.*(\d{3}_DF\d{6}_\d{3})_\d_\d+.*\.png'


ALIGNMENT_SHIFT = 1
HOME_DIR_STR = Path.home().as_posix()
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        def get_str(str_id) -> str:
            ...
            try:
                ...
                result = additional_button_names[str_id]
                logger.info(f"Using additional button name: {result}")
                return result
        ...
        self.file_list_widget.itemClicked.connect(self.file_item_double_clicked)
        ...
        quit = action(get_str('quit'), self.close, 'Ctrl+Q', 'quit', get_str('quitApp'))  # noqa
        open_dir = action(get_str('openDir'), self.add_active_dir, 'Ctrl+u', 'open', get_str('openDir'))
        add_dir = action(get_str("addDir"), self.add_active_dir, 'Alt+A', 'add', get_str("addDir"))
        reset_dirs = action(get_str("resetDirs"), self.reset_active_dirs, 'Alt+A+R', 'reset', get_str('resetDirs'))
        del_dir = action(get_str("delDir"), self.delete_active_dir, 'Alt+R', 'remove', get_str('delDir'))
        move_image = action(get_str("moveImage"), self.move_image_to_active_dir, "Ctrl+m")
        ...
        save_format = action(get_format_meta(self.label_file_format)[0], self.change_format, 'Ctrl+', get_format_meta(self.label_file_format)[1], get_str('changeSaveFormat'), enabled=True)
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, close=close, resetAll=reset_all,
                              ...
                              active_dirs_ops=(add_dir, del_dir, reset_dirs),
    ...
    def fixed_file_position(self, func):
        """
        maintains the file position for the file list in the main window

        :param func: the function you want to run whilst the file position is fixed
        :return: Nothing
        """
        ...
        @wraps(func)
        def wrapper():
            cur_img_idx = getattr(self, "cur_img_idx", 0)
            cur_img_idx += ALIGNMENT_SHIFT

            should_load_normally = False
            if not hasattr(self, "file_list_widget") or not hasattr(self, "m_img_list"):
                should_load_normally = True
            elif not getattr(self, "m_img_list", 0) or not self.file_list_widget.count() > 0:
                should_load_normally = True
            ...
            for idx in range(self.file_list_widget.count()):
                ...
                if org_img_path == some_img_path:
                    ...
                    self.cur_img_idx = self.m_img_list.index(org_img_path.as_posix())
                    break
            else:
    ...
    def setup_active_dir_dock(self):
        ...
        self.add_active_dir_button.clicked.connect(self.fixed_file_position(self.add_active_dir))
        self.add_active_dir_button.setShortcut("Alt+A")
        button_layout.addWidget(self.add_active_dir_button)

        self.reset_active_dir_button = QPushButton(self.tr("Reset Dirs"))
        self.reset_active_dir_button.clicked.connect(self.fixed_file_position(self.reset_active_dirs))
        self.reset_active_dir_button.setShortcut("Alt+R")
        button_layout.addWidget(self.reset_active_dir_button)

        self.remove_active_dir_button = QPushButton(self.tr("Delete Dir"))
        self.remove_active_dir_button.clicked.connect(self.fixed_file_position(self.delete_active_dir))
        self.remove_active_dir_button.setShortcut("Alt+D")
    ...
    # def do_save_if_dirty(self, func):
    #     """
    #     maintains the file position for the file list in the main window
    #
    #     :param func: the function you want to run whilst the file position is fixed
    ...
    def move_image_to_active_dir(self):
        ...
        if self.cur_img_idx is None or not self.m_img_list:
            return
        ...
        if not elmer_file:
            ...
            new_elmer_file_path = active_dir / elmer_file.name
        ...
        self.update_file_menu()
        org_cur_image_path = self.m_img_list[self.cur_img_idx]
        self.import_dir_images(active_dir)
        self.load_file(org_cur_image_path)
    ...
    def export_repeating_file_names_to_cmd(self):
        ...
        if self.m_img_list:
            ...
            start_dir = self.com_dir_path  # Replace it with your starting directory
    ...
    def active_dir_item_clicked(self, item):
        ...
        """
        When an active directory item is clicked, this method will find the
        index of the selected directory in the active directories list and
        update the current active directory.

        If the selected directory is the first item in the list (the
        container directory), then the next item after the container
        directory is selected instead. Finally, the title of the active
        directory dock widget is updated to reflect the
        current active directory.

        :param item: The item that was clicked in the active directory list widget.
        :return: None
        """
        index = 0
    ...
    def add_active_dir(self):
        ...
        # Update the active directories list and widget
        if self.cur_active_dir not in self.active_dirs:
            ...
            self.refresh_active_dir_widget()
        ...
        self.file_item_double_clicked(item=item)
        self.active_dir_list_widget.setCurrentItem(item)

        # Select the last opened active directory

    def refresh_active_dir_widget(self):
        """
        refreshes the active directories widget with the current active directories
        ...
        active_dir_items_list.extend(
            create_active_dir_item(active_dir, self.com_dir_path)
    ...
    def delete_active_dir(self):
        self.export_repeating_file_names_to_cmd()
        ...
        if selected_items:
            ...
            self.remove_active_dir(directory)

    def remove_active_dir(self, directory: Path):
        ...
        untouched_cur_active_dir = self.cur_active_dir
        ...
        self.cur_active_dir = untouched_cur_active_dir
        ...
        self.cur_img_idx = self.m_img_list.index(current_image_path.as_posix())
    ...
    def get_full_img_path(self, img_path: str) -> str:
        ...
        return self.com_dir_path / ascii_image_path.strip()
    ...
    def import_dir_images(self, dir_path):
        ...
        if self.m_img_list:
            ...
            com_img_dir_str = self.com_dir_path.as_posix()
            short_com_dir_str = com_img_dir_str.replace(HOME_DIR_STR, "~")
            short_com_dir_str = f"📂 {short_com_dir_str}"
            item = QListWidgetItem(short_com_dir_str)
            item.setFlags(item.flags() & ~Qt.ItemIsSelectable & ~Qt.ItemIsEnabled)
            self.file_list_widget.addItem(item)

        for imgPath in self.m_img_list:
            item_name = Path(imgPath).relative_to(self.com_dir_path)
            item = QListWidgetItem(f"📷 {item_name}")
            self.file_list_widget.addItem(item)

    def sort_img_list(self):
        self.m_img_list = list(filter(lambda img: re.match(img_name_pattern, os.path.basename(img)), self.m_img_list))
        ...
        self.m_img_list.sort(key=lambda img: re.match(data_source_pattern, os.path.basename(img)).groups()[0])
    ...
    def save_file(self, _value=False):
        ...
        ## Delete old annotation file if it exists and replace with new one
        old_ann_path = self.scan_all_for_file(label_file_name)
        if old_ann_path and old_ann_path.exists():
            os.remove(old_ann_path)
            label_path = old_ann_path
        else:
...
def calculate_com_dir_path(active_dirs) -> Path:
    """
