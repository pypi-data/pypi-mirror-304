...
# async def get_user_id(token: str = Header(..., description="JWT Token")):
#     try:
#         with open("public_key.pem", "rb") as f:
#             public_key = f.read()
        
#         payload = jwt.decode(token, public_key, algorithms=[ALGORITHM])
#         user_id = payload.get("user_id")  # Asegúrate de que `user_id` esté en los claims del token
#         if user_id is None:
#             raise HTTPException(status_code=401, detail="Token inválido: faltante user_id")
#         return user_id
#     except jwt.ExpiredSignatureError:
#         raise HTTPException(status_code=401, detail="Token inválido o expirado")
    
async def get_user_info(token: str = Header(..., description="JWT Token")):
    try:
        with open("public_key.pem", "rb") as f:
            public_key = f.read()
        ...
        return user_id, role  # Devuelve ambos valores como una tupla
    except jwt.ExpiredSignatureError:
        raise HTTPException(status_code=401, detail="Token inválido o expirado")
