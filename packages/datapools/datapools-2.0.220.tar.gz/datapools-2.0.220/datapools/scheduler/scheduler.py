...
class CrawlerScheduler(Stoppable):
    ...
    async def _enqueue_worker_task(
        ...
        if isinstance(task, CrawlerHintURL):
            ...
            qtask = WorkerTask(
                url=str(task.url), status=status, type=task.type, metadata=task.metadata, content_type=task.type
            )
        else:
        ...
        await todo_queue.push(
            QueueMessage(
                session_id=session_id, message_type=QueueMessageType.Task, data=qtask.to_dict()  # , priority=priority
            )
        )
