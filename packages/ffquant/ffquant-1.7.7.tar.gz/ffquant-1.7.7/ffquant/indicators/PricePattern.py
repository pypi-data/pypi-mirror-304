...
class PricePattern(bt.Indicator):
    ...
    def next(self):
        ...
        if current_bar_time_str not in self.cache:
            ...
            # 用于判断当前行情
            self.cache[current_bar_time_str] = self.determine_pattern(self.data.open[0], self.data.high[0], self.data.low[0], self.data.close[0], self.sma_long, self.short_price_change_rate_list[-5:], highest_price, lowest_price)
        else:
            if self.p.debug:
    ...
    def determine_pattern(self, bar_open_price, bar_high_price, bar_low_price, bar_close_price, ema_50, price_rate_list, high_price_in_20, low_price_in_20):
