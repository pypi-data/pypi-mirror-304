...
def run_and_show_performance(cerebro, strategy_name, riskfree_rate = 0.01, use_local_dash_url=False, debug=False):
    ...
    if strategy_name is None or strategy_name == '':
        raise Exception('strategy name cannot be empty')
...
def show_performance(strats, riskfree_rate = 0.01, debug=False):
    for strat in strats:
        ...
        if debug:
            stdout_log(f"Assigned port for user {getpass.getuser()}: {port}")
