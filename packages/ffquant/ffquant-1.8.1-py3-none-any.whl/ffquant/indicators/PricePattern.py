...
class PricePattern(bt.Indicator):
    ...
    def determine_pattern(self, bar_open_price, bar_high_price, bar_low_price, bar_close_price, ema_50, price_rate_list, high_price_in_20, low_price_in_20, close_price_list):
        ...
        if (bar_close_price - bar_open_price) / bar_open_price >= 0.01:
            ...
            if (high_price_in_20 - bar_close_price) / (high_price_in_20 - low_price_in_20) >= 0.5:
                if self.judge_price_percent_rate(ret, close_price_list):
            ...
            if (bar_close_price - low_price_in_20) / (high_price_in_20 - low_price_in_20) >= 0.5:
                if self.judge_price_percent_rate(ret, close_price_list):
