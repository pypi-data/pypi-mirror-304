...
class Tts_Config:
    ...
    def __init__(self):
        ...
        """
        g_uper_log.info("TTS配置类")
        self.__dict = dict()
        self.__client_config_support_count = 1
        self.__load_config("tts_scheme", g_commonpath.uppercompter_config_path)
        if self.scheme_config_support_count > 0:
            for i in range(self.scheme_config_support_count):
                # 拼接配置文件SECTION
                section_name = f"tts_scheme_{i + 1}"
                tts_scheme_config = Tts_Scheme_Config(section_name, g_commonpath.uppercompter_config_path)
                g_uper_log.debug(tts_scheme_config)
                self.__dict[tts_scheme_config.scheme_name] = tts_scheme_config
        g_uper_log.info(f"所有TTS配置信息：{self.__dict}")
...
class Tts_Scheme_Config:
    """
    类功能描述：TTS方案配置类，从配置文件中获取
    """
    ...
    def __load_config(self, section, fconfig_path):
        ...
        try:
            ...
            self.__cache_path = config.get(section, "cache_path", fallback="")
            g_uper_log.debug(f"cache_path -> {self.cache_path}")
    ...
    def __scheme_subtype_conversion_to_eum(self, client_subtype:int):
        ...
        """
        if client_subtype == -1:
            return TtsSubTypeEnum.NONE
        elif client_subtype == 0:
            return TtsSubTypeEnum.WINDOWS
        else:
            return TtsSubTypeEnum.NONE