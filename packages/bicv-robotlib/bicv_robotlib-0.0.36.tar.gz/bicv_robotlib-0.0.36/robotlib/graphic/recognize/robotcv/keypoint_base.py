...
from robotlib.graphic.recognize.utils.logger import get_logger
...
class KeypointMatching(object):
    ...
    def find_best_result(self):
        ...
        G.LOGGING.debug("[%s] threshold=%s, result=%s" % (self.METHOD_NAME, self.threshold, best_match))
    ...
    def _get_key_points(self):
        ...
        G.LOGGING.debug("_get_key_points len(kp_sch)=%d and len(kp_src)=%d" % (len(kp_sch),len(kp_src)))
        # print("_get_key_points len(kp_sch)=%d and len(kp_src)=%d" % (len(kp_sch),len(kp_src)))
