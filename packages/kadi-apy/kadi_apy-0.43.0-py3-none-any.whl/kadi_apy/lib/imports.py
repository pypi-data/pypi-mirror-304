    ...
    with zipfile.ZipFile(file_path) as ro_crate, TemporaryDirectory() as tmpdir:
        ...
        for root_part in root_parts:
            ...
            record_metadata = {
                ...
                "description": _extract_description(dataset_entity),
            }
            ...
            for dataset_part in dataset_parts:
                ...
                file_description = _extract_description(file_entity)
                ...
                if (
                    ...
                    property_values = _extract_property(
                        dataset_entity, "variableMeasured", as_list=True
                    )
                    extras = _map_properties_to_extras(property_values)
...
def _extract_description(entity):
    description = entity.get("description", "")
    if isinstance(description, dict) and description["@type"] == "TextObject":
...
def _map_properties_to_extras(property_values):
    extras_list = []

    for property_value in property_values:
        keys = str(property_value.get("propertyID", "")).split(".")
        ...
        if property_value.get("description") is not None:
            extra["description"] = _extract_description(property_value)

        if (term_iri := property_value.get("identifier")) is not None:
            extra["term"] = str(term_iri)

        if (unit := property_value.get("unitText")) is not None:
            extra["unit"] = str(unit)

        validation = {}

        minimum = property_value.get("minValue")
        maximum = property_value.get("maxValue")
