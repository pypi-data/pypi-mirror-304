...
class FfmpegFlag:
    ...
    def force_format(cls, format: FfmpegVideoFormat):
        ...
        Notation: **-f {format}**
        """
        format = FfmpegVideoFormat.to_enum(format)
    ...
    def codec(cls, codec: str):
        ...
        # TODO: I think this can be '-c:a' or '-c:v' for audio or video codecs
        # so maybe we need audio_codec() to set '-c:a' and video_codec() also
        return f'-c {codec}'
    ...
    def filter(cls, filter: FfmpegFilter):
        ...
        """
        filter = FfmpegFilter.to_enum(filter)

        return f'-filter {filter.value}'
...
class FfmpegHandler:
    ...
    # This command concatenates images
