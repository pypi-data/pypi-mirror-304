...
class PwpInstallerUi(PwpGui):
    def __init__(self, args, installer):
        ...
        self.column_sizes([30, 15, 15, 15, 15, 10])
        ...
        row += 1
        GuiLabel(self, self.frm, text="BASE: default working directory",
                 fr_text="BASE: Dossier de travail par défaut",
                 column=0, row=row, bold=True, width="")

        GuiLabel(self, self.frm, text=self.default_base, fr_text=self.default_base,
        ...
        GuiLabel(self, self.frm, text="HOME: user personal directory",
                 fr_text="HOME: Dossier personnel",
                 column=0, row=row, bold=True, width="")
        GuiLabel(self, self.frm, text=installer.user_home, fr_text=installer.user_home,
                 column=1, row=row, width="", col_span=4)
        ...
        GuiLabel(self, self.frm, text="piwiPre EXE version in target dir",
                 fr_text="version de piwiPre installée dans le dossier cible",
                 column=0, row=row, bold=True, width="")
        ...
        # self.pwp_version.grid(column=1, row=row, columnspan=4, sticky="W")        # noqa
...
class Installer:
    def __init__(self, arguments=None):
        ...
        self.ui = None
        self.async_ffmpeg = None
        self.async_piwipre = None
        self.async_packages = None
    ...
    def launch_piwi_pre(self):
        ...
        try:
            ...
            self.msg(f'starting "{target}" --gui true --home "{self.user_home}" --base "{self.user_base}"')
            subprocess.Popen([target, "--gui", "true", "--home", self.user_home,
                              "--base", self.user_base],  # non blocking call.
                             shell=True,
                             # check=True,
                             # text=True,
                             )  # noqa
    ...
    def compute_default_values(self):
        ...
        if self.ffmpeg_exists:
            ...
            if self.ffmpeg_version is None and self.async_ffmpeg is None:
                ...
                self.async_ffmpeg = AsyncObject(asynchronous,
                                                [target, "-version"],
                                                self,
        ...
        if self.piwipre_version is None and self.async_piwipre is None:
            ...
            self.async_piwipre = AsyncObject(asynchronous,
                                             [target, "--version"],
                                             self,
                                             r"current version: '(.*)'",
                                             "piwipre_version",
    ...
    def get_html(self, url):
        # No need to make asynchronous, because is done during the install phase
