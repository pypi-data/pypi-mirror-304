...
from piwiPre.pwpGui import GuiLabel, GuiExpandable, PwpGui, GuiStringEditor, GuiDirChooser, GuiButton, GuiRadios, \
    GuiScrollable, GuiFrame, GuiSeparator, GuiValue, GuiGroup, GuiEntry
...
class PwpEditorUI(PwpGui):
    ...
    def __init__(self, father: "PwpConfigurator", config: PwpConfig or None = None):
        ...
        self.column_sizes([29, 15, 15, 15, 15, 15, 15, 15, 15])
        ...
        GuiLabel(self, self.frm, column=6, row=row,
                 ...
                 width=55)
        # Parameters Menu starts hidden
        self.parameters_menu = GuiGroup()
        ...
        self.base_status = GuiValue(self, self.frm, row=row, column=4,
                                    width=16,
                                    dico={True: "configured", False: "Not configured !"},
                                    fr_dico={True: "configuré", False: "À configurer !"}, )
        self.parameters_menu.add_item(self.base_status)
        ...
        label5 = GuiLabel(self, self.frm, column=6, row=row,
                          text="You can configure another BASE and switch to it",
                          fr_text="On peut configurer une autre BASE, et s'y déplacer",
                          width="")
        self.parameters_menu.add_item(label5)

        # -------------- HOME

        row += 1
        GuiLabel(self, self.frm, column=0, row=row,
                 text="HOME", fr_text="HOME",
                 width="", bold=True)

        val = os.path.abspath(self.configurator.home)
        self.do_home.set(val)

        entry = GuiEntry(self.frm, width=PwpGui.VALUE_WIDTH,
                         textvariable=self.do_home, column=1, row=row, columnspan=3)
        # self.parameters_menu.add_item(entry)

        self.home_status = GuiValue(self, self.frm, row=row, column=4,
                                    width=16,
                                    dico={True: "configured", False: "Not configured !"},
                                    fr_dico={True: "configuré", False: "Sans configuration"}, )
        self.parameters_menu.add_item(self.home_status)

        self.set_home_button = GuiButton(self, self.frm, row=row, column=5,
                                         text="Change HOME",
                                         fr_text="Changer HOME", command=self.launch_change_home)
        self.parameters_menu.add_item(self.set_home_button)

        label4 = GuiLabel(self, self.frm, column=6, row=row,
                          text="You can configure another dir, but will HAVE TO change it through command-line",
                          fr_text="On peut configurer un autre HOME, mais il faut le changer sur la ligne de commande",
                          width="")
        self.parameters_menu.add_item(label4)
        ...
        self.configure_params_button = GuiButton(self, self.frm, row=row, column=5,
                                                 text="Parameters",
                                                 fr_text="Paramètres", command=self.gui_change_parameters)
        ...
        row += 1
        self.sep0 = GuiSeparator(self, self.frm, row=row,
                                 text="Change parameters",
                                 fr_text="Changer les paramètres")
        self.parameters_menu.add_item(self.sep0)

        # -------------- action buttons
        row += 1

        label = GuiLabel(self, self.frm, column=0, row=row, text="Configure", fr_text="Configurer",
                         width="", bold=True)
        self.parameters_menu.add_item(label)

        self.configure_home_button = GuiButton(self, self.frm, row=row, column=1,
                                               text="Config HOME",
                                               fr_text="Config HOME", command=self.launch_configure_home)
        self.parameters_menu.add_item(self.configure_home_button)

        self.configure_base_button = GuiButton(self, self.frm, row=row, column=2,
                                               text="Config BASE",
                                               fr_text="Config BASE", command=self.launch_configure_base)
        self.parameters_menu.add_item(self.configure_base_button)

        self.configure_other_button = GuiButton(self, self.frm, column=3, row=row,
                                                text="Config Son",
                                                fr_text="Config Sous Rep",
                                                command=self.launch_configure_other)

        self.parameters_menu.add_item(self.configure_other_button)

        self.undo_button = GuiButton(self, self.frm, column=4, row=row,
                                     text="Cancel Param",
                                     fr_text="Annuler Param",
                                     command=lambda: self.parameters_menu.hide() or self.configure_menu.hide(),
                                     background=PwpGui.ORANGE)
        self.parameters_menu.add_item(self.undo_button)
        ...
        self.bat_radio = GuiRadios(self, self.frm, row=row, column=0,
                                   ...
                                   fr_dico={"true": "oui", "false": "non"},
                                   width=20)
        self.configure_menu.add_item(self.bat_radio)

        self.bat_radio_label = GuiLabel(self, self.frm, column=6, row=row,
                                        text="Allows to start piwiPre from the file explorer",
                                        fr_text="Permet de démarrer piwiPre depuis l'explorateur de fichiers",
                                        width=55)
        self.configure_menu.add_item(self.bat_radio_label)
        ...
        if father.do_tests:
            ...
            self.root.after(1000, self.scenario_0)
        self.from_python_to_ui()

    def gui_change_parameters(self):
        self.parameters_menu.show()  # unhide the change parameters menu
    ...
    def scenario_0(self):
        ...
        self.root.after(2 * 1000, self.scenario_0p1)
        val_field.modify(x=50, y=50)

    def scenario_0p1(self):
        val_field: ValueField = self.multi_level.all_lines['ssh-user']
        editor: GuiStringEditor = val_field.editor
        editor.choose()
        self.root.after(1000, self.scenario1)
    ...
    def scenario1(self):
        ...
        self.root.after(3 * 1000, self.scenario2)
    ...
    def scenario2(self):
        ...
        self.root.after(4 * 1000, self.scenario3)
        album.modify()
    ...
    def scenario3(self):
        ...
        self.root.after(5 * 1000, self.run)
        self.root.after(5500, self.scenario_suicide)
        thumbnails.modify()

    def scenario_suicide(self):
        if self.suicide:
            # this is set to True when piwiPre is finished
            self.exit()
        else:
            self.root.after(200, self.scenario_suicide)
    ...
    def display_multilevel(self, start_row):
        ...
        if equal_path(os.path.dirname(self.do_dir_to_configure.get()), self.configurator.base):
            my_range = range(0, 3)
        else:
            my_range = range(1, 3)
    ...
    def refresh_main_buttons(self):
        ...
        self.msg(f"BASE is configured : '{self.configurator.base_is_configured}'")

        self.home_status.set(self.configurator.home_is_configured)
        self.base_status.set(self.configurator.base_is_configured)
        self.do_dir_to_configure.set(self.configurator.dir_to_configure)

        # enable or not the configure buttons depending on home/base configured or not

        self.configure_home_button.enable()
        if self.configurator.home_is_configured:
            self.configure_base_button.enable()
            if self.configurator.base_is_configured:
                self.configure_other_button.enable()
        else:
            self.configure_base_button.disable()
            self.configure_other_button.disable()
    ...
    def launch_change_home(self):
        x, y = self.set_home_button.get_xy()
        GuiDirChooser(self, os.path.abspath(self.initial_home),
                      "HOME",
    ...
    def launch_change_base(self):
        x, y = self.set_base_button.get_xy()
        GuiDirChooser(self, os.path.abspath(self.do_base.get()),
                      "BASE",
                      home=self.initial_home,
                      base=self.initial_base,
                      called=self.gui_set_base,
                      x=x + 10, y=y + 10,
                      language=self.language)

    def gui_set_base(self, path):
        self.do_base.set(os.path.abspath(path))
        # we want to evaluate again the configuration, because BASE has changed, hence the default values
        self.gui_set_dir(self.do_dir_to_configure.get())
        # self.from_ui_to_python_to_ui() is called by gui_set_dir()

    def launch_configure_other(self):
        ...
        # then, configure it
        self.gui_set_dir(self.do_base.get())
        x = self.configure_other_button.winfo_rootx()
        y = self.configure_other_button.winfo_rooty()
        GuiDirChooser(self, os.path.abspath(self.do_dir_to_configure.get()),
                      "Other directory",
                      home=self.initial_home,
                      base=self.initial_base,
                      called=self.continue_configure_other,
                      x=x + 10, y=y + 10,
                      language=self.language)

    def continue_configure_other(self, path):
        self.configure_menu.show()
        self.gui_set_dir(path)

    def launch_configure_base(self):
        self.configure_menu.show()
        self.gui_set_dir(self.do_base.get())

    def launch_configure_home(self):
        self.configure_menu.show()
    ...
    def from_ui_to_python(self):
        ...
        self.configurator.base_is_configured = os.path.isfile(self.configurator.base + '/piwiPre.ini')
        if self.configurator.home_is_configured:
            if not self.configurator.base_is_configured:
                self.configurator.dir_to_configure = self.configurator.base
        else:
    ...
    def from_python_to_ui(self):
        ...
        self.do_base.set(self.configurator.base)
        self.base_status.set(self.configurator.home_is_configured)
    ...
    def run_no_save(self):
        # TODO: Verify that we take into account unsaved arguments
        self.from_ui_to_python()
        self.configurator.run()
...
class PwpConfigurator:
    def __init__(self,
        ...
        self.base_is_configured = os.path.isfile(self.base + '/piwiPre.ini')
        ...
        if self.home_is_configured:
            if not self.base_is_configured:
                target = self.base
        else:
            target = self.home
