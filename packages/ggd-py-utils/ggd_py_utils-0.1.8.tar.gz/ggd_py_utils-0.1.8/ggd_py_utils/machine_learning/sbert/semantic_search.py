...
def load_resources(
        ...
        df_data_file:str,
    ) -> tuple[SentenceTransformer, object, DataFrame]:
...
def semantic_search(
    ...
    model : SentenceTransformer
        The SentenceTransformer model to use for the search.
    index : object
    ...
    query_vector:list = model.encode(sentences=query_cleaned).tolist()
    
    from numpy import expand_dims
    
    query_vector = expand_dims(query_vector, axis=0) if len(query_vector.shape) == 1 else query_vector
