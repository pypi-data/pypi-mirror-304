...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        try:
            ...
            #     repo_or_dir=f"{self.DSPEECH_HOME}/silero-vad",
            #     model="silero_vad",
            #     verbose=False,
            #     onnx=silero_use_onnx,
            #     source="local"
            # )
            self.silero_vad_model=SileroVAD()
...
# Just use fast_whisper to transcribe the audio
class DSpeech_Whisper:
    def __init__(self, model='tiny', device='auto', compute_type='float16', device_index=0):
        ...
        self.model=WhisperModel(
            ...
            download_root=f"{os.environ['DSPEECH_HOME']}"
        )
        self.device=device
    ...
    def generate(self, input_data, language=None, hotwords=None):
        # if input_data is a existing file path
        if isinstance(input_data, str) and os.path.exists(input_data):
            r =  self.model.transcribe(input_data, language=language, hotwords=hotwords)
            # r if Tuple[Iterable[Segment], TranscriptionInfo]:
            # change it to [{"text":<all_text>, "segments":<segments>, "info":<info>}, ...]
            all_text = " ".join([seg.text for seg in r[0]])
            return [{"text":all_text, "segments":r[0], "info":r[1]}]
            return self.transcribe_file(input_data, language=language, hotwords=hotwords)
        else:
            # if is a audio data is a 1D audio data
            # if isinstance(input_data, np.ndarray) and len(input_data.shape) == 1:
            # save input_data to a temp file
            r =  self.model.transcribe(input_data, language=language, hotwords=hotwords)
            # r if Tuple[Iterable[Segment], TranscriptionInfo]:
            # change it to [{"text":<all_text>, "segments":<segments>, "info":<info>}, ...]
            all_text = " ".join([seg.text for seg in r[0]])
    ...
    # def transcribe_multi_segment_lang_detection_file(self, audio_file,
    #                                                 beam_size=5,
    #                                                 vad_filter=True,
    #                                                 word_timestamps=True):
    #     segments, info = self.model.detect_language_multi_segment(audio_file,
    #                             beam_size=beam_size, 
    #                             vad_filter=vad_filter, 
    #                             word_timestamps=word_timestamps)
    #     print(info)
    ...
    def init_batch_transcription(self):
        self.batch_model=BatchedInferencePipeline(self.model)
    
    def transcribe_batch(self, audio_file, batch_size=10, beam_size=5, vad_filter=True, word_timestamps=True, language=None, hotwords=None):
        segments, info = self.batch_model.transcribe(audio_file,
                                batch_size=batch_size, 
                                beam_size=beam_size,
                                vad_filter=vad_filter,
                                word_timestamps=word_timestamps,
