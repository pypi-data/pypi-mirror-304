...
class SparkDFWrapper(DataFrameWrapper):
    ...
    def capture_column_statistics(
        ...
        stats_config = client.is_feature_flag_enabled("stats-calculation-configuration")

        column_split_by_type = self.__get_list_column_split_by_type__(
            list_col_schema, maximum_columns_for_statistics, stats_config
        )
        result_numerical, result_str, result_bool, result_date = self.__compute_spark_stats__(
            column_split_by_type, sample_rows_for_statistics, stats_config
        )
    ...
    def __compute_spark_stats__(
        self, column_values: Dict[str, list], sample_rows_for_statistics: int, stats_config: bool
        ...
        if stats_config and sample_rows_for_statistics > 0 and sample_rows_for_statistics < total_rows:
    ...
    def __get_list_column_split_by_type__(
        self, list_col_schema: list[Column], maximum_columns_for_statistics: int, stats_config: bool
    ) -> Dict[str, list]:
        column_split_by_type: Dict[str, list] = {"numerical": [], "string": [], "boolean": [], "date": []}

        for idx, col in enumerate(list_col_schema):
            if stats_config and idx >= maximum_columns_for_statistics:
                break
