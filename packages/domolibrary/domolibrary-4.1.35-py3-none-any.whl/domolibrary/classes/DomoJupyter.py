...
class DomoJupyterDataSource:
    ...
    async def _from_json(cls, obj, dj_workspace):
        ...
        try:
            domo_dataset = await dmds.DomoDataset.get_from_id(auth=auth, dataset_id=dataset_id)
            is_exists = True
...
class DomoJupyterAccount:
    ...
    async def _from_json(cls, obj, dj_workspace):
        ...
        try:
            domo_account = await dmacc.DomoAccount.get_by_id(auth=dj_workspace.auth, account_id=account_id)
            is_exists = True
...
async def get_by_id(
    ...
    return await cls._from_json(auth=auth, obj=res.response, jupyter_token=jupyter_token)
