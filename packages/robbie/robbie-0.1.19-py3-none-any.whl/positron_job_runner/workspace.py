...
def upload_results_to_s3():
    ...
    try:
        ...
        results_tar_file_name = f"{results_dir}/result.tar.gz"
        with tarfile.open(results_tar_file_name, "w:gz") as tar:
            for file in result_files:
                logger.debug(f"Adding to tar: {file.name}")
                tar.add(file.full_path, arcname=file.name)

        file_size = os.path.getsize(results_tar_file_name)
        if (file_size >= FILE_SIZE_THRESHOLD):
            size_in_mb = round(file_size / (1024 * 1024), 2)
            logger.warn(f"Size: {size_in_mb} Mb. It might take a long time to upload it.")

        logger.debug(f"Uploading to cloud storage: {results_tar_file_name}")
        cloud_storage.upload_file(results_tar_file_name, f"{S3_RESULT_PATH}/result.tar.gz")
