...
def beams_and_circles(max_iterations: int) -> None:
    # Create the domain
    ...
    circles = components.ComponentGroup(
        topology_description_function=components.circle(
            Point2D(*domain.node_coordinates),
        ),
        variable_initial=np.array([[1.0, 0.5, 0.1]]),
        variable_mins=np.array([0.0, 0.0, 0.05]),
        variable_maxes=np.array(
            [domain.dims.x, domain.dims.y, np.min(domain.dims) / 2],
        ),
    )
    ...
    plot.save_component_animation(
        ...
        filename="multi_component_example",
    )
...
if __name__ == "__main__":
    ...
    beams_and_circles(args.iterations)
