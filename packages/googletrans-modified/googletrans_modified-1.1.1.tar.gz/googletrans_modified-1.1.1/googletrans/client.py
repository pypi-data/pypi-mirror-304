...
class Translator:
    ...
    def __init__(
            ...
            proxies: dict[str, httpcore.AsyncHTTPProxy] = None,
        ...
        if service_urls:
            ...
            self.client_type = 'webapp'
            self.tok1en_acquirer = TokenAcquirer(client=self.client, host=self.service_urls[0])

            # if we have a service url pointing to client api we force the use of it as default
            # client
            for _ in enumerate(service_urls):
                if re.search('googleapis', service_urls[0]):
