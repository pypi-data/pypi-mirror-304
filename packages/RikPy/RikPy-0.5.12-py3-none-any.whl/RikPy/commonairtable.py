...
def airtable_fetch_filtered_rows(table_name, filter_formula="", fields="", exclude_fields=[]):
    ...
    try:
        ...
        # Fetch records from the table based on the filter formula
        if fields is None or (isinstance(fields, str) and fields == ""):
            records = table.all(formula=filter_formula)
        else:
            records = table.all(formula=filter_formula, fields=fields)

        # Optional: Process records here if necessary
        # Exclude specified fields
        if exclude_fields is not None and exclude_fields != "":
            records = [
                {k: v for k, v in record['fields'].items() if k not in exclude_fields} 
...
def airtable_fetch_specific_row(table_name, record_id):
    """
...
def main():
    ...
    fields=None
    fields = ['team', 'campaign_feed']
    exclude_fields = ['Generation_content', 'Banner_groups'] 
    ...
    records = custom_response.data
    
    for record in records:
        print(record)  # Prints each record retrieved
