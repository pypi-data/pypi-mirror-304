...
class AIGateway:
    ...
    def _select_model(self, weights, exclude=None):
        ...
        sorted_models = sorted(self.model_usage_count.items(), key=lambda x: (x[1], random.random()))
        
        # Filter out excluded models and models not in weights
        available_models = [model for model, _ in sorted_models 
                          if model in weights and model not in (exclude or set())]
