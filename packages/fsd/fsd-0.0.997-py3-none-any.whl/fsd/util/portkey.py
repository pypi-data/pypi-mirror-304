...
class AIGateway:
    ...
    async def coding_prompt(self, 
        ...
        current_model_index = random.randint(0, 1)  # Randomly select the initial model
        initial_model_index = current_model_index

        for attempt in range(3):
            model = models[current_model_index]
            try:
                logger.debug(f"Attempt {attempt + 1}: Attempting coding with {model}")
                ...
                logger.error(f"v{2 if current_model_index == 0 else 1} failed, trying next model")
                
                if attempt == 0:
                    # Switch to the other model
                    current_model_index = 1 - current_model_index
                elif attempt == 1:
                    # Switch back to the initial model
                    current_model_index = initial_model_index
            
            if attempt < 2:
                logger.error(f"Model failed in attempt {attempt + 1}. Retrying with {'another' if attempt == 0 else 'the initial'} model in 1-3 seconds...")
                await asyncio.sleep(random.uniform(1, 3))
        
        logger.debug("Bedrock model failed, falling back to Azure models")
        azure_models = [self.azure1_model, self.azure2_model]
        for azure_model in azure_models:
            try:
                logger.debug(f"Attempting to use {azure_model.__class__.__name__} for coding")
                completion = await azure_model.prompt(messages, max_tokens, temperature, top_p)
                logger.debug(f"Successfully received response from {azure_model.__class__.__name__}")
                return completion
            except Exception as e:
                logger.error(f"Error in prompting {azure_model.__class__.__name__}: {str(e)}")
            ...
            while error_429_count < 2:
                ...
                try:
                    ...
                    if "429" in str(e):
                        ...
                        logger.error(f"Error in generating image {model_type} model, retrying...")
                        break
            tried_models.add(model_type)
