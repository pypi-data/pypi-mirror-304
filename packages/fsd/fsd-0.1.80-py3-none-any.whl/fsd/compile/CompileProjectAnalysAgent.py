...
class CompileProjectAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language, file_attachments, focused_files, assets_link):
        ...
        prompt = (
             f"Execute ONLY what the user explicitly requests - do not add any extra tasks or suggestions. Here is the user's request:\n{user_prompt}\n\n"
             f"Respond using the language: {original_prompt_language}\n\n"
             "Important: Focus solely on the specific task requested. Do not suggest additional work or improvements beyond what was directly asked for."
        )
