...
class DependencyProjectAnalysAgent:
    ...
    def initial_setup(self, dependency_files, OS_architecture):
        ...
        system_prompt = (
            ...
            "5. For package.json initialization:\n"
            "   - Must include 'build' script for production builds\n"
            "   - Must include 'dev' script for development environment\n" 
            "   - Must include 'start' script to run production build\n"
            "   - Must configure all necessary build tools and dependencies\n"
            "   - Must set up proper entry points for both dev and production\n"
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language, file_attachments, focused_files, assets_link):
        prompt = (
            f"Focus only on installing the specific dependencies requested by the user. Do not add any extra tasks:\n\n"
            f"{user_prompt}\n\n"
            "Provide only the necessary installation commands with clear formatting.\n\n"
