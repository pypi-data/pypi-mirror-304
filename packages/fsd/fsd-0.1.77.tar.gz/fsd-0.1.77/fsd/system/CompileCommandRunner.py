...
class CompileCommandRunner:
    ...
    def run_command(self, command, method='bash'):
        ...
        Implements a timeout for specific long-running commands.
        """
        markdown_block_open = False  # Flag to track if markdown block is open
        process = None  # Initialize process variable
        timeout = None  # Default timeout (None means no timeout)

        # Define commands that require a timeout (add more as needed)
        timeout_commands = ['localhost run', 'localhost build']

        try:
            # Determine if the command requires a timeout
            if any(command.startswith(cmd) for cmd in timeout_commands):
                timeout = 5  # seconds
                logger.info(f"Command '{command}' is identified as a long-running task with a timeout of {timeout} seconds.")
            ...
            while True:
                ...
                if timeout and (time.time() - start_time) > timeout:
                    # Timeout reached for specific commands
                    logger.info(f"Timeout of {timeout} seconds reached for command '{command}'. Proceeding without waiting for completion.")
                    break
                time.sleep(0.1)  # Prevent busy waiting

            # If the process is still running and a timeout was set, we don't kill it.
            # Instead, we just proceed, allowing the subprocess to continue running.
            if timeout and (time.time() - start_time) > timeout and process.poll() is None:
                return_code = None  # Indicate that the process is still running
                logger.info("```")  # Close the markdown block if open
                logger.info(
                    f"#### Command is still running after timeout.\n"
                    f"----------------------------------------"
                )
                logger.info("The `CommandRunner` has initiated the command and is proceeding to the next step without waiting for its completion.")
                return return_code, output

            # If no timeout or process finished before timeout
            ...
            markdown_block_open = False  # Markdown block is now closed
