    ...
    async def process_image_generation_pro(self, steps: List[Dict]):
        ...
        async def generate_image_task(step):
            try:
                ...
                encoded_path = urllib.parse.quote(step['file_path'])
                logger.info(f"{os.path.basename(step['file_path'])}\n")
                logger.info(f"{encoded_path}\n")
                logger.info(f"![{filename}]({encoded_path})")
            except Exception as e:
