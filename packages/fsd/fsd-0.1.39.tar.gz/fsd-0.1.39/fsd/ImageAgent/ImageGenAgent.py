...
class ImageGenAgent:
    ...
    async def process_image_generation_pro(self, steps: List[Dict]):
        ...
        Ensures that image paths with spaces are correctly handled for Markdown rendering.

        Args:
            steps (List[Dict]): A list of dictionaries containing image generation parameters.
        """
        semaphore = asyncio.Semaphore(5)  # Limit to 5 concurrent tasks

        async def generate_image_task(step: Dict):
            async with semaphore:
                try:
                    ...
                    absolute_path = Path(step['file_path']).resolve()

                    # Calculate the relative path from the current working directory
                    try:
                        relative_path = absolute_path.relative_to(Path.cwd())
                    except ValueError:
                        # If the image is not within the current working directory, use absolute path
                        relative_path = absolute_path

                    # Convert to a POSIX-style path (uses forward slashes)
                    posix_path = relative_path.as_posix()

                    # Encode the path, preserving forward slashes
                    encoded_path = urllib.parse.quote(posix_path, safe="/")

                    # Construct the Markdown image syntax
                    markdown_image = f"![{filename}]({encoded_path})"

                    # Log the Markdown image syntax
                    logger.info(markdown_image)

                except Exception as e:
                    logger.error(f"#### Failed to generate image for step {step}: {str(e)}")

        # Create a list of tasks
        tasks = [generate_image_task(step) for step in steps]
    ...
    async def process_image_generation(self, steps: List[Dict]):
        """
        Processes each image generation step sequentially, ensuring that image paths with spaces
        are correctly handled for Markdown rendering.
