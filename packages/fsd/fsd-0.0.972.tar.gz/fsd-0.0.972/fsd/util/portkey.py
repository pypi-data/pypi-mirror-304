...
class AIGateway:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.azure_model1 = AzureModel(cls.API_KEY, cls.VIRTUAL_KEYS["azure1"], cls.CONFIG_ID)
            cls._instance.azure_model2 = AzureModel(cls.API_KEY, cls.VIRTUAL_KEYS["azure2"], cls.CONFIG_ID)
    ...
    async def coding_prompt(self, 
        ...
        for attempt in range(3):
            try:
                ...
                completion = await asyncio.to_thread(self.portkey.chat.completions.create, **common_params)
                logger.debug(f"Successfully received response from {model_type} model for coding")
                return completion
            except Exception as e:
                logger.error(f"Error in prompting {model_type} model for coding with v2: {str(e)}")
                if attempt < 2:
                    logger.debug(f"Retrying with v1 model...")
                    try:
                        common_params["model"] = "anthropic.claude-3-5-sonnet-20240620-v1:0"
                        completion = await asyncio.to_thread(self.portkey.chat.completions.create, **common_params)
                        logger.debug(f"Successfully received response from {model_type} model (v1) for coding")
                        return completion
                    except Exception as e2:
                        logger.error(f"Error in prompting {model_type} model for coding with v1: {str(e2)}")
        ...
        while len(tried_models) < len(self.IMAGE_MODEL_WEIGHTS):
            ...
            try:
                if model_type.startswith("dalle3"):
                    image = getattr(self, f"{model_type}_model").generate_image(prompt, size)
                else:
                    model_id = {
