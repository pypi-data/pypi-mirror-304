...
class AIGateway:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.bedrock2_model = BedrockModel2(cls.API_KEY, cls.VIRTUAL_KEYS["bedrock"], cls.CONFIG_ID)
    ...
    async def arch_prompt(self, 
        ...
        while len(tried_models) < len(self.Architect_MODEL_WEIGHTS):
            ...
            try:
                ...
                logger.error(f"Error in prompting {model_type} model for architecture: {str(e)}")
                tried_models.add(model_type)
                await asyncio.sleep(2)
