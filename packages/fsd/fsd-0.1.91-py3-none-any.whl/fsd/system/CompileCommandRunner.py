...
class CompileCommandRunner:
    ...
    def run_command(self, command: str, method: str = 'bash') -> Tuple[int, List[str]]:
        """
        ...
        inactivity_timeout = 5  # Seconds to wait for new output before considering done
        ...
        try:
            ...
            if process.poll() is None:
                try:
                    logger.info(f"Attempting to terminate the process group after inactivity timeout of {inactivity_timeout} seconds.")
                    ...
                    try:
                        ...
                        logger.info(f"Process group {os.getpgid(process.pid)} terminated gracefully.")
                    except subprocess.TimeoutExpired:
                        logger.info("Process did not terminate after SIGTERM; sending SIGKILL.")
