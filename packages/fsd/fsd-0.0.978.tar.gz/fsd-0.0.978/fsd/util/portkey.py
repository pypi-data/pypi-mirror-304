...
class AIGateway:
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        ...
        while len(tried_models) < len(self.IMAGE_MODEL_WEIGHTS):
            ...
            tried_models.add(model_type)
            logger.info(f"Attempting to use {model_type} model for image generation")
            try:
                if model_type.startswith("dalle3"):
                    logger.info(getattr(self, f"{model_type}_model"))
                ...
                logger.error(f"Error in generating image with {model_type}: {str(e)}")
