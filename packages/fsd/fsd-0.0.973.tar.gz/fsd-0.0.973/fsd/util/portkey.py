...
class AIGateway:
    ...
    async def coding_prompt(self, 
        ...
        for model in models:
            for attempt in range(3):
                try:
                    logger.debug(f"Attempt {attempt + 1} for coding")
                    completion = await self.bedrock.coding_prompt(messages, model)
                    logger.debug(f"Successfully received response from {model} for coding")
                    return completion
                except Exception as e:
                    logger.debug(f"Error in prompting for coding: {str(e)}")
                    if attempt < 2:
                        logger.debug(f"Retrying in 2-3 seconds...")
                        await asyncio.sleep(random.uniform(2, 3))
                    else:
                        logger.error(f"All attempts failed for {model}, trying next model if available")
