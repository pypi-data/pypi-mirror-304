...
class AIGateway:
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024", style: str = "0", tier: str = "Free"):
        ...
        # If style is 0, try SDXL first for both tiers with random bedrock model
        if style == "0":
            # Try first random model
            bedrock_model = random.choice([self.bedrock_model, self.bedrock2_model])
            try:
                return bedrock_model.generate_image(prompt, model="stability.stable-diffusion-xl-v1")
            except Exception as e:
                logger.debug(f"First SDXL attempt failed with {bedrock_model.__class__.__name__}: {str(e)}")
                # Try the other model
                backup_model = self.bedrock2_model if bedrock_model == self.bedrock_model else self.bedrock_model
                try:
                    return backup_model.generate_image(prompt, model="stability.stable-diffusion-xl-v1")
                except Exception as e:
                    logger.debug(f"Second SDXL attempt failed with {backup_model.__class__.__name__}: {str(e)}")
        
        while len(tried_models) < len(model_weights):
            model_type = self._select_model(model_weights, exclude=tried_models)
            try:
