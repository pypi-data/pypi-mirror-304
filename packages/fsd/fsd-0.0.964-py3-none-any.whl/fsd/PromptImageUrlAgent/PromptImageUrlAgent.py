...
class PromptImageUrlAgent:
    ...
    async def get_image_links(self, idea):
        ...
        prompt = (
            "Analyze the provided development plan and extract any image links (png, jpg, jpeg, webp) that can be converted to base64. "
            ...
            "}\n\n"
            "If no valid image links are found, return an empty list:"
            "{\n"
            "    \"assets_link\": []\n"
            "}\n\n"
    ...
    async def process_image_links(self, idea):
        ...
        return await self.get_image_links(idea)
