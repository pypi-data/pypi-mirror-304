...
_prefix: str = env_get_str(key=f"{APP_PREFIX}_S3_ENGINE",
                           values=[e.value for e in S3Engine])
if _prefix:
    _default_setup: bool = True
    _S3_ENGINES.append(S3Engine(_prefix))
else:
    _default_setup: bool = False
    # noinspection PyUnresolvedReferences
    _engines: str = env_get_str(key=f"{APP_PREFIX}_S3_ENGINES",
                                values=[e.value for e in S3Engine])
    if _engines:
        _S3_ENGINES.extend([S3Engine(v) for v in _engines.split(sep=",")])
...
def _except_msg(exception: Exception,
    ...
    """
    # noinspection PyTypeChecker
    endpoint: str = _S3_ACCESS_DATA[engine].get(S3Param.ENDPOINT_URL)
