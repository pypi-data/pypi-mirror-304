    ...
    *,
    ...
    try:
        ...
        for package in installed_packages:
            ...
            # console.print(f"Successfully installed {package_name} to {find_toml_file()} {'for ' + hatch_env if hatch_env else ''}",style="bold light_goldenrod2")
...
@click.option("--manager", "-m", type=click.Choice(["hatch", "uv"]), default="uv", help="Package manager to use")
@click.option(
    "--auth",
    "-a",
    default=os.getenv("PYPI_TOKEN"),
    help="PyPI authentication token. Defaults to PYPI_TOKEN environment variable.",
)
def publish_command(bump=False, build=False, manager="uv", auth=None) -> None:
    """Publish a package to PyPI."""
    if not auth:
        auth = os.getenv("PYPI_TOKEN")
    try:
        if bump:
            version = bump_pkg()
        if build:
            run_command([manager, "build"], show=True).readlines()
        publisher = ["hatch", "publish"] if manager == "hatch" else ["twine", "upload", "dist/*"]
        run_command([*publisher, "-u", "__token__", "-p" if manager == "uv" else "-a", auth], show=True).readlines()
        console.print(f"Package published successfully with version {version}", style="bold light_goldenrod2")
    except Exception:
        traceback.print_exc()
