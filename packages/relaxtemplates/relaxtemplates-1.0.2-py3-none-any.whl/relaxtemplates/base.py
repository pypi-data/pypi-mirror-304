...
# Update the token regex to include comment tokens
...
class _Fragment(object):
    ...
    def clean_fragment(self):
        if self.raw[:2] in (VAR_TOKEN_START, BLOCK_TOKEN_START, COMMENT_TOKEN_START):
...
class Template:
    def __init__(self, template_name, context, base_dir="templates"):
    ...
    def load_template(self, template_name):
        template_path = os.path.join(self.base_dir, f"{template_name}.html")
        try:
            with open(template_path, 'r') as f:
                return f.read()
        except FileNotFoundError:
            raise TemplateError(f"Template '{template_name}' not found at path '{template_path}'")

    def compile_template(self, content):
        # Check if template extends another template
        extend_match = re.search(r"{%\s*extend\s+'(.+?)'\s*%}", content)
        if extend_match:
            base_template_name = extend_match.group(1)
            base_content = self.load_template(base_template_name)
            
            # Extract and store blocks from the child template
            self.extract_blocks(content)

            # Replace blocks in the base template
            content  = self.replace_blocks(base_content)
            # Handle include directive
            content = self.handle_includes(content)
            return content
        else:
            return content

    def extract_blocks(self, content):
        block_regex = re.compile(r"{%\s*block\s+(\w+)\s*%}(.*?){%\s*endblock\s*%}", re.S)
        for match in block_regex.finditer(content):
            block_name, block_content = match.groups()
            self.blocks[block_name] = block_content.strip()

    def replace_blocks(self, base_content):
        for block_name, block_content in self.blocks.items():
            block_pattern = re.compile(r"{%\s*block\s+" + re.escape(block_name) + r"\s*%}.*?{%\s*endblock\s*%}", re.S)
            base_content = block_pattern.sub(block_content, base_content)
        return base_content
    
    def handle_includes(self, content):
        """Replaces include directives with the content of the included templates."""
        include_regex = re.compile(r"{%\s*include\s+'(.+?)'\s*%}")
        while True:
            match = include_regex.search(content)
            if not match:
                break
            included_template_name = match.group(1)
            included_content = self.load_template(included_template_name)
            content = content[:match.start()] + included_content + content[match.end():]
        return content

    def render(self):
        # Compile the final content with the base template and overrides
        compiled_content = self.compile_template(self.template_content)
        # Compile the regex variables
        return Compiler(compiled_content).compile().render(self.context)
