...
def answer_question(question,document_id):
    try:
        ...
        model = RAG()
        pred=model(question)
        response_json=read_answer(pred.answer)
...
async def extract_legal_clauses(paragraphs):
    ...
    try:
        ...
        for paragraph in paragraphs:
            ...
            joined_text +=text+" \n "
            joined_ids.append(str(paragraph['no_paragraph']))

            if text.endswith("."):
                joined_paragraphs.append(joined_text)
                joined_paragraphs_ids.append(joined_ids)
                joined_text=""
                joined_ids=[]
        ...
        return (response_batches,paragraphs,containername,joined_paragraphs_ids)
    except Exception as exc:
        ...
        with Pool(processes=10) as pool:
            for key in response_batches:
                ...
                if len(ids)>0:
                    ...
                    input_data=InputLegalClause(prompt,clause_name,containername,ids)
...
async def llm_review_clause(input_data:InputLegalClause):
    try:
        ...
        response_json.update(paragraph_ids=input_data.paragraph_ids)
...
def read_answer(xml_string):

    open_tag="<answer>"
    close_tag="</answer>"
    start = xml_string.find(open_tag)
    end = xml_string.find(close_tag)
    answer = xml_string[start + len(open_tag):end]
    
    start = answer.find("<text>")
    end = answer.find("</text>")
    text = answer[start + len("<text>"):end]

    start = answer.find("<document>")
    end = answer.find("</document>")
    document = answer[start + len("<document>"):end]

    start = document.find("<offset_start>")
    end = document.find("</offset_start>")
    offset_start = document[start + len("<offset_start>"):end]

    start = document.find("<offset_end>")
    end = document.find("</offset_end>")
    offset_end = document[start + len("<offset_end>"):end]

    start = document.find("<number_of_document>")
    end = document.find("</number_of_document>")
    number_of_document = document[start + len("<number_of_document>"):end]


    return {"text":text,"offset_start":offset_start,"offset_end":offset_end,
