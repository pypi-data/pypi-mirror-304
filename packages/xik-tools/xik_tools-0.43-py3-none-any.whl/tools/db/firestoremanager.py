...
class FirestoreManager:
    ...
    def save_list(self, dict_list: dict):
        """
        dict_list = {doc_key: data_list}
        """
        for doc_key, data_list in dict_list.items():  # .items() 추가
            self.db.document(doc_key).set({'data_list': data_list})

    
    def read_list(self, doc_key):
        doc = self.db.document(doc_key).get()  
        if doc.exists:
