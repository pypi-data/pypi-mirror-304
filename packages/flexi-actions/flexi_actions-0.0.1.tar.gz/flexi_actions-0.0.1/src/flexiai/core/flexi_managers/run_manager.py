...
class RunManager:
    ...
    def wait_for_run_completion(self, thread_id):
        """
        Waits for a specific run in the thread to complete.
        ...
        try:
            while True:
                ...
                self.logger.info(f"Current status of run {run}: {run.status}")

                if run.status in ["queued", "in_progress", "cancelling", "requires_action"]:
                    self.logger.info(f"Run {run} is currently {run.status}. Waiting for completion...")
                    time.sleep(1)  # Wait for 1 second before checking again
                else:
                    self.logger.info(f"Run {run} has completed with status {run.status}.")
                    break
        except OpenAIError as e:
            self.logger.error(f"Failed to retrieve run {run} for thread {thread_id}: {str(e)}", exc_info=True)
            raise
        except Exception as e:
            self.logger.error(f"An unexpected error occurred while waiting for run {run} completion in thread {thread_id}: {str(e)}", exc_info=True)
            raise
