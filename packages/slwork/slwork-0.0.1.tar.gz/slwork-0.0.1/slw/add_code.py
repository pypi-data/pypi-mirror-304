# from bearcolor import bearprint as prt
...
from slw.slw_const import DATA_PATH, CACHE_PATH
...
def load_pd_data(fname, sname, index_col=False, usecols=None ):

    if not os.path.exists(CACHE_PATH):
        os.makedirs(CACHE_PATH)
    tmp_file = CACHE_PATH / f"xx_get_data1_{fname}_{sname}_{index_col}_{usecols}.pkl"
...
def load_np_data(fname, sname, index_col=False, usecols=None ):
    fpath = DATA_PATH

    tmp_file = Path(
        f"{CACHE_PATH}/xx_get_data2_{fname}_{sname}_{index_col}_{usecols}.pkl"
    )
