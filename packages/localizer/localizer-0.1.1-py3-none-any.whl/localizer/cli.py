...
@click.pass_context
def cli(ctx):
    """localizer - Translation Management Tool"""
    ctx.obj = localizerConfig()
...
def validate_reference(config: localizerConfig, reference_path: str):
    """
    Validate a specific reference path
    
    Example: localizer validate ref tabs.index.button
    """
    try:
...
@click.pass_obj
def validate_docs(config: localizerConfig):
    """
    Validate all references in docs.md against reference.yaml
    
    Example: localizer validate docs
    """
    try:
        reference_data = config.read_reference()
...
def generate_translation(config: localizerConfig, lang_code: str, output: str):
    """
    Generate translation for specified language
    
    Example: localizer translate generate ko -o ko.json
    """
    try:
        ...
        if lang_code not in config.languages:
            raise click.ClickException(
                ...
                "Add it first with: localizer config --add-language {lang_code}"
            )
...
def translate_all(config: localizerConfig, output: Path):
    """
    Generate translations for all configured languages
    
    Example: localizer translate all -o ./translations
    """
    try:
        ...
        if not config.languages:
            raise click.ClickException(
                ...
                "Add languages first with: localizer config --add-language <lang_code>"
            )
