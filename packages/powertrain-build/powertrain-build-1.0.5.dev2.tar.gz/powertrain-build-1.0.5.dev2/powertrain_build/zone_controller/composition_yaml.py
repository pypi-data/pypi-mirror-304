...
class CompositionYaml(ProblemLogger):
    ...
    def _get_nvm_info(self):
        ...
        """
        # TODO Assert data types in "_generate_nvm_structs_updated"?
        ...
        # TODO non value elements
        data_types = {}
        for nvm_name, nvm_data in yaml_nvm_definitions.items():
            nr_of_used_signals = len(project_nvm_definitions[nvm_name]["signals"])
            nr_of_unused_signals = project_nvm_definitions[nvm_name]["size"] - nr_of_used_signals
            data_type_name = f"dt_{nvm_name}"
            nvm_data["datatype"] = data_type_name
            nvm_data["init"] = [0] * nr_of_used_signals
            nvm_data["init"].append([0] * nr_of_unused_signals)
            data_types.update({
                data_type_name: {
                    "type": "RECORD",
                    "elements": {
                        signal["name"]: signal["type"] for signal in project_nvm_definitions[nvm_name]["signals"]
                    },
                },
                f"{data_type_name}_Unused": {
                    "type": "ARRAY",
    ...
    def _get_software_components(self):
        ...
        if self.build_cfg.get_composition_config("includeNvm"):
            swcs[software_component_name]["nv-needs"], nvm_data_types = self._get_nvm_info()
        else:
            nvm_data_types = {}
