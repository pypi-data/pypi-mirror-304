"""

This module provides a number of objects (mostly functions) useful for
embedding all functions used to compute the KKT points from the
projection of a point x0 to a quadric Q.


"""

import numpy as np
import matplotlib.pyplot as plt

eps = pow(10, -14)


def _f(Q, mu, x0):
    """Secular function.

    Given the quadric `Q` and the `n` dimensional point `x0`,
    this function evaluates the (secular) function f.

    .. math:: f(\\mu) = c + \\sum_{i=1}^n \\lambda_i \\Big ( \\frac{x^0_i - \\frac{b^2_i \\mu}{2}}{1+\\mu \\lambda_i}\\Big )^2  + \\frac{b^r_i (x^0_i - \\frac{b^r_i \\mu}{2})}{1+\\mu \\lambda_i}

    Parameters
    ----------
    Q : Quadric instance
        The quadric onto which the point x0 has to be projected.
    mu : float
        Lagrange multiplier.
    x0 : ndarray
        1-D ndarray containing the point to be projected.

    Returns
    _______
    float
        Evaluation of f defined with Q and x0 at point mu.

    """
    if isinstance(mu, float) or isinstance(mu, int):
        mu = np.ones(1) * mu
    _sum = mu * 0
    for i in range(Q.dim):
        if abs(Q.eig[i]) < eps:
            _sum += Q.b_reduced[i] * (x0[i] - Q.b_reduced[i] * mu / 2)
        else:
            num_1 = (x0[i] - Q.b_reduced[i] * mu / 2)
            idx_1 = abs(num_1) > eps
            idx_2 = abs(mu + 1 / Q.eig[i]) < eps
            idx_12 = idx_1 & idx_2
            idx_1Not2 = idx_1 & ~ idx_2
            _sum[idx_12] = -np.sign(mu[idx_12]) * np.inf
            _sum[idx_1Not2] += Q.eig[i] * pow(num_1[idx_1Not2] / (1 + mu[idx_1Not2] * Q.eig[i]), 2)
            num_2 = Q.b_reduced[i] * (x0[i] - Q.b_reduced[i] * mu / 2)
            idx_1 = abs(num_2) > eps
            idx_12 = idx_1 & idx_2
            idx_1Not2 = idx_1 & ~ idx_2
            _sum[idx_12] = -np.sign(mu[idx_12]) * np.inf
            _sum[idx_1Not2] += num_2[idx_1Not2] / (1 + mu[idx_1Not2] * Q.eig[i])
    return _sum + Q.c


def _d_f(Q, mu, x0):
    """Derivative of the secular function.

    Given the quadric `Q` and the `n` dimensional point `x0`,
    this function evaluates the derivative of the (secular) function f.

    .. math:: f'(\\mu) = -2 \\sum_{i=1}^n   -\\lambda_i \\left( b_i^r + 2 \\lambda_i x^0_i \\right) \\left( 2 x_0[i] - b_i^r \\mu \\right) \\frac{1}{2(1 + \\mu \\lambda_i)^3} - b_i^r \\left( b_i^r + 2 \\lambda_i x^0_i \\right) \\frac{1}{2 \\left( \\lambda_i \\mu + 1 \\right)^2}

    Parameters
    ----------
    Q : Quadric instance
        The quadric onto which the point x0 has to be projected.
    mu : float
        Lagrange multiplier.
    x0 : ndarray
        1-D ndarray containing the point to be projected.
    Returns
    _______
    float
        Evaluation of f' defined with Q and x0 at point mu.

    """

    _sum = 0
    for i in range(Q.dim):
        if (Q.eig[i] * mu + 1) > eps:
            _sum += -Q.eig[i] * (Q.b_reduced[i] + 2 * Q.eig[i] * x0[i]) * (2 * x0[i] - Q.b_reduced[i] * mu) / 2 / (1 + mu * Q.eig[i])**3
            _sum += - Q.b_reduced[i] * (Q.b_reduced[i] + 2 * Q.eig[i] * x0[i]) / (2 * (Q.eig[i] * mu + 1)**2)
    return _sum


def _get_e1(Q, x0):
    """Obtain largest negative pole.

    Given the quadric `Q` and the `n` dimensional point `x0`,
    this function obtain the largest negative pole of the rational function :math:`f`
    (obtained as :math:`f(\\mu)` = `fun._f(Q, mu, x0)`).

    If no such pole exists, the function returns -inf.

    Remark that poles cancel by zeros will be omitted.

    Parameters
    ----------
    Q : Quadric instance
        The quadric onto which the point x0 has to be projected.
    x0 : ndarray
        1-D ndarray containing the point to be projected.
    Returns
    _______
    e1 : float
        Largest negative pole of f.
    """

    e1 = -np.inf
    for i, x in enumerate(x0):
        if Q.eig[i] > eps and abs(x + Q.b_reduced[i] / 2 / Q.eig[i]) > eps:
            return -1 / Q.eig[i]
    return e1


def _get_e2(Q, x0):
    """Obtain smallest positive pole.

    Given the quadric `Q` and the `n` dimensional point `x0`,
    this function obtain the smallest positive pole of the rational function :math:`f`
    (obtained as :math:`f(\\mu)` = `fun._f(Q, mu, x0)`).

    If no such pole exists, the function returns inf.

    Remark that poles cancel by zeros will be omitted.

    Parameters
    ----------
    Q : Quadric instance
        The quadric onto which the point x0 has to be projected.
    x0 : ndarray
        1-D ndarray containing the point to be projected.
    Returns
    -------
    e2 : float
        Smallest positive pole of f.
    """

    e2 = np.inf
    for i, _ in enumerate(x0):
        if Q.eig[-i] < -eps and abs(x0[-i] + Q.b_reduced[-i] / 2 / Q.eig[-i]) > eps:
            return -1 / Q.eig[-i]
    return e2


class Fun():
    """A function class.

    The Fun class provides useful function definitions for computing
    KKT points.

    Parameters
    ----------
    Q : Quadric instance
        The nonempty quadric onto which the point x0 has to be projected.
    x0 : ndarray
        1-D ndarray containing the point to be projected.
    Attributes
    ----------
    x_std : function
        Given mu, returns the associated standardized x.
    x_not_std : function
        Given mu, returns the associated not standardized x.
    f : function
        Given mu, returns f(mu).
    d_f : function
        Derivative of f.
    e1 : float
        Largest negative pole of f (-np.inf if no such pole exists).
    e2 : float
        Smallest positive pole of f (np.inf if no such pole exists).
    interval : tuple of float
        Search interval for the root: (e_1, e_2).
    dist : function
        Distance function to x0_not_std.
    """

    def __init__(self, Q, x0):

        E = np.ones(Q.dim)

        def inv_I_lA(mu):
            return np.diag(1 / (E + mu * Q.eig))  # Temporary function

        def _x_std(mu):
            x = (x0 - Q.b_reduced * mu / 2)
            idx = np.where(abs(x) > eps)  # other are 0 even if denom is
            x[idx] = x[idx] / (1 + mu * Q.eig[idx])
            return x

        self.x_std2 = lambda mu: inv_I_lA(mu) @ x0
        self.x_not_std2 = lambda mu: Q.to_non_standardized(self.x_std2(mu))
        self.x_std = lambda mu: _x_std(mu)
        self.x_not_std = lambda mu: Q.V @ self.x_std(mu)

        self.f = lambda mu: _f(Q, mu, x0)
        self.d_f = lambda mu: _d_f(Q, mu, x0)

        self.e1 = _get_e1(Q, x0)
        self.e2 = _get_e2(Q, x0)
        self.interval = self.e1, self.e2
        x0_not_std = Q.to_non_standardized(x0)

        self.dist = lambda _x: np.linalg.norm(_x - x0_not_std)

    def plot(self):
        m = 100
        t = np.linspace(max(self.e1, -self.e2, -1) - 1, min(self.e2, -self.e1, 1) + 1, m)
        y = np.zeros_like(t)
        plt.figure()
        for i, _t in enumerate(t):
            y[i] = self.f(_t)
        plt.scatter(self.e1, 0, label="e1")
        plt.scatter(self.e2, 0, label="e2")
        plt.legend()
        plt.plot(t, y)
        plt.show()
