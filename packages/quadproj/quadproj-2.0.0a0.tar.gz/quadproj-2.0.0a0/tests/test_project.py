...
def test_get_e1():
    ...
    x0_not_std[2] = 3
    x0 = Q.to_standardized(x0_not_std)
    assert fun._get_e1(Q, x0) == -0.5
    assert fun._get_e2(Q, x0) == np.inf
    ...
    x0_not_std[2] = 3
    x0 = Q.to_standardized(x0_not_std)
    assert fun._get_e1(Q, x0) == -np.inf  # we have a pole/zero cancellation
    assert fun._get_e2(Q, x0) == 1 / 1.5
...
def test_project_parabola():
    ...
    print("Dist is", np.linalg.norm(x0 - x_tild))
    print("Is point feasible", Q.is_feasible(x_tild))
...
def test_elliptic_paraboloid():
    ...
    test_starting_point_from_feasible_point(dim=dim, param=param)
...
def test_starting_point_from_feasible_point(dim=3, param=None, show=False):
    ...
    if not Q.is_feasible(x_project_2):
    ...
    if Q.dim not in [2, 3]:
        if flag_raise:
            print(f"{Q.is_feasible(x_project_2)=}")
            print(f"{Q.is_feasible(x_project)=}")
            raise ValueError
        return
...
def test_projection_empty_double_plane():
    A = np.array([[-1, 0], [0, 0]])
    b = np.zeros(2)
    c = -1
    Q = Quadric(A=A, b=b, c=c)
...
def test_projection_simple_line():
    A = np.array([[1, 0], [0, 0]])
    b = np.array([-2, 0])
...
def test_projection_double_lines():
    np.random.seed(456)
...
def test_projection_ellipsoid_cylinder_2():
    ...
    assert Q.is_cylindrical and Q.sub_Q.is_ellipsoid
    x_project = project(Q, x0)
    assert Q.is_feasible(x_project)
    test_starting_point_from_feasible_point(3, param, show=SHOW)
...
def test_projection_paraboloid_cylinder():
    ...
    x_project = project(Q, x0)
    assert Q.is_feasible(x_project)
    assert np.allclose(x_project, x0)
    ...
    if SHOW:
        ...
        plot_x0_x_project(ax, Q, x0, x_project, flag_circle=True)
        plt.show()
    assert np.allclose(x_project, np.array([1, 0.5, 0.5]))


def test_projection_paraboloid_cylinder_rotated():
    A = np.array([[0.5, 0, 0], [0, 0, 0], [0, 0, 0]])
    b = np.array([0, -2, 3])
    c = -1
    Q = Quadric(A=A, b=b, c=c)
    x0 = np.array([0, 5, 6])
