...
def project(Q, x0_not_std, flag_get_all_KKT=False):
    ...
    elif Q.is_cylindrical and not Q.is_paraboloid_cylinder:

        _x_project = np.zeros(Q.dim)

        T = Q.T
        x0 = T @ Q.to_standardized(x0_not_std)

        x0_noncylindric = x0[Q.noncylindric_indices]
        _x_project[Q.cylindric_indices] = x0[Q.cylindric_indices]
        x_project_noncylindric = project(Q.sub_Q, x0_noncylindric)
        _x_project[Q.noncylindric_indices] = x_project_noncylindric
        return Q.to_non_standardized(T @ _x_project)
    ...
    for k, eig_bar_k in enumerate(eig_bar):
        ...
        while i >= 0 and abs(eig[-1] - eig_bar_k) < eps:
            ...
            L_k.append(i)
            i -= 1
        K_k = [i2 for i2 in L_k if abs(Q.eig[i2]) > eps and abs(x0[i2] + Q.b_reduced[i2] / 2 / Q.eig[i2]) < eps]
    ...
    if not Q.is_feasible(x_project_not_std):
        if Q.evaluate_point(x_project_not_std) > pow(10, -4):  # TODO TO REMOVE
...
def get_KKT_point_root(Q, x0_not_std):
    ...
    elif Q.is_paraboloid or Q.is_paraboloid_cylinder:
        if F.e2 == np.inf:
            mu_1 = 0
            output_newton = my_newton(F, mu_1)
...
def get_squared_radius_k(Q, x0, L_k, eig_bar_k):
    ...
    _sum = [Q.eig[j] * ((x0[j] - Q.b_reduced[j] * mu_k / 2) / (1 + Q.eig[j] * mu_k))**2 + Q.b_reduced[j] * (x0[j] - Q.b_reduced[j] / 2 * mu_k) / (1 + Q.eig[j] * mu_k)
            for j in range(Q.dim) if j not in L_k and j < Q.r]
    _sum2 = [Q.b_reduced[j] * (x0[j] - Q.b_reduced[j] / 2 * mu_k) / (1 + Q.eig[j] * mu_k)
             for j in range(Q.dim) if j not in L_k and j >= Q.r]
    radius_k = 1 / eig_bar_k * (-Q.c - sum(_sum) - sum(_sum2))
    return radius_k
...
def get_xd_k(Q, x0, K_k, eig_bar_k):
    ...
    """
    k_prime = K_k[0]
    xd_k = np.ones_like(x0) * (-Q.b_reduced[k_prime] / 2 / Q.eig[k_prime])
    for i, x0_i in enumerate(x0):
        mu_k = -1 / eig_bar_k
        if i not in K_k:
            xd_k[i] = (x0_i - Q.b_reduced[i] * mu_k / 2) / (1 + Q.eig[i] * mu_k)
        elif i == k_prime:
...
def my_newton(F, mu_0, **hist):
    ...
    while (output.iteration < iteration_max and output.rtol > eps_rtol and
        ...
        output.fx = F.f(x)
        output.dfx = F.d_f(x)
        if abs(output.dfx) < eps_rtol and output.fx > 0.1:
            if hist.get("df"):
