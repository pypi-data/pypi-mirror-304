...
class Quadric:
    ...
    **Remark**: We sort the eigenvalues from the largest to the smallest.
        ...
        if self.r == 0:
            ...
            if self.r < self.dim - 1 and r2 > self.r:
                ...
                for i in non_zero_cylindric_indices:
                    T[pivot_cylinder, i] = -  self.b_reduced[pivot_cylinder] / self.b_reduced[i]
                self.T = T  # currently unused
            elif r2 > self.r:
    ...
    def is_parallel_planes(self):
        return self.type == "parallel planes"

    @property
    def is_single_plane(self):
        return self.type == "single plane"
    ...
    def plot(self, show=False, path_file=None, show_principal_axes=False, **kwargs):
        ...
        **kwargs : {fig, ax}
        ...
        if dim == 1:
            ax.scatter(self.sol_R, 0, color=quadric_color, label=r'$\mathcal{Q}$', zorder=1)
            if self.is_parallel_planes:
                ax.scatter(self.sol_L, 0, color=quadric_color, zorder=2)
        elif dim == 2:
            x = np.zeros_like(T)
            y = np.zeros_like(T)
            ...
            if self.is_one_sheet_hyperboloid:
                ...
                u_y = z
                if self.gamma > 0:
                    u_x_1 = -self.sub_Q.axes[0] * np.tan(t)
                    u_x_2 = -self.sub_Q.axes[1] * np.tan(t + np.pi)
                    u_z_1 = self.sub_Q.axes[1] / np.cos(t)
                    u_z_2 = self.sub_Q.axes[1] / np.cos(t + np.pi)
                else:
                    u_z_1 = -self.sub_Q.axes[1] * np.tan(t)
                    u_z_2 = -self.sub_Q.axes[1] * np.tan(t + np.pi)
                ...
                u_x1 = self.sol_L * np.ones((m1 // 2, m2 // 2))
                u_x2 = self.sol_R * np.ones((m1 // 2, m2 // 2))
                u_y1 = t
                u_y2 = t
                u_z1 = s
                u_z2 = s
            ...
            if self.is_two_sheet_hyperboloid or self.is_hyperboloid_cylinder or self.is_elliptic_cone_cylinder or self.is_parallel_planes:
