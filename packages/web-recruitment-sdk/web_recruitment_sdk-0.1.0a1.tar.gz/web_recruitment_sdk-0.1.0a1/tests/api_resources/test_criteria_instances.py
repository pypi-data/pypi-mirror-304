# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from web_recruitment_sdk import WebRecruitmentSDK, AsyncWebRecruitmentSDK
from web_recruitment_sdk.types import CriteriaInstanceCreateResponse

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestCriteriaInstances:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: WebRecruitmentSDK) -> None:
        criteria_instance = client.criteria_instances.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        )
        assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: WebRecruitmentSDK) -> None:
        response = client.criteria_instances.with_raw_response.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        criteria_instance = response.parse()
        assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: WebRecruitmentSDK) -> None:
        with client.criteria_instances.with_streaming_response.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            criteria_instance = response.parse()
            assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncCriteriaInstances:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_create(self, async_client: AsyncWebRecruitmentSDK) -> None:
        criteria_instance = await async_client.criteria_instances.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        )
        assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.criteria_instances.with_raw_response.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        criteria_instance = await response.parse()
        assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.criteria_instances.with_streaming_response.create(
            body=[
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
                {
                    "answer": "yes",
                    "criteria_id": 0,
                    "patient_id": 0,
                },
            ],
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            criteria_instance = await response.parse()
            assert_matches_type(CriteriaInstanceCreateResponse, criteria_instance, path=["response"])

        assert cast(Any, response.is_closed) is True
