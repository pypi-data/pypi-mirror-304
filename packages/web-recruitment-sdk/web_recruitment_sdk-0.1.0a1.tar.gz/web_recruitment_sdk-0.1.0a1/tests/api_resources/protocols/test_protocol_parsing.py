...
class TestProtocolParsing:
    ...
    def test_method_retrieve(self, client: WebRecruitmentSDK) -> None:
        protocol_parsing = client.protocols.protocol_parsing.retrieve(
            0,
        )
    ...
    def test_raw_response_retrieve(self, client: WebRecruitmentSDK) -> None:
        response = client.protocols.protocol_parsing.with_raw_response.retrieve(
            0,
        )
    ...
    def test_streaming_response_retrieve(self, client: WebRecruitmentSDK) -> None:
        with client.protocols.protocol_parsing.with_streaming_response.retrieve(
            0,
        ) as response:
...
class TestAsyncProtocolParsing:
    ...
    async def test_method_retrieve(self, async_client: AsyncWebRecruitmentSDK) -> None:
        protocol_parsing = await async_client.protocols.protocol_parsing.retrieve(
            0,
        )
        assert_matches_type(ProtocolParsingRead, protocol_parsing, path=["response"])
    ...
    async def test_raw_response_retrieve(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.protocols.protocol_parsing.with_raw_response.retrieve(
            0,
        )
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.protocols.protocol_parsing.with_streaming_response.retrieve(
            0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
