...
class SpotterBuilder:
    ...
    def _create_collection(cls, tmp_dir: Path, project_root: Path) -> None:
        ...
        try:
            ExecutableRunner.execute_command(cmd)
    ...
    def _modify_ee(cls, ee: Dict[Any, Any], tmp_dir: Path, project_root: Path) -> CommentedMap:
        ...
        if "append_final" in ee["additional_build_steps"]:
            _ = [ee["additional_build_steps"]["append_final"].append(x) for x in append_list_content]
        else:
    ...
    def build(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ee_modified_path = self._get_ee_modified_file_path(tmp_path)
            ...
            self._create_tmp_ee(ee_modified_path, ee_modified)
            cmd = self._get_cmd(ee_modified_path, self.args)
