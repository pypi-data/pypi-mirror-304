...
def run_mailjet_maintenance() -> None:
    ...
    """
    neon_creds = boto3.client("ssm").get_parameters(
        Names=[
            "/neon/api_key",
            "/neon/api_user",
        ],
    ...
    neon_creds = NeonCredentials(
        api_key=neon_creds["Parameters"][0]["Value"],
        api_user=neon_creds["Parameters"][1]["Value"],
    )
    ...
    orientation_accts = getNeonAccounts(
        searchFields=orientation_search_fields,
        credentials=neon_creds,
        neonAccountDict=orientation_accts,
    )
    waiver_accts = getNeonAccounts(
        searchFields=waiver_search_fields,
        credentials=neon_creds,
        neonAccountDict=waiver_accts,
    )
    member_accts = getNeonAccounts(
        searchFields=member_search_fields,
        credentials=neon_creds,
        neonAccountDict=member_accts,
    )
