...
class WebsocketHandler(BaseHandler):
    events = {event.resource: event.event for event in api_events if ApiType.WS not in event.exclude}
    commands = {event.event: event.resource for event in api_events if ApiType.WS not in event.exclude}
