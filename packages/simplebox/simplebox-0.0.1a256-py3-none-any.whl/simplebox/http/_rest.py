...
class BaseRest(metaclass=ABCMeta):
    ...
    def register(func, decorator):
        ...
        :param func: Functional functions
        :param decorator: Functional function decorators
        :return:
        example:
        class Api:
            rest = Rest()
        ...
        api1 = Api(Rest(server_name="server1"))
        api2 = Api(Rest(server_name="server2"))
        #  api1 and api2 will be used server2
        ...
        class Api:
            ...
            def __init__(self, rest):
                self.rest = rest
                self.rest.register(self.func, self.rest.get(api_name="api name"))

            # !!!!!!!!!!!!!!WARNING!!!!!!!!!!!!!!!!
            # When you manually register a decorator, you don't want to use it on a functional function.
            def func(self, response):
                return response
...
class RestWrapper(BaseRest):
    ...
    def __request(self, func: callable, kwargs: dict, api_name: str = None, server_name: str = None, host: str = None,
        ...
        # noinspection PyBroadException
        try:
            req_args = _utils_wrapper.run_before_hooks(self.__hooks, hooks or Hooks(),
            ...
            kwargs[_Constant.RESPONSE] = _utils_wrapper.run_after_hooks(self.__hooks,
                                                                        hooks or Hooks(),
                                                                        optional_args.get("hooks") or Hooks(),
                                                                        rest_resp)
    ...
    def register(func, decorator):
        if not ismethod(func):
            raise TypeError(f'Expected type is method, but got func is "{type(func).__name__}"')
        if not isfunction(decorator):
            raise TypeError(f'Expected type is function, but got decorator is "{type(decorator).__name__}"')
        setattr(func.__self__, func.__name__, decorator(func))
