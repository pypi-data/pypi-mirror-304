...
class FirstParser:
    ...
    types = [
        ...
        ('oper', FstSyntaxMatcher('OPER', '')),

        ('assign', FstSyntaxMatcher('IDENT', 'ASSIGN')),
        ('oper_assign', FstSyntaxMatcher('IDENT', 'OPER', 'ASSIGN')),
        ...
        ('one_expr', FstSyntaxMatcher('STR')),
        ('one_expr', FstSyntaxMatcher('INT')),
        ('one_expr', FstSyntaxMatcher('IDENT')),
...
class SecParser:
    ...
    """
    exprs = [
        ('oper_e', SecSyntaxMatcher(('one_expr', 1), ('oper', 1))),
        ('oper_e', SecSyntaxMatcher(('one_expr', 1))),
        ('getattr', SecSyntaxMatcher(('one_expr', 1), ('get_attr', -1))),
        ('assign', SecSyntaxMatcher(('assign', 1), ('expr_e', -1))),
    ]

    def _merge_expr(self, pos):
        for expr_name, matcher in self.exprs:
            res, consumption = matcher.match(self.phrases[pos:])
            if res is not None and len(res) > 0:
                res = list(res)
                return expr_name, res, consumption
        return None, None, 0
    ...
    def _parse(self):
        ...
        while self.pos < len(self.phrases):
            expr_name, res, length = self._merge_expr(self.pos)
            if res is None:
                self.results.append((self.phrases[self.pos]))
                self.pos += 1
                continue
            self.results.append(
                Phrase(expr_name, res)
            )
            self.pos += length
            changed = True
        return changed
