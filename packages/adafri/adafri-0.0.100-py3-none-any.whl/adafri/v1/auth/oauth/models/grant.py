...
def read_file():
    url = os.getenv('ASSETS_ENDPOINT')
    pem_file_name = os.getenv('OAUTH_PRIVATE_KEY')
    issuer = os.getenv('JWT_ISSUER')
    print('isser for read file', issuer)
    print('pem file', pem_file_name)
    if pem_file_name is None:
        pem_file_name = 'privateKey.pem'
    if url is not None:
        url = url + "/" + pem_file_name
        # Send a GET request to download the file content
        response = requests.get(url)
        if response.status_code == 200:
            return RSAKey.import_key(response.content).as_dict(private=True)

    fp = os.getcwd() + "/" + pem_file_name
...
class OpenIDCode(_OpenIDCode):
    ...
    def get_jwt_config(self,  grant):
        print('openid code jwt config', DUMMY_JWT_CONFIG)
...
class OpenIDImplicitGrant(_OpenIDImplicitGrant):
    ...
    def get_jwt_config(self):
        print('openid implicit jwt config', DUMMY_JWT_CONFIG)
        return DUMMY_JWT_CONFIG
...
class OpenIDHybridGrant(_OpenIDHybridGrant):
    ...
    def get_jwt_config(self):
        print('openid hybrid jwt config', DUMMY_JWT_CONFIG)
        return DUMMY_JWT_CONFIG
