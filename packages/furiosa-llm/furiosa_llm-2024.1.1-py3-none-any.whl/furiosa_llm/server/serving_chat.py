    ...
    async def create_chat_completion(
        ...
        try:
            ...
            prompt_str: str = apply_chat_template(
                messages,
                # TODO: override chat template if request.chat_template is not None
                self.chat_template,
                self.llm.tokenizer,
            )
        ...
        try:
            # XXX: temporary code for 2024.01
            prompt_len = len(self.llm.tokenizer.encode(prompt_str, add_special_tokens=False))
            # TODO - make LLM.max_seq_len_to_capture an attribute and use it
            sampling_params = request.to_sampling_params(2048 - prompt_len)
