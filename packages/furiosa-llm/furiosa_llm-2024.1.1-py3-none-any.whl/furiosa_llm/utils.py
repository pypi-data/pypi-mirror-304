...
if typing.TYPE_CHECKING:
    from furiosa_llm.api import Bucket, PagedAttentionConfig, KvCacheSharingAcrossBeamsConfig
...
def _generate_input_sample_for_gptj_causal_lm(
    ...
    sample: Dict[str, Any] = {
        ...
        "position_ids": torch.arange(
            ...
            bucket.attention_size + bucket.input_len(is_prefill),
    ...
    if paged_attention_config is not None:
        ...
        if is_packed_optimized:
            ...
            sample["new_key_location"] = _range_and_reshape(
                bucket.batch_size, bucket.input_len(is_prefill), dtype=torch.int32
            )
            sample["new_value_location"] = _range_and_reshape(
                bucket.batch_size, bucket.input_len(is_prefill), dtype=torch.int32
            )
            if not is_prefill or not is_quantized:
                sample["bucket_size"] = bucket.attention_size

            if not is_quantized:
                sample["is_prefill"] = is_prefill

            if kv_cache_sharing_across_beams_config is not None and not (
                is_prefill and is_quantized
            ):
            ...
            if is_prefill:
                ...
                if kv_cache_sharing_across_beams_config is None:
                    sample["past_valid_key_indices"] = torch.arange(
                        bucket.batch_size * bucket.input_len(is_prefill),
                    ...
                    sample["past_valid_key_decode_indices"] = tensor_generator(
                        bucket.batch_size
                        * ((bucket.attention_size - bucket.input_len(is_prefill)) - max_prompt_len),
                        dtype=torch.int32,
                    )
                ...
                torch.IntTensor(
                    [
                        tuple(
                            range(
                                b * attention_length,
                                b * attention_length + bucket.input_len(is_prefill),
                            )
                        )
...
def generate_input_sample(
    ...
    if arch in ("GPTJForCausalLM", "LlamaForCausalLM", "OPTForCausalLM", "GPT2LMHeadModel"):
...
def enforce_torch_load_with_mmap(user_func):
    @wraps(user_func)
    def decorator(*args, **kwargs):
        # Workaround to avoid loading full model parameters.
        # This workaround has already been applied to latest transformers, but needed for 4.31.0.
        original_torch_load = torch.load
        torch.load = partial(torch.load, mmap=True)
        try:
