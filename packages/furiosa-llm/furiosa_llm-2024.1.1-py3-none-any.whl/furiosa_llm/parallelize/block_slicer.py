...
GPTJ_FIRST_LAYERNORM_WEIGHT_PATTERN = r"transformer\.h\.\d+\.ln_1(\._QLV4_layernorm)?\.weight"
...
BERT_EMBEDDING_OR_ATTENTION_OUTPUT_LAYERNORM_BIAS_PATTERN = r"(bert\.encoder\.layer\.\d+\.output\.LayerNorm(\._QLV4_layernorm)?\.bias)|(bert\.embeddings\.LayerNorm(\._QLV4_layernorm)?\.bias)"
ROBERTA_EMBEDDING_OR_ATTENTION_OUTPUT_LAYERNORM_BIAS_PATTERN = r"(roberta\.embeddings\.LayerNorm\.bias)|(roberta\.encoder\.layer\.\d+\.output\.LayerNorm\.bias)"
# This is the 1st rms norm weight name of Llama blocks.
LLAMA_FIRST_RMS_NORM_WEIGHT_PATTERN = (
    r"model\.layers\.\d+\.input_layernorm(\._QLV4_layernorm)?\.weight"
)
    ...
    for n in gm.graph.nodes:
        if node_to_bitmap[n.name] == 0:
            ...
            if len(parent_colors) != 1:
                raise ValueError(f"Node {n.name} is dead node but its parents has different colors")
            node_to_bitmap[n.name] = next(iter(parent_colors))

    # check if all nodes are colored
    for n in gm.graph.nodes:
        assert node_to_bitmap[n.name] != 0, f"Node {n.name} is not colored"
