...
from furiosa_llm.parallelize.config import Device, NodeId, TensorId

# Replicate = mpc.Replicate
# Shard = mpc.Shard
# Partial = mpc.Partial
# ReduceOp = mpc.ReduceOp
...
def to_exportable_reduce_op_type(reduce_op: RedOpType) -> mpc.ReduceOp:
    if reduce_op == c10d.ReduceOp.SUM:
        return mpc.ReduceOp.SUM
    elif reduce_op == c10d.ReduceOp.AVG:
        return mpc.ReduceOp.AVG
    elif reduce_op == c10d.ReduceOp.MAX:
        return mpc.ReduceOp.MAX
    elif reduce_op == c10d.ReduceOp.MIN:
...
def from_exportable_placement_type(self: mpc.Placement) -> Placement:
    if isinstance(self, mpc.Shard):
        ...
        return Partial(from_exportable_reduce_op_type(self.reduce_op))
...
class ShardSpec(dt.placement_types.DTensorSpec):
    """
    ...
    def __eq__(self, other: Union['ShardSpec', dt.placement_types.DTensorSpec]) -> bool:  # type: ignore [override]
        if not isinstance(other, (ShardSpec, dt.placement_types.DTensorSpec)):
            return False
