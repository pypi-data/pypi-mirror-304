...
from furiosa_llm.parallelize.pipeline.types import DeviceId, ParamfileFormat, ParamFileInfo
...
def write_without_concurrency_issue(
    s: Union[str, Dict[str, torch.Tensor]], path: Union[str, os.PathLike]
) -> None:
    path = Path(path)
    ...
    while True:
        try:
            ...
            if isinstance(s, str):
                ...
                save_tensors(s, path.parent / tmp_filename)
            break
