...
def add_qparam_info(original_model: torch.nn.Module, transformed_gm: GraphModule):
    ...
    # To suppress https://github.com/furiosa-ai/model-compressor-private/blob/main/model_compressor/quant_op/tensor_quantizer/quantizer_module.py#L185
    from model_compressor.quant_op.tensor_quantizer.quantizer_module import logger  # type: ignore
    ...
    for node in transformed_gm.graph.nodes:
        if node.op != "get_attr":
            continue
        actual_tensor = getattr(transformed_gm, node.target)
...
def add_param_buffer_original_name_info(
    ...
    for node in transformed_gm.graph.nodes:
        if original_info := name_mappings.get(node.target):
            original_name, is_parameter = original_info
            set_original_name(node, original_name)
            if is_parameter:
                set_as_weight_param(node)
