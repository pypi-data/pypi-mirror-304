...
class SignalAnnotatorApp(TemporaryEventMixin):
    ...
    def on_click_figure(self, event):
        ...
        if event.button == 1:  # 左键点击
            ...
            if self.state.active_event is None:
                # 创建新标注
                self.state.active_event = Event(name='', onset=at_x, duration=None)
                self.update_annotation_fields()
    ...
    def build_tk_ui(self):
        ...
        ttk.Button(info_frame, text='保存', command=self.save_annotations).pack(side=tk.RIGHT)
...
def launch_app(signals: EEGFileInterface, fn_on_exit: Callable[[], None] | None = None):
    """基于 Tkinter 实现的信号可视化应用界面

    UI 描述:
    ----------
    - 顶层:
        - 波形的基本信息描述, 包括通道标签, 采样率, 时间长度
        - 空白弹性区域, 用于分割左右
        - 按钮: 保存
        - 按钮: 退出
    - 中间: Matplotlib 波形图对象
        - 可交互, 监听鼠标点击事件, 并注册回调函数 `on_click_figure`
        - 处于节省资源考虑，分页加载波形图
        - 具体内容受到「数据加载」按钮组和「active_event」状态影响
    - 底部: 按钮区域
        - 数据加载按钮组
            - 按钮: 向左加载
            - 数字 input: 当前开始时间, 单位: 秒
            - 按钮: 向右加载
            - 数字 input: 页面长度, 单位: 秒
            - 数字 input: interval, 默认无输入
        - 空白弹性区域, 用于分割左右
        - active_event 状态描述 UI 组
            - 文本 Input: 标注开始时间, 单位: 秒
            - 文本 Input: 标注持续时间, 单位: 秒
            - 文本 Input: 标注标签, 默认无输入
        - 小空白区域
        - 标注按钮组
            - 按钮: 添加标注
