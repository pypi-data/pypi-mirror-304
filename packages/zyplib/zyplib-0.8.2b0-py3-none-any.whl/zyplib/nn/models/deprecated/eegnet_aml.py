...
class EEGNet(base.BaseModule):
    ...
    def __init__(
        ...
        self.output_sizes = {}
        # block1
        time_padding = int((len_kernel1 // 2))
        self.conv1 = nn.Conv2d(
            in_channels=1,
            out_channels=f1,
            kernel_size=(1, len_kernel1),
        ...
        self.output_sizes['depthwise1'] = convtransp_output_shape(
            self.output_sizes['conv1'], kernel_size=(in_ch, 1), stride=1, pad=0
        )
        ...
        self.output_sizes['avg_pool_block1'] = (
            1,
            int(self.output_sizes['depthwise1'][1] / 4),
        )
        self.dropout_block1 = nn.Dropout(p=dropout[0])

        # block2
        self.separable_block2 = deepwise_separable_conv(
            nin=f1 * d, nout=f2, kernelSize=len_kernel2
        )
        ...
        self.output_sizes['avg_pool_block2'] = (
            1,
            int(self.output_sizes['separable_block2'][1] / 4),
        )

        self.dropout_block2 = nn.Dropout(dropout[1])

        self.flatten = nn.Flatten()
        n_size = self.get_features_dim(in_ch, samples)
        self.dense = nn.Linear(n_size, n_class)
