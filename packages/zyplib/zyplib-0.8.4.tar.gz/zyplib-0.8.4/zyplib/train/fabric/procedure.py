...
from simple_inject import Inject, auto_inject, inject
...
class FabricTrainingContext:
    ...
    def launch(self):
        self.fabric.launch()
        if isinstance(self.train_metric_recorder, TorchMetricRecorder):
            self.train_metric_recorder.to_device(self.fabric.device)
        if isinstance(self.val_metric_recorder, TorchMetricRecorder):
            self.val_metric_recorder.to_device(self.fabric.device)
...
def train_epoch(
    ...
    **train_step_kwargs: Any,
    ...
    losses = []
    for idx, batch in enumerate(rich_progress(train_loader, f'Training epoch {epoch_idx}', color='green')):
...
def val_epoch(
    ...
    **val_step_kwargs: Any,
    ...
    losses = []
    for idx, batch in enumerate(rich_progress(val_loader, f'Validating epoch {epoch_idx}', color='blue')):
        metrics = fn_val_step(
...
def test(
    ...
    fn_loss: nn.Module,
    epoch_idx: Optional[int] = None,
    fn_test_step: Optional[Callable] | None = None,
    **val_step_kwargs: Any,
) -> Dict[str, float]:
    """单个验证轮次
    ...
    `fn_val_step`:
    ----------
    - 依赖注入: 从 `namespace=train` 自动获取 `validation_step` 或者 `test_step`
    - 默认不提供, 则使用 `procedure.validation_step`; 如果提供了, 则使用提供的函数
    - 接受以下的关键字参数: `fn(ctx: FabricTrainingContext, batch, fn_loss, batch_idx)`
    - 返回值: `Dict[str, Any]`, 至少包含 `loss`
    ...
    fn_test_step = (
        fn_test_step
        or inject('test_step', 'train')
        or inject('validation_step', 'train')
        or validation_step
    )

    if ctx.fabric:
        ctx.fabric.call('on_test_start', dataloader=test_loader)
        test_loader = ctx.fabric.setup_dataloaders(test_loader)

    losses = []
    for idx, batch in enumerate(rich_progress(test_loader, 'Testing', color='blue')):
        metrics = fn_test_step(
            ctx=ctx, batch=batch, fn_loss=fn_loss, batch_idx=idx, **val_step_kwargs
        )
        if ctx.fabric:
            ctx.fabric.call(
    ...
    if ctx.fabric:
        ctx.fabric.call('on_test_end', metrics=metrics)
    return metrics
...
def train_loop(
    ...
    Fabric 需要在创建之前 launch!

    依赖注入
    ----------
    可以将 `training_step` 和 `validation_step` 函数注入到 `"train"` namespace 下作。
    如果不注入，使用默认的训练和验证函数。函数需满足如下签名:
    ```py
    ...
    Parameters
    ----------
    - `context` : `procedure.FabricTrainingContext`; 必须包含 fabric!
    - `fabric` : `Fabric`
        - _description_
    - `train_loader` : `DataLoader`
        - _description_
    ...
    train_loader = context.fabric.setup_dataloaders(train_loader)
    if val_loader:
        val_loader = context.fabric.setup_dataloaders(val_loader)
