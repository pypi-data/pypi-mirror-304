...
from zyplib.utils.kv import flatten_dict, prefixed_dict
...
class SaveHparams(FabricCallbackProtocol):
    ...
    def on_train_start(self, **kwargs):
        ...
        if hparams:
            ...
            print_warn(f'{self} 无法获取 hparams!')
...
class PrintTrainInfo(FabricCallbackProtocol):
    ...
    def on_train_start(self, **kwargs):
        self.tictoc.tic()
        print_info(f'开始训练: {now()}'.center(120, '='))
        print()
...
class FabricLogMetrics(EnsureFabricMixin, FabricCallbackProtocol):
    ...
    def scalar_dict(metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        scalar_metrics = {}
        for key, value in metrics.items():
            scalar_value = ensure_scalar(value)
            if scalar_value is not None:
                scalar_metrics[key] = scalar_value
            elif isinstance(value, dict):
                # 如果 value 是 dict，那么就展平它
                flattend_dict = flatten_dict(value)
                scalar_metrics.update(prefixed_dict(f'{key}.', flattend_dict))
            else:
                # 这里的 value 就不一定是 scalar 了, 可能是 tensor, 只能祈祷 fabric 能正确处理...
