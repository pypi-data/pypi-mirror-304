...
def default_fabric(
    ...
    **fabric_kwargs,
    ...
    if not no_csv_logger:
        ...
        _loggers.append(csv)
        parser = ParseCSVLogfile(
            os.path.join(csv.log_dir, 'metrics.csv'), to_excel=True, to_yaml=False
        )
        _callbacks.append(parser)
    if not no_tensorboard_logger:
        _loggers.append(
            loggers.TensorBoardLogger(root_dir=root_dir, name=dir_name, version=version)
        )
