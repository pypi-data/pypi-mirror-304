...
class PIPKinematics(FingerKinematics):
    ...
    def __init__(self, x, y, r, scale = 1.0) -> None:
        ...
        self.tendon_length_at_zero = self.get_tendon_length_from_angle(
            np.array([0.0]))
    ...
    def _get_tendon_length_from_tangent_angle(
            self, angles: np.ndarray) -> np.ndarray:
        """
    ...
    def _get_tendon_length_from_large_angle(
            self, angles: np.ndarray) -> np.ndarray:
        """
    ...
    def get_tendon_length_from_angle(self, angles: np.ndarray) -> np.ndarray:
        ...
        tendon_lengths[angles < self.phi_dash] = \
            self._get_tendon_length_from_tangent_angle(
                angles[angles < self.phi_dash])
        # angles larger or equal to phi_dash
        tendon_lengths[angles >= self.phi_dash] = \
            self._get_tendon_length_from_large_angle(
                angles[angles >= self.phi_dash]
            )
