...
class GraspGui(tk.Frame):
    ...
    def _setup_action_buttons(self, row_idx: int = 1) -> None:
        ...
        self._action_button_span = (
            self._n_action_buttons * self._action_button_col_step
        )
    ...
    def _change_control_mode(self) -> None:
        ...
        control_mode_display_name = \
            self._control_mode_names[self._ctrl_mode.get()]
        control_mode_method_name = \
            self._get_control_method_name_from_display_name(
                control_mode_display_name)
        ...
        setup_fcn = getattr(
            self,
            f"_setup_{control_mode_method_name}ler_gui",
            None
        )
        if setup_fcn is not None:
            setup_fcn()

    def _get_control_method_name_from_display_name(
    ...
    def _setup_motor_controller_gui(self, row_idx=3) -> None:
        ...
        for motor_idx in range(self.gc.num_of_motors):
            ...
            self.motor_labels[-1].grid(
                row=controller_row_idx+motor_idx, column=0)
            # make slider
            ...
            self.motor_sliders.append(
                ttk.Scale(
                    ...
                    id=slid_id: self._changed_motor_slider_value_callback(
                        id, value
                    ),
                    length=300,
                )
    ...
    def _setup_joint_controller_gui(self, row_idx=3) -> None:
        ...
        for joint_idx in range(self.gc.num_of_joints):
            ...
            self.joint_labels.append(
                ttk.Label(self.gui_root, text=joint_name))
            self.joint_labels[-1].grid(
                row=controller_row_idx+joint_idx, column=0)
            # make slider
            ...
            self.joint_sliders.append(
                ttk.Scale(
                    ...
                    to=range_max,
                    command=lambda value,
                    id=slid_id: self._changed_joint_slider_value_callback(
                        id, value
                    ),
                    length=300,
                )
    ...
    def _clear_motor_controller_gui(self) -> None:
        ...
        motor_control_widgets = [
            "motor_sliders", "motor_status_displays", "motor_pos_displays",
            "motor_cur_displays", "motor_labels"
        ]
    ...
    def _changed_motor_slider_value_callback(
        self, id: int, value: float
    ) -> None:
        """
    ...
    def _changed_joint_slider_value_callback(
        self, id: int, value: float
    ) -> None:
        """
        ...
        self._update_motor_pos_displays(
            motor_pos_array=motor_status[0],
            pos_displays=self.joint_pos_displays)
        self._update_motor_cur_displays(
            motor_cur_array=motor_status[2],
            cur_displays=self.joint_cur_displays)

    def _update_motor_cmd_displays(
            self,
            status_displays=None,
            status_array=None) -> None:
        """
    ...
    def _update_motor_pos_displays(
            self,
            motor_pos_array=None,
            pos_displays=None) -> None:
        """
    ...
    def _update_motor_cur_displays(
            self,
            motor_cur_array=None,
            cur_displays=None) -> None:
        """
