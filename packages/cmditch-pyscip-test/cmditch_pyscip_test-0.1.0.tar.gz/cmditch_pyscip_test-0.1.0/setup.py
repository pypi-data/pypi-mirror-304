...
if not scipoptdir:
    ...
    if os.path.exists(os.path.join(scipoptdir, "include")):
        ...
        print(
            f"SCIPOPTDIR={scipoptdir} does not contain an include directory; searching for include files in src or ../src directory."
        )
        ...
        if os.path.exists(os.path.join(scipoptdir, "src")):
            ...
            if os.path.exists(os.path.join(scipoptdir, "src", "scip")):
                ...
                if os.path.exists(os.path.join(scipoptdir, "..", "src", "scip")):
                    ...
                    sys.exit(
                        f"Could neither find src/scip nor ../src/scip directory in SCIPOPTDIR={scipoptdir}. Consider installing SCIP in a separate directory."
                    )
        else:
            sys.exit(
                f"Could not find a src directory in SCIPOPTDIR={scipoptdir}; maybe it points to a wrong directory."
            )
...
extensions = [
    Extension(
        ...
        define_macros=(
            [("CYTHON_TRACE_NOGIL", 1), ("CYTHON_TRACE", 1)]
            if compile_with_line_tracing
            else []
        ),
    )
]
...
setup(
    ...
    long_description="Still testing",
    long_description_content_type="text/markdown",
    url="https://github.com/SCIP-Interfaces/PySCIPOpt",
    author="Murray Rothbard",
    author_email="murray@mises.ru",
    license="MIT",
