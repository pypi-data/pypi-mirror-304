...
class RemoteServerService:
    ...
    def connect_to_instance(
        ...
        try:
            if private_key_path:
                os.system(f"ssh -o PreferredAuthentications=publickey -i {private_key_path} {username}@{ip_address}")
            else:
    ...
    def connect_to_container(
        ...
        try:
            if private_key_path:
                os.system(f"ssh -tt -o PreferredAuthentications=publickey -i {private_key_path} {username}@{ip_address} 'docker exec -it {docker_name} sh -c \"cd {starting_directory} && {shell.value}\"'")
            else:
