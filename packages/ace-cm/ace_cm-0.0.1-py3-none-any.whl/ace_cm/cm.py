...
_component_func = components.declare_component("CM.sync_cookies", path=str(build_path))


class CM(MutableMapping[str, str]):
    def __init__(self, *, path: str = None, prefix=""):
        self._queue = st.session_state.setdefault('CM.queue', {})
        self._prefix = prefix
        raw_cookie = self._run_component(save_only=False, key="CM.sync_cookies")
    ...
    def save(self):
        if self._queue:
            self._run_component(save_only=True, key="CM.sync_cookies.save")
