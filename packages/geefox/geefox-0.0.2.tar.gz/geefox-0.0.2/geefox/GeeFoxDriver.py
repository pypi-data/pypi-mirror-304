...
class GeeFoxDriver:
    ...
    def scan(self, urls: list[str], use_multithreading=False, max_threads=10, output=True):
        '''
        使用实例针对指定的Url列表发起扫描，并可选择是否使用多线程处理，以及是否输出结果为表格

        :param urls: url 列表，例如 ["http://test.org"] 或 ["url1", "url2", "url3"]
        :param use_multithreading: 是否启用多线程处理，默认为 False
            ...
            for rule in self.process_examples.rules:
                if rule['pattern'] == 'code':
                    ...
                    if rule['logic'] == 'all':
                        if rule['range'] == 'all':
                            p = all(
                                item in response_body or item in str(response_headers) or item in str(response_status)
