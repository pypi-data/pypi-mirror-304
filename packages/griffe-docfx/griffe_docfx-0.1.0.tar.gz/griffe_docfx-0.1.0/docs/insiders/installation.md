...
*griffe-docfx Insiders* is a compatible drop-in replacement for *griffe-docfx*,
...
*griffe-docfx Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/griffe-docfx.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/griffe-docfx.git
```
...
Of course, you can use *griffe-docfx Insiders* directly using Git:

```
git clone git@github.com:pawamoy-insiders/griffe-docfx
```

When cloning with Git, the package must be installed:

```
pip install -e griffe-docfx
```

## Upgrading

When upgrading Insiders, you should always check the version of *griffe-docfx*
