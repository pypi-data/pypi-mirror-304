...
def upload_and_remove_file(root_dir, remote_name, remote_dir, suffix="zip"):
    if isinstance(root_dir, str):
        root_dir = Path(root_dir)
    root_dir = root_dir.expanduser().absolute()
    command = f'rsync -av -I --include="*/" --include="*.{suffix}" --exclude="*" "{root_dir}/" "{remote_name}":"{remote_dir}"'
    logger.info(f"executing \"{command}\"")
    result = delegator.run(command)
    if result.err != '':
        raise RuntimeError(f"rsync command failed. {result.err}")

    for file_path in root_dir.glob(f"**/*.{suffix}"):
        target_path = file_path.relative_to(root_dir)
        if str(target_path) in result.out:
            logger.info(f"{target_path} Uploaded. Delete {suffix} file.")
            file_path.unlink()
        else:
