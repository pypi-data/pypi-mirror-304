...
def check_api(ctx: Context, *cli_args: str) -> None:
    ...
    ctx.run(
        tools.griffe.check("mkdocstrings_handlers.csharp", search=["src"], color=True).add_args(*cli_args),
...
def docs_deploy(ctx: Context, *, force: bool = False) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/mkdocstrings-csharp" in origin:
            ctx.run(
                "git remote add upstream git@github.com:mkdocstrings/csharp",
                silent=True,
                nofail=True,
...
def release(ctx: Context, version: str = "") -> None:
    ...
    if "pawamoy-insiders/csharp" in origin:
        ctx.run(
            lambda: False,
