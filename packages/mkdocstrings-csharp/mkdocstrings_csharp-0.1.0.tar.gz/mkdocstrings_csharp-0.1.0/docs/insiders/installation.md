...
*mkdocstrings-csharp Insiders* is a compatible drop-in replacement for *mkdocstrings-csharp*,
...
*mkdocstrings-csharp Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/mkdocstrings-csharp.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/mkdocstrings-csharp.git
```
...
Of course, you can use *mkdocstrings-csharp Insiders* directly using Git:

```
git clone git@github.com:pawamoy-insiders/mkdocstrings-csharp
```

When cloning with Git, the package must be installed:

```
pip install -e mkdocstrings-csharp
```

## Upgrading

When upgrading Insiders, you should always check the version of *mkdocstrings-csharp*
