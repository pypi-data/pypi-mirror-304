...
import torch
import signal
signal.signal(signal.SIGINT, lambda self,code: os._exit(0))
...
class DrugSiteData:
    def __init__(self, folder, dataset_path, site_path, cache_file, protein_cavity_same_feature=False, resolution_scale=0.5, max_distance=35, max_translation=3, footprint=None, file_format='mol2', use_cache=True, debug_size=None):
        self.folder = folder
        ...
        if use_cache and os.path.exists(cache_file):
            ...
            names.sort()
            with tqdm(total=len(names), unit="ligand", bar_format='{l_bar}{bar}|{n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_inv_fmt}]') as pbar:
              for i, name in enumerate(names):
                if debug_size and i==debug_size: break
                pbar.set_description(name)
                protein = dataset_path / name / (name + "_protein.pdbqt")
                protein_coordinate =[]
                ...
                os.system(f'{folder / "protein_feature_tool" / "protein_feature_tool"} \
                         -i {dataset_path / name / (name + "_protein.pdbqt")} \
                         -o {site_path / (name + "_feature_generate.txt")} \
                         -data {folder/ "protein_feature_tool" / "17_FEATURES_DATA.txt"}')

                feature = site_path / (name + '_feature_generate.txt')
                protein_features_part = np.loadtxt(feature)

                os.system(f'{folder / "surface_tool" / "surface_tool"} \
                         -i {dataset_path / name / (name + "_protein.pdbqt")} \
                         -o {site_path / (name + "_surface.txt")}')
                surface = site_path / (name + '_surface.txt')
...
class DrugMain:
    def site(folder, args, do_train, model_cache_best, site_path, device=torch.device("cuda")):
        def valid(dataset_name = args.dataset_name, debug_size=None, use_cache_data=1, show_loss_epoch=1):  
            print('Site Searching ...')
            data_valid = DrugSiteData(folder, dataset_path =args.dataset_path, 
                site_path = site_path, cache_file = site_path / (dataset_name +'.npz'),
                use_cache=use_cache_data, debug_size=debug_size)
            ...
            if os.path.exists(model_cache_best):
                ...
                module.DrugSiteMind = DrugSiteMind
                network = torch.load(model_cache_best, weights_only=False).to(device)  
            else:
                raise FileNotFoundError('can not find model file', model_cache_best)

            network.eval()

            print('Docking >>>')
            save_file = 1
            with tqdm(total=len(load_valid), unit="ligand", 
                 bar_format='{l_bar}{bar}|{n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_inv_fmt}]') as pbar:
              for i, batch in enumerate(load_valid):  
                ...
                os.system(f'{folder / "DSDP_blind_docking/DSDP_blind"} \
                	--ligand {args.dataset_path / names[0] / (names[0] + "_ligand.pdbqt")} \
                	--protein {args.dataset_path / names[0] / (names[0] + "_protein.pdbqt")} \
                	--site_npy {args.site_path / (names[0] + ".npy")} \
                	--out {args.out / (names[0] + "_out.pdbqt")} \
                	--log {args.log / (names[0] + "_out.log")} --top_n {args.top_n} >> {args.out.parent / "results.log"}')
                pbar.update(1)
              pbar.set_description("")
            print('Finished !!!')
        train() if do_train else valid()

def blind_docking(args, folder, *argv, **kwargs):
    args.o = Path(args.o)
    args.dataset_path = Path(args.dataset_path)
    if args.o == args.dataset_path:
        raise FileExistsError("The output path and the input path should be different")
    if not args.site_path:
        args.site_path = args.o / "site"
    if not args.site_path.exists():
        args.site_path.mkdir(parents=True)
    args.out = args.o / "out"
    if not args.out.exists():
        args.out.mkdir(parents=True)
    args.log = args.o / "log"
    if not args.log.exists():
        args.log.mkdir(parents=True)
    args.dataset_name = Path(args.dataset_path).stem
    time_start = time.time()
    DrugMain.site(folder, args, False, folder / "binding_site_model.pth", args.site_path)
    time_end = time.time()
    print(f'Time: {time_end - time_start} s')

def redocking(args, folder, *argv, **kwargs):
    os.system(f"{folder/'DSDP_redocking'/'DSDP_redock'} --ligand {args.ligand} --protein {args.protein} \
    --box_min {args.box_min[0]} {args.box_min[1]} {args.box_min[2]} \
    --box_max {args.box_max[0]} {args.box_max[1]} {args.box_max[2]} \
    --exhaustiveness {args.exhaustiveness} --search_depth {args.search_depth} --top_n {args.top_n}  \
    --out {args.out} --log {args.log}")

def testing(args, folder, parser):
    print("=" * 30)
    print("-" * 12 + "Input:" + "-" * 12)
    print(f'copy {Path(folder) / "test_data"} {Path(".") / "test_data"}')
    print("-" * 11 + "Results:" + "-" * 11)
    shutil.copytree(Path(folder) / "test_data", Path(".") / "test_data")
    print("=" * 30)
    command = "redock --ligand ./test_data/1a2b/1a2b_ligand.pdbqt --protein ./test_data/1a2b/1a2b_protein.pdbqt --box_min 2.241 20.008 21.314 --box_max 24.744 35.470 38.495 --exhaustiveness 384 --search_depth 40 --top_n 1  --out ./1a2b_redock.pdbqt --log ./1a2b_redock.log"
    print("-" * 12 + "Input:" + "-" * 12)
    print("DSDP " + command)
    print("-" * 11 + "Results:" + "-" * 11)
    args = parser.parse_args(command.split())
    redocking(args, folder, parser)
    print("=" * 30)
    command = "blind -i ./test_data -o ./blind_results --exhaustiveness 384 --search_depth 40 --top_n 1"
    print("-" * 12 + "Input:" + "-" * 12)
    print("DSDP " + command)
    print("-" * 11 + "Results:" + "-" * 11)
    args = parser.parse_args(command.split())
    blind_docking(args, folder, parser)

def main():
    folder = Path(__file__).parent
    parser = ArgumentParser(description="DSDP: A Blind Docking Strategy Accelerated by GPUs")
    subparsers = parser.add_subparsers()
    parserA = subparsers.add_parser("blind", help="dock without known binding site")
    
    parserA.add_argument('-i', '--dataset_path', dest="dataset_path", type=str, required=True, help='Path to the dataset file, please put the pdbqt documents of protein and ligand to one folder')
    parserA.add_argument('-o', '--output_path', dest="o", type=str, required=True, help='Output path of the results')
    parserA.add_argument('--site_path', type=str, help="Path to the predicted sites")
    parserA.add_argument('--exhaustiveness', type=int, default=384, help='Number of sampling threads')
    parserA.add_argument('--search_depth', type=int, default=40, help='Number of sampling steps')
    parserA.add_argument('--top_n', type=int, default=1, help='Top N results are exported')
    parserA.set_defaults(func=blind_docking)

    parserB = subparsers.add_parser("redock", help="dock with known binding site")
    parserB.add_argument("--ligand", type=str, required=True, help="File name of ligand")
    parserB.add_argument("--protein", type=str, required=True, help="File name of protein")
    parserB.add_argument("--box_min", required=True, metavar=("x_min", "y_min", "z_min"), type=float, nargs=3, help="x y z minima of box")
    parserB.add_argument("--box_max", required=True, metavar=("x_max", "y_max", "z_max"), type=float, nargs=3, help="x y z maxima of box")
    parserB.add_argument("--exhaustiveness", type=int, default=384, help="Number of sampling threads, default 384")
    parserB.add_argument("--search_depth", type=int, default=40, help="Number of sampling steps, default 40")
    parserB.add_argument("--top_n", type=int, default=10, help="Top N results are exported, default 10")
    parserB.add_argument("--out", type=str, default="OUT.pdbqt", help="Output file name of redocking, default 'OUT.pdbqt'")
    parserB.add_argument("--log", type=str, default="OUT.log", help="Log file name of redocking, default 'OUT.log'")
    parserB.set_defaults(func=redocking)

    parserC = subparsers.add_parser("test", help="do a simple blind docking and redocking for unittest and example")
    parserC.set_defaults(func=testing)

    args = parser.parse_args()
    args.func(args, folder, parser)
