...
def upgrade(version: str):
    ...
    if version:
        call(f"pip install --upgrade lookout-config=={version}")
        call(f"pip install --upgrade lookout-cli=={version}")
    else:
        call("pip install --upgrade lookout-config")
...
@click.option("--gpu", type=bool, help="Should we use the GPU?")
def configure(default: bool, gpu: bool):  # type: ignore
    """Configure Lookout"""
