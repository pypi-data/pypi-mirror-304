...
class HealthLogUi:
	...
	def export_log_field(self,fields=[]):
	...
	def showAllField(self,fields=[]):
		try:
			fields.extend(["DTOE","HLID"])
	...
	def __init__(self):
		...
		self.cmds={
			...
			'exec':lambda self=self:self.showAllField(fields=['BloodSugar','BloodSugarUnitName'])
			},
			'ls lai':{
			'cmds':['ls lai','lslai','list long insulin','list long acting insulin'],
			'desc':'list long acting insulin intake',
			'exec':lambda self=self:self.showAllField(fields=['LongActingInsulinName','LongActingInsulinTaken','LongActingInsulinUnitName'])
			},
			'ls sai':{
			'cmds':['ls sai','lssai','list short insulin','list short acting insulin'],
			'desc':'list long acting insulin intake',
			'exec':lambda self=self:self.showAllField(fields=['ShortActingInsulinName','ShortActingInsulinTaken','ShortActingInsulinUnitName'])
			},
			'ls heart rate':{
			'cmds':['lshr','ls heart rate','ls hrt rt'],
			'desc':'list heart rate',
			'exec':lambda self=self:self.showAllField(fields=['HeartRate','HeartRateUnitName'])
			},
			'ls weight':{
			'cmds':['lswt','ls weight','ls wt',],
			'desc':'list weight',
			'exec':lambda self=self:self.showAllField(fields=['Weight','WeightUnitName'])
			},
			'ls height':{
			'cmds':['lsht','ls height','ls ht',],
			'desc':'list height',
			'exec':lambda self=self:self.showAllField(fields=['Height','HeightUnitName'])
			},
			'ls consumed':{
			...
			'exec':lambda self=self:self.export_log_field(fields=['BloodSugar','BloodSugarUnitName'])
			},
			'xpt lai':{
			'cmds':['xpt lai','xptlai','export long insulin','export long acting insulin'],
			'desc':'export long acting insulin intake',
			'exec':lambda self=self:self.export_log_field(fields=['LongActingInsulinName','LongActingInsulinTaken','LongActingInsulinUnitName'])
			},
			'xpt sai':{
			'cmds':['xpt sai','xptsai','export short insulin','export short acting insulin'],
			'desc':'export long acting insulin intake',
			'exec':lambda self=self:self.export_log_field(fields=['ShortActingInsulinName','ShortActingInsulinTaken','ShortActingInsulinUnitName'])
			},
			'xpt heart rate':{
			'cmds':['xpthr','xpt heart rate','xpt hrt rt'],
			'desc':'export heart rate',
			'exec':lambda self=self:self.export_log_field(fields=['HeartRate','HeartRateUnitName'])
			},
			'xpt weight':{
			'cmds':['xptwt','xpt weight','xpt wt',],
			'desc':'export weight',
			'exec':lambda self=self:self.export_log_field(fields=['Weight','WeightUnitName'])
			},
			'xpt height':{
			'cmds':['xptht','xpt height','xpt ht',],
			'desc':'export height',
			'exec':lambda self=self:self.export_log_field(fields=['Height','HeightUnitName'])
			},
			'xpt consumed':{
