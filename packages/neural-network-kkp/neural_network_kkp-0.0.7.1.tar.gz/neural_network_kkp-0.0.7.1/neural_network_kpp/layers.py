...
class DenseLayer:
    ...
    def backward(self, grad_output):
        ...
        if grad_output.shape != grad_activation.shape:
            grad_activation = grad_activation.reshape(grad_output.shape)

        grad_input = np.dot(grad_output * grad_activation, self.weights.T)
        self.grad_weights = np.dot(self.inputs.T, grad_output * grad_activation)
        self.grad_biases = np.sum(grad_output * grad_activation, axis=0, keepdims=True)
        return grad_input
