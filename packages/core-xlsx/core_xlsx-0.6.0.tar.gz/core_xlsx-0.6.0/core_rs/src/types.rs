...
pub struct XLSXSheet {
    ...
    pub _indexes: HashMap<(u32, u32), usize>,
}
...
impl XLSXSheet {
    pub fn new(name: String, index: i32, cols: u32, rows: u32) -> Self {
        ...
        for r in 1..=rows {
            for c in 1..=cols {
                ...
                // Insert Indexes
                indexes.insert((r, c), cells.len() - 1);
            }
        }
    ...
    pub fn write_cell(&mut self, row: u32, col: u32, value: String) -> Result<&mut XLSXSheetCell> {
        if row < 1 || col < 1 {
            bail!("row and col must be greater than 0");
        }

        if self._indexes.contains_key(&(row, col)) {
            // Ячейка существует, редактируем ее
            let cell_index = self._indexes[&(row, col)];
            ...
            self._indexes.insert((row, col), index);
            ...
            if mr > 1 || mc > 1 {
                ...
                for r in 1..=row {
                    for c in 1..=col {
                        if let Vacant(e) = self._indexes.entry((r, c)) {
                            let cell = XLSXSheetCell::new(r, c, None);
                            new_cells.push(cell);
                            e.insert(self.cells.len() + new_cells.len() - 1);
                        }
                    }
                }
                ...
                for (i, cell) in self.cells.iter().enumerate() {
                    self._indexes.insert((cell.row, cell.column), i);
                }
            }
    ...
    pub fn write_cell_with_formula(
        ...
        value: String,
        formula: String,
    ) -> Result<&mut XLSXSheetCell> {
        let cell = self.write_cell(row, col, value)?;
    ...
    pub fn write_style_for_cell(&mut self, row: u32, col: u32, style_id: String) -> Result<()> {
        ...
        if let Some(&index) = self._indexes.get(&(row, col)) {
            self.cells[index].set_style_id(style_id)?;
            Ok(())
        } else {
            bail!("Cell with row: {} & col: {} not found!", row, col)
        }
    }
