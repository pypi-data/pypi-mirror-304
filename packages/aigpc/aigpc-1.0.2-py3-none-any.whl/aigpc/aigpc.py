...
print("Risk Control V4.0 v20241025")
print("---------------------------------------------------------------------------------------------")
...
def TEST_calculate_score(df):  
	...
	for ids,cols in df.iterrows():
       ...
       #金叉 用ma30
		#elif  (cols["ma30"]==cols["ma20"]) and (cols["ma30"]>df.loc[ids-1,"ma30"]) and cols["ma10"]>cols["ma20"] and cols["ma20"]>cols["ma30"]:
		#	df.loc[ids,"score"]=100
		#	df.loc[ids,"record"]="100%:MA-GOOD；GOLD"
        ...
        #死叉 用ma30   
		#elif  cols["ma30"]==cols["ma30"] and (cols["ma30"]<df.loc[ids-1,"ma30"]) and cols["ma10"]<cols["ma20"] and cols["ma20"]<cols["ma30"]:          
		#	df.loc[ids,"score"]=0
		#	df.loc[ids,"record"]="0%:MA-BAD；DEAD"
		...
		#金叉 macd	
		#elif (cols["macd"]==macfac and cols["macd"]>df.loc[ids-1,"macd"]) and  cols["diff"]<macfac:			
		#	df.loc[ids,"score"]=50
		#	df.loc[ids,"record"]="50%: MACD IS GOOD(GOLD) BUT DIF IS NOT GOOD；"
...
def calculate_score(df):  
	'''计算打分'''     
	print('目前用的规则：MA MACD-DIF 100%-50%-0%')     
	df=df.reset_index()
	df["score"]=0
	df["record"]=""
	macfac=0
