...
class ChatGPTAutomation:
    ...
    def __init__(self):
        ...
        if os_type == "Windows":
            for program_files_path in [os.environ.get('ProgramFiles'), os.environ.get('ProgramFiles(x86)')]:
                program_files_path = program_files_path.replace("\\", "/")
                self.chrome_path = f'{program_files_path}/Google/Chrome/Application/chrome.exe'  # Windows路径
                if os.path.exists(self.chrome_path):
                    break
            else:
                raise('Cannot find a Chrome installed in "Program Files" or "Program Files (x86)"')
    ...
    def launch_chrome_with_remote_debugging(self, port, url):
        ...
        def open_chrome():
            chrome_cmd = f'"{self.chrome_path}" --remote-debugging-port={port} --user-data-dir=remote-profile --app={url} --no-first-run '
