...
def download_chrome_testing():
    ...
    if not os.path.isdir(chrome_dir):
        ...
        url = [version['url'] for version in response.json()['versions'][0]['downloads']['chrome'] if version['platform'] == tag][0]
        ...
        print('将自动启动一个Chrome浏览器，点击不登录后会进入ChatGPT主页，请在这里登录你的ChatGPT账号，然后关闭浏览器')
        try:
            from .chatgpt_on_selenium import ChatGPTAutomation
        except:
            from AutoTrans.chatgpt_on_selenium import ChatGPTAutomation
        chat_bot = ChatGPTAutomation(chrome_path=chrome_dir+'/chrome.exe')

def run_uvicorn():
    install_and_import(['fastapi', 'deepl', 'selenium', 'nltk', 'pyperclip']) # 检查包依赖
    ...
    # 构建命令并运行
    command = ['start', '/B', 'uvicorn', '__init__:app', '--port', '3006', '--reload']
