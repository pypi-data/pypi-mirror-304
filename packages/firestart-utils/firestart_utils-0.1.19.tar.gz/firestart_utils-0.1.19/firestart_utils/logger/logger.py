...
class LakeHouseLogger:
    ...
    def __init__(self, environment: str, location: str, spark: SparkSession):
        self.environment = environment
        self.location = location
        self.spark = spark 
        self.logger_constructor =  StructType([StructField(const, logger_construct[const], True if const == "parent_notebook_run_uuid" else False) for const in logger_construct])
    ...
    def __log_to_lakehouse__(self, level: str, status: str, message: str, parent_notebook_run_uuid: str | None):
        ...
        df_logging_row = self.spark.createDataFrame([parameters], self.logger_constructor)
