...
class Util:
    ...
    def get_secret_from_custom_keyvault(self, keyvault_name, secret_name) -> str:
    ...
    def get_secret_default_keyvault(self, secret_name):
        foundSecret = notebookutils.credentials.getSecret(self.keyvault_name, secret_name)

        if(foundSecret == None):
            raise Exception(f"Secret with name {secret_name} not found in the keyvault {self.keyvault_name}")
