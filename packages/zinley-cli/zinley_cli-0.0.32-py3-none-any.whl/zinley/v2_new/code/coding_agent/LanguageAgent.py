...
class LanguageAgent:
    ...
    async def get_language_plan(self, session, user_prompt):
        """
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        f"You are a senior {self.role} and prompt engineering specialist. "
    ...
    async def get_language_plans(self, user_prompt):
        """
        ...
        async with aiohttp.ClientSession() as session:
            plan = await self.get_language_plan(session, user_prompt)
            return plan
