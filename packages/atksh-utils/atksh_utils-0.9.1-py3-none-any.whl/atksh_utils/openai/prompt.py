...
- However, if you need to see the raw output of the functions that your `answer_subtask` function calls, you can call them directly.
- Otherwise, you should not run any code or visit any web pages directly. Use `answer_subtask` to do so.
- If you are already in a subtask, you can call `answer_subtask` to answer the subtask if needed.
...
- Summarize the following search results with respect to the given query_text and select the top five results to visit.
...
<more>
5. <The 5th summary of the last page> (url: `<last page URL>`, updated at <yyyyy-mm-dd> if available, otherwise omitted)
```
