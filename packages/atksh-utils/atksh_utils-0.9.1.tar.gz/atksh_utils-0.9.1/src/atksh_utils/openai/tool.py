...
def get_run_subtask_function(ai: "OpenAI"):
    ...
    def run_subtask(
        ...
        """Runs the subtask with the given context description, subtask description, output format, and subtask arguments for each subtask.
...
def pip_install(command: str) -> str:
    ...
    with Shell_LOCK:
        try:
            ret = subprocess.check_output(f"{PYTHON_PATH} -m pip install -q {command}", shell=True)
        except subprocess.CalledProcessError as e:
            return_code = e.returncode
            output = e.output
            ret = f"Error: return_code={return_code}, output={output}"
