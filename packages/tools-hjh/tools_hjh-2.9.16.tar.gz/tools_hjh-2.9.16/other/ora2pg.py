        ...
        try:
            ...
            for idx in metadata['indexes']:
                ...
                try:
                    ...
                    dst_conn.commit()
                    log.info('PG执行SQL成功', create_index_sql)
                except Exception as _:
                    if 'does not exist' in str(_):
                        log.warning('PG执行SQL失败', create_index_sql, str(_))
                        ...
                        try:
                            ...
                            if 'does not exist' in str(_):
                                ...
                                log.error('PG执行SQL失败', create_index_sql, str(_))
                    else:
                        log.error('PG执行SQL失败', create_index_sql, str(_))
        except Exception as _:
            log.error('PG建立索引失败', dst_schema + '.' + dst_table, str(_))
        finally:
            dst_cur.close()
            dst_conn.close()
            src_db.close()
...
def copy_table():
    ...
    tp = ThreadPool(32, while_wait_time=0.1)
    for table_map in table_map_list:
...
def compare_data_number():
    ...
    def count(db, sql, k):
        ...
        log.info('对数报告', '是否一致：' + str(tables_data_num[k][1] == tables_data_num[k][2]), k, 'src=' + str(tables_data_num[k][1]), 'dst=' + str(tables_data_num[k][2]), 'scn=' + str(tables_data_num[k][0]), str(e1))
    ...
    def count2(table_map, src_db, dst_db, global_scn):
        ...
        log.info('对数报告', '是否一致：' + str(tables_data_num[report_table_id][1] == tables_data_num[report_table_id][2]), report_table_id, 'src=' + str(tables_data_num[report_table_id][1]), 'dst=' + str(tables_data_num[report_table_id][2]), 'scn=' + global_scn, str(e1), str(e2))
...
def compare_index():
    ...
    def compare_index_one(table_map):
        ...
        try:
            ...
            dst_num = -2
            e2 = str(_)
        log.info('比较索引数量', '是否一致：' + str(src_num == dst_num), src_schema + '.' + src_table, 'src=' + str(src_num), 'dst=' + str(dst_num), str(e1), str(e2))
        dst_db.close()
        src_db.close()
...
def compare_constraint():
    ...
    def compare_constraint_one(table_map):
        ...
        try:
            ...
            dst_num = -2
            e2 = str(_)
        log.info('比较主外键数量', '是否一致：' + str(src_num == dst_num), src_schema + '.' + src_table, 'src=' + str(src_num), 'dst=' + str(dst_num), str(e1), str(e2))
        src_db.close()
        dst_db.close()
    ...
    try:
        ...
        log.error('比较主外键数量时，Oracle连接失败', str(_))
