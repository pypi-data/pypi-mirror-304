...
def _formatValue (nFormat, bIsProp, nColumn, sKey, sValue):
    ...
    if nFormat == _FORMAT_TEXT:
        return sKey + " = " + _quoteValue (sValue)
...
def _printResultSet (wCurrent, nFormat, sTitle, cur):
    ...
    for iRow, row in enumerate (cur.fetchall ()):
        for iColumn, oValue in enumerate (row):
            sKey = "{} Row {} Column {} {} {}".format (sTitle, iRow + 1, iColumn + 1, _quoteValue (cur.description [iColumn][0]), cur.columntypename [iColumn])
            _printValue (wCurrent, nFormat, False, iColumn + 1, len (row), sKey, oValue)
                    ...
                    if sCommand == "":
                        ...
                        if bVerboseConnection:
                            ...
                            with con.cursor () as cur:
                                cur.execute ("{fn teradata_nativesql}{fn teradata_connected}|{fn teradata_session_number}|{fn teradata_provide(remote_address)}:{fn teradata_provide(remote_port)}|Teradata Database {fn teradata_database_version}") # teradata_nativesql never produces a fake result set
                                sResult = cur.fetchone () [0]
                        ...
                        with con.cursor () as cur:
                            try:
                                ...
                                if setIgnoreSQLErrors is not None and len (setIgnoreSQLErrors) == 0:
                                    bIgnore = True
                                else:
