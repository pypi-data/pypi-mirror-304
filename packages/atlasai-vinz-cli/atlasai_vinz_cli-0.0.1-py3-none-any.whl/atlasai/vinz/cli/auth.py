...
def include_authorization(url, headers, access_key=None, secret_key=None):
    ...
    headers.update({
        ...
        'X-Vinz-Date': arrow.utcnow().isoformat(),
        'X-Vinz-Credential': '/'.join([product, version, access_key]),
        'X-Vinz-SignedHeaders': 'x-vinz-date;x-vinz-credential;host',
    })
...
def sign_request(headers, secret_key):
    product, version, access_key = headers['X-Vinz-Credential'].split('/')
    ...
    msg = '\n'.join([
        headers['X-Vinz-Date'],
        headers['X-Vinz-Credential'],
        headers['Host']
    ])
    headers['X-Vinz-Signature'] = hmac.new(key, msg.encode('utf-8'), 'sha256').hexdigest()
