...
class connect_wrapper(object):
    ...
    def general_execute(self,operation,seq=None):
        conn = self.conn_pool.connect() if self.__is_pool_status else self.conn
        cursor = conn.cursor()
        try:
            for _ in range(3):
                try:
...
class Dimension(table_basic):
    def __init__(self,conn,table_name,columns,lookup_columns,pks,cache=False,auth_increment = None,batch_size=1,**kwargs):
        '''维度表
        ...
        lookup_columns:list
            作为查询条件的字段名称列表 
        cache:bool
            缓存 默认False 将会缓存整个维度表至本地内存中 后续的操作增删改查会先处理缓存
        auth_increment:dict
        ...
        self.next_id = 0
        if cache:
            src = source_sql(self.conn,'select * from ({})'.format(self.table_name))
...
def Cache_table(data,datetime_cols = [],mode =None,inference = True,**kwargs):
