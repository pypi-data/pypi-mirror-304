...
from datajunction_server.internal.caching.hash import get_hash_key
...
async def get_sql_for_metrics(  # pylint: disable=too-many-locals
    ...
    *,
    ...
    if cached_result := cache.get(key=hash_key):
    ...
    sql = await build_and_save_sql_for_metrics(
        session,
        metrics,
        dimensions,
        filters,
    ...
    cache.set(key=hash_key, value=sql, timeout=10)
    return sql
