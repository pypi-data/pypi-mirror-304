...
def create_background(image, new_width, new_height, background_vibrancy, background_brightness, background_blur_radius):
    """
    Generate a modified background from the given image.
    Applies adaptive Gaussian blur based on image size.
    """
    ...
    # Cap the blur radius based on image size
    max_radius = min(new_width, new_height) // 2
    adjusted_blur_radius = min(background_blur_radius, max_radius)

    if adjusted_blur_radius < background_blur_radius:
        logging.warning(
            f"Blur radius {background_blur_radius} is too large for image size {new_width}x{new_height}. "
            f"Capping to {adjusted_blur_radius}."
        )

    # Ensure kernel size is odd and at least 3
    kernel_size = max(3, adjusted_blur_radius * 2 + 1)

    # Apply Gaussian blur using OpenCV
    blurred_image = cv2.GaussianBlur(frame_resized, (kernel_size, kernel_size), 0)
    ...
    desaturated_image = color_enhancer.enhance(background_vibrancy)

    # Adjust brightness
    brightness_enhancer = ImageEnhance.Brightness(desaturated_image)
    dulled_image = brightness_enhancer.enhance(background_brightness)
