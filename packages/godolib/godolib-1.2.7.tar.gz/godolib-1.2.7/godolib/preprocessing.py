...
class VolatilityCalculator (BaseEstimator, TransformerMixin):
    ...
    """
        ...
        if verbose not in valid_verboses:
            raise ValueError(f"'{verbose}' is not a valid mode. Choose from {valid_verboses}.")
        self.method = method
...
class ROCCalculator(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, fit_mode='all_positive', transform_mode='returns', exception=True, period=1, stationarity_threshold=0.05, update_name=True, verbose=0):
        valid_verboses=[0, 1]
    ...
    def transform(self, X, y=None):
        ...
        for feature, condition in self.conditions.items():
            if self.verbose==1:
                print (f'transforming feature: {feature} out of {X.shape[1]}')
            if condition:
...
class WindowTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_past, n_future, verbose=0):
        """
    ...
    def transform(self, X_input, y=None):
        ...
        for i in range(self.n_past, len(np.array(X_input)) + 1 - self.n_future):
            if verbose==1:
                print (f'{i - self.n_past} out of {len(np.array(X_input)) + 1 - self.n_future - self.n_past}')
            X.append(np.array(X_input)[i - self.n_past : i, 0 : np.array(X_input).shape[1]])
        if self.verbose==1:
            print ('preparing Ys')
        for layer in range(np.array(X_input).shape[1]):
            if self.verbose == 1:
                print (f'{layer} out of {np.array(X_input).shape[1]}')
            Y = []
            for i in range(self.n_past, len(np.array(X_input)) + 1 - self.n_future):
