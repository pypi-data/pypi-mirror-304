...
def find_path(vehicle_index, w1, w2, w3):
    # 初始节点和目标节点
    distance_matrix = pd.read_csv("数据集/distance.csv", header=None).values
    roads_matrix = pd.read_csv("数据集/roads.csv", header=None).values
    speed_matrix = pd.read_csv("数据集/speed.csv", header=None).values
    EVs_mapped = pd.read_csv("数据集/EVs_mapped.csv", header=None).values
    EVs_50 = pd.read_csv("数据集/EVs.csv", header=None, usecols=[4, 5, 6, 7]).values
    ...
    while current_node != destination_node:
        ...
        for next_node in candidates:
            ...
            energy_charged = 100 * travel_time if roads_matrix[current_node][next_node] == 1 else 0
