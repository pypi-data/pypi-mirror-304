...
class BoundingBox:
    ...
    def get_extend_bounds(self, margin: int, width_threshold: int = None,
                          height_threshold: int = None) -> 'BoundingBox':
        """
        extend bounding(add margin), return new bounding box
        :param margin: extend margin value
        :param width_threshold: extended bounding box width threshold. if not None, it will limit the bounding box to width_threshold.
        :param height_threshold: extended bounding box height threshold. if not None, it will limit the bounding box to height_threshold.
        :return: the extended bounding box
        """
        if width_threshold:
            width = max(0, min(int(self.width + margin), width_threshold - self.x))
        else:
            width = max(0, int(self.width + margin))
        if height_threshold:
            height = max(0, min(int(self.height + margin), height_threshold - self.y))
        else:
            height = max(0, int(self.height + margin))
        x = max(0, min(int(self.x - margin), width_threshold))
        y = max(0, min(int(self.y - margin), height_threshold))
