...
class Particle(Tube):
    ...
    def pop_edge(self, buffer, context):
        ...
        self._wip_completion(buffer, context)
    ...
    def _slice_order(self, buffer):
        _slice = []
        for uid, stream in buffer.items():
            if stream:
                wave__ = stream[0].get(MONOTONIC_KEY)
                if wave__:
                    _slice.append((wave__, uid))
            else:
    ...
    def _wip_completion(self, buffer, context):
        """
        ...
        """
        for wave__, uid in self._slice_order(buffer):
            stream = buffer[uid]
            data = stream.pop(0)
...
class XCondenser(Particle):
    """A particle that condense information before generate an edge

    Some operations may be executed in cascad using subclassing
    """
    ...
    def _wip_completion(self, buffer, context):
        """
        Subclassing Particle need to implement a mechanism that:

        - feed self._wip_edge
        - unset self._wip_uncompleted

        This class method just prepare some variables in context[]
        so you need to override, call this method and implement
        you own policy
        """
        ...
        for wave__, uid in self._slice_order(buffer):
            ...
            info["structure_keys__"] = structure_keys
            info["structure_values__"] = structure_values
            info["value_values__"] = data_values
            info["org"] = data
            ...
            #     self._wip_edge[MONOTONIC_KEY] = TO_NS(ts)
            #     break  # generate a edge
            self._wip_edge[MONOTONIC_KEY] = max(
                wave__, self._wip_edge.get(MONOTONIC_KEY, wave__)
            )
            break
        # we've classify all data to determine the logix of
        # the particle, so let's try to update the wip edge
        # with this new data, whatever this means for the subclass
        self._update_wip_edge(buffer, context)
        debug = False or random.random() < 0.05
        if debug or self._is_wip_edge_completed(buffer, context):
            self._push_wip_edge()
    ...
    def _update_wip_edge(self, buffer, context):
        ...
        for uid, info in delta.items():
            holder_key = info["structure_values__"]
            new_values = info["value_values__"]

            uid_holder = self._wip_edge[uid]
    ...
    def _push_wip_edge(self):
        # TODO: agp: uncomment
        # assert not self._wip_uncompleted

        # for uid, uid_holder in list(self._wip_edge.items()):
        #     for holder_key, last_holder in uid_holder.items():
        #         data = last_holder["org"]
        #         last = last_holder["last"]
        #         data.update(last)
        #         self._wip_edge[uid] = data

        # TODO: agp: remove debug
        self._wip_uncompleted.clear()
        foo = 1
...
class iXSMAParticle(XCondenser):
    def _calc(self, old_values, new_values):
...
class iCollector:
    ...
    async def _compute(self, edge, ekeys):
        ...
        for uid, uid_holder in edge.items():
            ...
            for holder_key, last_holder in uid_holder.items():
                org = last_holder["org"]
                # compose data into aggregated structure
                # TODO: use regexp?
                push = False
                for pair in self.TRANSLATE:
                    if all([_ and _ in org for _ in pair]):
                        aggregated[org.pop(pair[0])] = org.pop(pair[1])
                        push = True
...
class XSMAParticle(iCollector, iTimedParticle, iXSMAParticle):
