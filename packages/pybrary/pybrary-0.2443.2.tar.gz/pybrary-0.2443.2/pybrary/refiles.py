...
def _grep1(lines, matcher):
    excls = [p.isin for p in matcher.cont.excl]
    incls = [p.isin for p in matcher.cont.incl]
    for i, line in enumerate(lines, start=1):
        if all(incl(line) for incl in incls) and not any(excl(line) for excl in excls):
            yield i, line


def _grep2(lines, before, after, matcher):
...
def grep(fil, cont=None, before=0, after=0, matcher=None, **opts):
    ...
    lines = [l.rstrip() for l in open(fil).readlines()]
    if before or after:
        return _grep2(lines, before, after+1, matcher)
    else:
