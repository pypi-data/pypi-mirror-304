...
class FeatureColumnResource(ColumnResource):
    def __init__(
        ...
        **Example**
        ...
        """
        # Features cannot use the default target lag of DOWNSTREAM because we need to ensure
        #  that if users serve features, they should be up to date within the target lag period.
        if not snowflake_dynamic_table_config:
            snowflake_dynamic_table_config = SnowflakeDynamicTableConfig(
...
class Registrar:
    ...
    def register_column_resources(
        ...
        for feature in features:
            ...
            resource = FeatureVariant(
                ...
                snowflake_dynamic_table_config=feature.get(
    ...
    def register_training_set(
        ...
        **Example**:
        ...
        # Training sets cannot use the default target lag of DOWNSTREAM because there are
        #  no resources downstream of training sets that would set their target lag.
        if not snowflake_dynamic_table_config:
            snowflake_dynamic_table_config = SnowflakeDynamicTableConfig(
                target_lag=ONE_DAY_TARGET_LAG,
                refresh_mode=RefreshMode.AUTO,
