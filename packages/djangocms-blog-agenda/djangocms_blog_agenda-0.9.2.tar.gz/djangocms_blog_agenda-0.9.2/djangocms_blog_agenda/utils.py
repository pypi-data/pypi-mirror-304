...
def add_recurrent_posts(qs, reverse=False, after=None):
    ...
    for object in qs:
        if object.extension.exists():
            ...
            # Wether to keep the initial event in the posts_with_recurrences list
            if (
                not post_event.event_end_date  # no end date
                or not after  # or no `after` parameter
                or post_event.event_end_date  # or a end date greater than `after` parameter
