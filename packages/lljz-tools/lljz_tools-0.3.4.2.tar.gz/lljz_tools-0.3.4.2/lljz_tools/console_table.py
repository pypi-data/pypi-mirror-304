...
class ConsoleTable:
    ...
    def to_image(
        ...
        totalWidth = max(sum(width) + (len(width) + 1) * 10, captionW)
        totalHeight = sum(height) + (len(height) + 1) * 12 + (captionH + 15 if self.caption else 0)
        ...
        for i, row in enumerate([header, *values], start=1):
            curHeight = pHeight + i * 12 + sum(height[:i - 1])
            if i > 1:
                draw.rectangle(
