...
from nxtomo.utils.io import deprecated_warning, deprecated, ignore_deprecation_warning
...
class NXdetector(NXobject):
    ...
    def distance(self, distance: float | None) -> None:
        if not isinstance(distance, (type(None), float)):
            raise TypeError(
                f"distance is expected to be an instance of {float} or None. Not {type(distance)}"
            )
    ...
    def to_nx_dict(
        ...
        if (
            self.x_rotation_axis_pixel_position is not None
            and nexus_detector_paths.X_ROTATION_AXIS_PIXEL_POSITION is not None
        ):
            x_rot_axis_pos_path = (
                f"{self.path}/{nexus_detector_paths.X_ROTATION_AXIS_PIXEL_POSITION}"
            )
    ...
    def _load(
        ...
        if nexus_detector_paths.X_ROTATION_AXIS_PIXEL_POSITION is not None:
            self.x_rotation_axis_pixel_position = get_data(
                file_path=file_path,
                data_path=f"{data_path}/{nexus_detector_paths.X_ROTATION_AXIS_PIXEL_POSITION}",
            )
