...
from pawnlib.typing.converter import append_suffix, append_prefix, hex_to_number, FlatDict, FlatterDict, flatten, shorten_text, StackList, replace_path_with_suffix, format_text, format_link
...
class IconRpcHelper:
    ...
    def get_balance(self, url=None, address=None, is_comma=False, is_tint=True, return_as_hex=False, return_as_decimal=False):
        ...
        if is_valid_token_address(address):
            ...
            if return_as_decimal:
                ...
                # balance_float = Decimal(balance_number).quantize(Decimal("1e-18"), rounding=ROUND_DOWN)
    ...
    def get_bond(self, address=None, return_as_hex=False, return_key="result.bonds"):
        if not address:
            address = self.wallet.get('address')
    ...
    def send_all_icx_with_decimal(self, to_address, fee=None, step_limit=None, min_balance=0, margin_steps=1000):
        ...
        """
        # Get the sender's wallet address and balance
        ...
        # Convert the transaction fee to LOOP (smallest unit)
        ...
        # Convert minimum balance to LOOP (smallest ICX unit)
        min_balance_icx = Decimal(min_balance)
        min_balance_loop = int(min_balance_icx * Decimal('1e18'))

        # Calculate total required amount (transaction fee + minimum balance)
        ...
        # Cancel the transfer if there are insufficient funds
        ...
        # Update the payload with the final transfer amount
        ...
        # Check remaining balance after the transaction
        ...
        # Verify if the remaining balance is

        # 잔여 잔고가 최소 잔액 이하인지 확인
    ...
    #
    #     # Calculate the staking amount, excluding the fee
    #     staking_amount_icx = balance_icx - transaction_fee_icx - min_balance
    #
    #     # Calculate the new total stake (current staked amount + new stake)
    #     new_total_stake_icx = staking_amount_icx + current_staked_icx
    #
    #     # If the new total staking amount is less than or equal to the current staked amount, raise an error
    #     if new_total_stake_icx <= current_staked_icx:
    #         pawn.console.log(f"[INFO] No additional staking required. Current staking amount is sufficient.")
    #         return
    #
    #     # Update the payload with the final staking amount
    #     pawn.console.log(f"current_staked_icx={current_staked_icx}")
    #     pawn.console.log(f"staking_amount_icx={staking_amount_icx}")
    #
    #     payload['params']['data']['params']['value'] = hex(int(new_total_stake_icx * 1e18))  # Convert ICX to LOOP
    #
    #     # Sign and send the staking transaction
    #     try:
    #         self.sign_tx(payload=payload, step_limit=step_limit)
    #         result = self.sign_send()
    #         pawn.console.log(f"[SUCCESS] Staked {new_total_stake_icx} ICX successfully. TX Hash: {result.get('result')}")
    #     except Exception as e:
    #         pawn.console.log(f"[ERROR] Staking transaction failed: {e}")
    #         raise
    #
    ...
    #     updated_staked_loop = int(self.get_stake(return_as_hex=True), 16)
    #     updated_staked_icx = updated_staked_loop / 1e18
    #     pawn.console.log(f"[INFO] Updated Staked Amount: {updated_staked_icx} ICX")
...
class AsyncCallWebsocket:
    def __init__(
        ...
        self.logger = setup_logger(logger, "AsyncCallWebsocket", verbose)
    ...
    async def _handle_communication(self):
        ...
        try:
            ...
            # # Send Slack notification on exception
...
class AsyncGoloopWebsocket(AsyncCallWebsocket):
    ...
    def __init__(
        ...
        self.logger = setup_logger(logger, "AsyncGoloopWebsocket", verbose)
    ...
    async def handle_confirmed_transaction_list(self, response):
        ...
        if tx_hash:
            ...
            if block_data is None or not isinstance(block_data, dict):
                ...
                # await self.log_message(f"Invalid block data received for tx hash {tx_hash} in blockheight {readable_block_height}", "error")
                from pawnlib.utils.notify import send_slack
                send_slack(
                    # title="Invalid block data received",
    ...
    async def default_process_transaction(self, tx, block_height):
        ...
        try:
            ...
            shorten_tx_hash = shorten_text(tx_hash, width=6, placeholder='', truncate_side="left")
            ...
            if method == "setStake":
                ...
                await self.log_message(
                    f"🔵 <Staking> {from_highlighted}{from_prep_label} has staked 💰 {stake_value} ICX.",
                ...
                stake_value = self.get_stake_value(tx_data)
                await self.log_message(
                    f"🔵 <Staking> {from_highlighted}{from_prep_label} has unstaked 💰 {stake_value} ICX.",
                   slack_additional_message=full_tx_hash,
                   level="info"
                )

            elif method == "Send":
                _value = hex_to_number(tx.get('value', 0),  is_tint=True, symbol='ICX', debug=True)
                if data_type == "send":
                    _data_type_text = ""
                else:
    ...
    async def log_message(self, message, slack_additional_message="", level="info"):
        """
    ...
    def format_message(self, message: str, for_console: bool = True):
        ...
        """
        if for_console:
            return message  # Console formatting is already done with [bold red] etc.
        else:
            # Slack formatting - Markdown syntax for bold and color support through attachments
...
async def retry_operation(operation, max_attempts=3, delay=2, success_criteria=None, logger=None, verbose=0, *args, **kwargs):
    ...
    # Reuse the existing logger if provided, otherwise set up a new one
    _logger = setup_logger(logger, "retry_operation", verbose)

    while attempts < max_attempts:
        try:
            # Call the operation with provided args and kwargs
            result = await operation(*args, **kwargs)

            # Check the success criteria if provided
            if success_criteria:
                if success_criteria(result):
                    return result  # Return the result if it meets the success criteria
                else:
                    _logger.error(f"Operation result did not meet success criteria (Attempt {attempts+1}/{max_attempts})")
            else:
                return result  # If no success criteria, return result as success

        except Exception as e:
            _logger.error(f"Error during operation: {e} (Attempt {attempts+1}/{max_attempts})")

        # Increment the attempt counter and retry after delay if needed
        attempts += 1
