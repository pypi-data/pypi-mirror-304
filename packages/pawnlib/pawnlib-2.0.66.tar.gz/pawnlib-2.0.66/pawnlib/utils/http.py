...
from typing import Any, Dict, Iterator, Tuple, Union, Callable, Type, Optional, List, Awaitable
try:
...
class AsyncCallWebsocket:
    ...
    #                 self.logger.warn("WebSocket connection closed by server.")
    ...
    #                 self.logger.warn("WebSocket is closing...")
    #                 await self.close()
    #                 break
    #
    #             else:
    #                 self.logger.warn(f"Unknown message type: {msg.type}")
    #                 await self.reconnect()  # Handle unexpected message types with reconnection.
    #                 break
    #
    #     except aiohttp.ClientConnectionError as e:
    ...
    async def close(self, exit_on_close=True):
        ...
        if exit_on_close:
            from pawnlib.asyncio import shutdown_async_tasks
            await shutdown_async_tasks(exit_on_shutdown=True, logger=self.logger)
    ...
    async def graceful_close(self):
        ...
        try:
            ...
            # 시스템 종료
            self.logger.info("Shutting down the system gracefully.")
            sys.exit(0)
    ...
    #     :param exit_on_close: Whether to exit the script when closing the WebSocket connection.
    #     """
    #     if self._ws:
    #         await self._ws.close()
    #         self.logger.info("WebSocket connection closed.")
    ...
    #         await self.session.close()
        ...
        # 이벤트 루프가 이미 종료되었는지 확인
        # loop = asyncio.get_event_loop()
        # if loop.is_running():
        #     if exit_on_close:
        #         self.logger.info("Exiting the script.")
        #         # 안전하게 스크립트를 종료하기 위해 sys.exit(0)을 호출
        #         import sys
        #         sys.exit(0)
        # else:
        #     self.logger.warning("Event loop is not running, cannot close.")
...
class AsyncGoloopWebsocket(AsyncCallWebsocket):
    ...
    async def fetch_and_store_preps_info(self):
        try:
            platform_name = getattr(self.network_info, "platform", "icon")
            platform_methods = {
                "icon": (self.api_client.get_preps, "address"),
    ...
    async def handle_confirmed_transaction_list(self, response):
        ...
        # tx_hash = "0x328b7744641ecae079e305bc1471585f33898e4feff232a48bf9c8b5edc6c85a"
        ...
        if tx_hash:
            # block_data = await self.api_client.get_block_hash(tx_hash)

            def is_success(result):
                if not result:
                    return False
                return result.get("confirmed_transaction_list", False)
            ...
            if not block_data:
                block_data = {}
            # pawn.console.log(f"block_data={block_data}")
    ...
    def get_prep_info(self, address=None, key="name"):
        """
    ...
    async def default_process_transaction(self, tx, block_height):
        ...
        try:
            ...
            method = tx_data.get("method", "Send") if tx_data else "Send"
            ...
            if method == "setStake":
                ...
                await self.log_message(f"🔵 <Staking> {from_highlighted}{from_prep_label} has staked 💰 {stake_value} ICX.", level="info")

            elif method == "unStake":
                stake_value = self.get_stake_value(tx_data)
                await self.log_message(f"🔵 <Staking> {from_highlighted}{from_prep_label} has unstaked 💰 {stake_value} ICX.", level="info")

            elif method == "Send":
                _value = hex_to_number(tx.get('value', 0),  is_tint=True, symbol='ICX', debug=True)
                await self.log_message(
                    f"💸 <{shorten_tx_hash}> <Send> {from_highlighted}{from_prep_label} 👉 {to_highlighted}{to_prep_label} 💰 {_value} ",
                    level="info"
                )
            else:
            ...
            if self.check_tx_result_enabled:
                ...
                if tx_result == "OK":
                    await self.log_message(f"✅ <{shorten_tx_hash}> TX Result received: {tx_result}", level="info")
                else:
                    await self.log_message(f"❌ <{shorten_tx_hash}> Failed to retrieve TX Result: {tx_result}", level="error")
        except Exception as e:
    ...
    async def log_message(self, message, level="info"):
        ...
        try:
            ...
            if self.send_slack and level != "debug":
                ...
                asyncio.create_task(self.send_slack_notification(message, level=level))
...
# async def retry_operation(operation, max_attempts=3, delay=2, logger=None, verbose=0, *args, **kwargs):
#     """
#     Retry the given operation multiple times in case of failure.
#
...
#     :raises: Exception if the operation fails after max_attempts.
#     """
#     attempts = 0
...
#         except Exception as e:
#             attempts += 1
#             _logger.error(f"Error during operation: {e} (Attempt {attempts}/{max_attempts})")
