...
class InitWithKwargsTest(unittest.TestCase):
    ...
    def test_uppercase_initial_state(self):
        node = Task("foo", initial_state="open")
        self.assertEqual(node.initial_state(), "open")
        node = Task("foo", initial_state="HOLDING")
        self.assertEqual(node.initial_state(), "holding")
