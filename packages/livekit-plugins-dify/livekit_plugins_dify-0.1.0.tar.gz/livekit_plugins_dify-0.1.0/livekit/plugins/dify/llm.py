    ...
    def chat(
        ...
        *,
        ...
        request_data = {
            ...
            "user": self._opts.user or "default_user",
            #no temperature
        }
    ...
    async def __anext__(self):
        ...
        async for chunk in self._dify_stream.aiter_lines():
            ...
            try:
                ...
                # print(f"Parsed message: {message}")  # 打印解析后的消息内容
            ...
            if 'answer' in message:
                print(f"Message contains answer: {message['answer']}")
                chat_chunk = self._parse_message(message)
                if chat_chunk is not None:
                    print(f"Returning chat_chunk: {chat_chunk}")
                    self._skip_interval += 1  
                    return chat_chunk
            else:
