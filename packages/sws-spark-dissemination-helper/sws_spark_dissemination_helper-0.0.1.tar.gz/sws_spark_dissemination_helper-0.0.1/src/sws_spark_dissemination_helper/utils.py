...
def indigenous_mapping(
    ...
    return (
        ...
        .join(
            F.broadcast(
                df_mapping_code_correction.filter(
...
def apply_indigenous_mapping(
    df: DataFrame, df_mapping_code_correction: DataFrame
) -> DataFrame:

    return indigenous_mapping(
        df, df_mapping_code_correction, src_column="old_code", dest_column="new_code"
    )
