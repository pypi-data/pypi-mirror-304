...
from .utils import check_duplicates_in_df, check_mappings, correct_domain_filter
...
class SWSPostgresSparkReader:
    def __init__(
        self,
        spark: SparkSession,
        jdbc_conn_properties: Dict[str, str],
        jdbc_url: str,
    ) -> None:
        self.spark = spark
        self.jdbc_conn_properties = jdbc_conn_properties
    ...
    def read_pg_table(
        ...
        jdbc_conn_properties: Dict[str, str] = self.jdbc_conn_properties
        ...
        if partition_column is not None and num_partitions > 1:
            df_min_max_id = (
                ...
                .option("url", jdbc_url)
                .option("user", jdbc_conn_properties["user"])
                .option("password", jdbc_conn_properties["password"])
