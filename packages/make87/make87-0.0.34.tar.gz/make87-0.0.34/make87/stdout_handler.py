...
def setup_stdout_handler(topic_names):
    """Sets up the stdout handler."""
    try:
        topic = SessionManager.get_instance().get_topic(topic_names.STDOUT)
        stdout_handler = StdOutHandler(topic)
...
def cleanup_stdout_handler():
    ...
    if isinstance(sys.stdout, StdOutHandler):
        sys.stdout.flush()
