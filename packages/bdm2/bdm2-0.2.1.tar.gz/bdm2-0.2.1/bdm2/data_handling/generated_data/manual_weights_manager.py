...
def check_if_all_kg(weights: pd.Series) -> bool:
    ...
    assert os.path.exists(
        default_standard
    ), f"default standard weight does not exist.\nCheck: {default_standard}"
...
def convert_to_kg(
        weights: pd.Series,
        standard_weights_in_kg: Optional[
            pd.Series
        ] = None,  # TODO: DEPRECATE. MORE SIMPLE APPROACH APPLIED
) -> pd.Series:
    """
    ...
    if not weights.empty:
        # max_age = int(weights.index.max())
        # if max_age > max(standard_weights_in_kg.index):
        #     for age in range(int(max(standard_weights_in_kg.index)), max_age + 1):
        #         standard_weights_in_kg.loc[age] = standard_weights_in_kg[max(standard_weights_in_kg.index)]

        rows_in_grams = (
                weights > 10
        )  # (weights / standard_weights_in_kg.loc[weights.index]) > 100
...
def get_manual_weights_filename(client: str, manual_postfix: str) -> str:
    ...
    """
    return os.path.join(
        manual_weights_dir, f"{client}\\Manual_measure_{client}{manual_postfix}.xlsx"
    )
...
def get_all_adjusted_weights(
        ...
        filters: Filter = None,
        units="kg",
        weight_postfix_column_name="weights_postfix",
) -> pd.DataFrame:
    ...
    if not os.path.exists(adjusted_weights_folder):
        logger.info(f"{adjusted_weights_folder} is not exist")
        return pd.DataFrame()

    files = [
        x
        for x in os.listdir(adjusted_weights_folder)
        if x.endswith(".csv") and x.startswith("adjusted_std_weights")
    ]
    ...
    try:
        for file_item in files:
            ...
            adj_weights = load_weights_from_csv(
                os.path.join(adjusted_weights_folder, file_item)
            )
        ...
        if not output_df.empty:
            if units == "kg":
                output_df["weight"] = convert_to_kg(
                    output_df.set_index("daynum")["weight"]
                ).values
            elif units == "g":
                output_df["weight"] = convert_to_g(
                    output_df.set_index("daynum")["weight"]
                ).values
            ...
            device_valuable_columns = house_match_columns + [
                breed_type_column,
                gender_column,
            ]

            output_df = pd.merge(
                output_df, devices[device_valuable_columns], on=house_match_columns
            )
        ...
        logger.info("output_df has duplicates")
        output_df = output_df[
            output_df[not device_match_columns + ["daynum"]].duplicated()
        ]
...
def get_all_manual_weights(
    ...
    if len(local_filters.cycles) == 0:
        local_filters.cycles = pd.ExcelFile(
            manual_weights_filename, engine="openpyxl"
        ).sheet_names
        local_filters.cycles.sort()

    for cycle_item in local_filters.cycles:
        logger.info("Reading {} {}".format(manual_weights_filename, cycle_item))
        try:
            weights = pd.read_excel(
                manual_weights_filename,
                sheet_name=cycle_item,
                engine="openpyxl",
                index_col=1,
            )
            ...
            logger.info("No {} sheet".format(cycle_item))
            continue
        ...
        for house_item in weights.columns:
            ...
            if (house_item not in local_filters.houses) and (
                    len(local_filters.houses) != 0
            ):
                continue
    ...
    if not output_df.empty:
        if units == "kg":
            output_df["weight"] = convert_to_kg(
                output_df.set_index("age")["weight"]
            ).values
        elif units == "g":
            output_df["weight"] = convert_to_g(
                output_df.set_index("age")["weight"]
    ...
    devices = (
        PostgresDevicesStorage()
        .get_devices(filters=local_filters)
        .groupby(house_match_columns, as_index=False)
        .first()
    )
    device_valuable_columns = house_match_columns + [breed_type_column, gender_column]
    output_df = pd.merge(
        output_df, devices[device_valuable_columns], on=["farm", "cycle", "house"]
    )
    output_df = output_df.set_index(device_valuable_columns).reset_index()
...
def save_manual_data(df: pd.DataFrame, save_fname: str, round: int = -1, units="g"):
    ...
    for c, c_group in df.groupby("cycle"):
        ...
        if units == "kg":
            c_group_c["weight"] = convert_to_kg(
                c_group_c.set_index("age")["weight"]
            ).values
        elif units == "g":
            c_group_c["weight"] = convert_to_g(
                c_group_c.set_index("age")["weight"]
            ).values
        if round >= 0:
        ...
        try:
            ...
            logger.info(e)
            logger.info(
                f"Could not create pivot table for {c}. Check device table. "
                f"It could be, that there are house duplicates!"
            )
            continue
        try:
            cols = np.array(
                sorted(c_group_pivot.columns, key=lambda x: (int)(x.split(" ")[-1]))
            )
        except Exception as e:
...
def save_adjusted_weights(
        weights: pd.Series, save_fname: str, round: int = -1, units="kg"
):
    assert units in ["kg", "g"]

    if (
            not os.path.exists(os.path.dirname(save_fname))
            and os.path.dirname(save_fname) != ""
    ):
        os.makedirs(os.path.dirname(save_fname))
    if units == "kg":
...
def match_adjusted_weights(
    ...
    if any([(x not in df.columns.values) for x in match_columns]):
        logger.info("Not all required columns are in df")
        logger.info("Required columns: {}".format(match_columns))
        return df
    ...
    if from_db:
        logger.info(f"Weights src for {client}{manual_postfix}: db")
        try:
            ...
            logger.info(e)
            logger.info(f"Weights src for {client}{manual_postfix}: excel")
            adj_manual_weight = get_all_adjusted_weights(
                client, manual_postfix, filters, units=units
            )
            adj_manual_weight = adj_manual_weight.rename(
                columns={"daynum": match_age_col, "weight": new_col_name}
            )
    else:
        logger.info(f"Weights src for {client}{manual_postfix}: excel")
        adj_manual_weight = get_all_adjusted_weights(
    ...
    if adj_manual_weight.empty:
        logger.info(
            f"NO likely weights were found for {client}, {filters.generate_label()}"
        )
        return df
