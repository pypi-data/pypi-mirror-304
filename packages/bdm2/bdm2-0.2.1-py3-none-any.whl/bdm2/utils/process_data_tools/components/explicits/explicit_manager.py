...
from bdm2.constants.global_setup.env import AWS_ACCESS_KEY_KNEX, AWS_SECRET_KEY_KNEX, AWS_REGION_KNEX
from bdm2.constants.patterns.regexp_patterns import pattern_image_name_output_folder
...
try:
    ...
    warnings.warn("No pyarrow module found! Operations with parquet will raise errors!")
...
def means_by(
    ...
    if save_dtypes:
        ...
        if len(problem_cols):
            logger.info(f"means_by: could not save dtypes for {problem_cols}\n")
    return output_df


def define_sess_ids(df: pd.DataFrame, sess_gap_in_minutes: float = 5) -> pd.Series:
    """
...
def get_only_not_filtered(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def get_s_datetime_from_Image_name(df: pd.DataFrame) -> pd.Series:
...
def filter_by_time(df: pd.DataFrame, filters: Filter) -> pd.DataFrame:
    """
...
def compare_explicits_by_day(
        explicits_map: pd.DataFrame, columns: List[str], plot_type=None
):
...
def get_explicit_files(
    ...
    @param engine_config: configuration class
    ...
    if useRolled:
        activeExplicitFolder = engine_config.ForwardRunViz_rolled_folder
    else:
        activeExplicitFolder = engine_config.ForwardRunViz_folder

    explicit_dir: str = os.path.join(
        engine_config.local_results_dir, device.path, activeExplicitFolder
    )
    # explicit_dir = engine_config.local_results_dir + "\\" + device.path + "\\" + activeExplicitFolder
...
def get_age_and_data_from_payload(df: pd.DataFrame, key: str) -> Tuple[int, pd.DataFrame]:
    try:
        row4age_parse = df.iloc[0]['Image_name']
        match = pattern_image_name_output_folder.search(row4age_parse)
        matched_row = match.group(0)
        tmp = re.match(pattern_image_name_output_folder, matched_row)
        age = int(tmp['age'])
        print(f"Processing chunk\nKey: {key}, Age: {age}")
        return age, df
    except Exception as e:
        print(f"Error parsing age from key {key}: {e}")
        return -1, df


def process_s3_key(s3_resource, key: str) -> Tuple[int, pd.DataFrame]:
    print(f"Processing key: {key}")
    try:
        obj = s3_resource.Object(key)
        response = obj.get()
        data = response['Body'].read()

        # Check if data is empty
        if not data:
            print(f"Error: The file is empty - {key}")
        ...
        if file_extension == "txt":
            df = pd.read_csv(io.StringIO(buffer.getvalue().decode('utf-8')), sep=';')
            return get_age_and_data_from_payload(df, key)

        elif file_extension == "parquet":
            df = pq.read_table(buffer).to_pandas()
            # age, _ = get_age_and_data_from_payload(df, key)
            return get_age_and_data_from_payload(df, key)

        else:
            print(f"Unsupported file extension for key: {key}")
            return -1, pd.DataFrame()
...
def generate_key(device: List[str]) -> str:
    return f"{'/'.join(device)}/ForwardRunViz_explicit/"


def generate_s3_devices_keys(filter: Filter) -> List[str]:
    from bdm2.utils.schemas.models.storages.devices.postgres_devices_storage import PostgresDevicesStorage
    pg_storage = PostgresDevicesStorage()
    devices = pg_storage.get_devices(filters=filter)
    devices_slim = devices[["client_id", "farm_id", "cycle", "house_id", "device"]].drop_duplicates()
    devices_slim_list = devices_slim.values.tolist()

    keys_list = []
    try:
        for device in devices_slim_list:
            keys_list.append(generate_key(device))
    except Exception as e:
        print(f"Error generating keys: {e}")

    print(f"Found {len(keys_list)} device keys")
    return keys_list


def process_files_concurrently(explicit_files, s3_resource, filter=None):
    explicit_files_output = []
    age_map = {}
    explicits_data_map = {}
    with ThreadPoolExecutor(max_workers=4) as executor:
        # Подготовьте задачи для выполнения
        futures = [executor.submit(process_file, file.key, s3_resource, filter) for file in explicit_files]

        # Обрабатывайте завершенные задачи
        for future in as_completed(futures):
            result, age, df = future.result()
            if result is not None:
                explicit_files_output.append(result)
                age_map.setdefault(result, age)
                explicits_data_map.setdefault(result, df)

    return explicit_files_output, age_map, explicits_data_map


def process_file(file, s3_resource, filter=None):
    try:
        # if isinstance(s3_resource, S3Handler):
        # Проверяем, какой ресурс использовать
        # if 'brddevice' in s3_resource.bucket_name:
        age, df = process_s3_key(s3_resource, file)
        # else:
        #     age, df = process_s3_key(s3_resource.root_bucket, file.key)

        # Проверка фильтрации
        if filter is not None:
            # age = get_age_from_explicit_file(file)
            if not filter.check_age(age):
                return None  # Пропускаем файл, если не проходит фильтрацию

        return file, age, df  # Возвращаем ключ файла, если все хорошо
...
def get_explicit_files_froms3(
    ...
    #     activeExplicitFolder = engine_config.ForwardRunViz_folder
    filter = Filter()
    filter.clients = [device['client']]
    filter.breed_types = [device['breed_type']]
    filter.genders = [device['gender']]
    filter.farms = [device['farm']]
    filter.cycles = [device['cycle']]
    filter.houses = [device['house']]
    filter.devices = [device['device']]

    key_s3 = generate_s3_devices_keys(filter)
    from bdm2.utils.s3.connector import S3Handler
    conn_s3 = S3Handler(access_key=AWS_ACCESS_KEY_KNEX, secret_key=AWS_SECRET_KEY_KNEX, region_name=AWS_REGION_KNEX)
    s3_split_root = conn_s3.root_bucket.Bucket("brd-split-device-output")
    s3_brddevice_root = conn_s3.root_bucket.Bucket("brddevice")

    brddevice_b = False

    explicit_files = list(s3_split_root.objects.filter(Prefix=key_s3[0]))

    if not len(explicit_files):
        explicit_files = list(s3_brddevice_root.objects.filter(Prefix=key_s3[0]))
        brddevice_b = True
    explicit_files = explicit_files[0:30]  # TODO: ДРОПНУТЬ!
    explicit_files_output = []
    ages_map = {}
    explicits_map = {}
    ...
    if explicit_files:
        ...
        file_list, age, explicits_data_map = process_files_concurrently(explicit_files,
                                                                        s3_split_root if not brddevice_b else s3_brddevice_root,
                                                                        filters)
        for i in file_list:
            explicit_files_output.append(i)
        for k, v in explicits_data_map.items():
            explicits_map.setdefault(k, v)
        # explicit_files_output.append(file)
        for k, v in age.items():
            ages_map.setdefault(k, v)
    finish_dt = datetime.datetime.now()
    diff = finish_dt - start_dt
    diff = diff.seconds / 60
    print(f'takes {diff}')
    return explicit_files_output, brddevice_b, ages_map, explicits_map
...
def get_explicit_files_completeness(
    ...
    device_match_columns + [ age_column_name, sess_column_name]
    ...
    farm_folders = [
        f for f in os.listdir(engine_config.local_results_dir) if f in unique_farms
    ]
    if filters is None:
        filters = Filter()
    ...
    df_output = pd.DataFrame(
        columns=device_match_columns + [age_column_name, sess_column_name]
    )
    for _, device in devices.iterrows():
        _, files = get_explicit_files(
            device, engine_config, filters, useRolled=useRolled
        )
        if len(files) == 0:
            continue
        for f in files:
            ...
            s2add = device[device_match_columns]
...
def get_features_df_from_explicits(
        device: pd.Series,
        engine_config: EngineConfig,
        filters: Filter,
        features: List[str],
    ...
    df_output = pd.DataFrame()
    explicit_dir, explicit_files = get_explicit_files(
        device, engine_config=engine_config, filters=filters, useRolled=useRolled
    )
    ...
    for file in explicit_files:
        try:
            ...
            if dropFiltered and engine_config.engine_version > "_v4.0":
    ...
    logger.info(
        f"mean load_explicit file_sizes = {np.round(np.mean(file_sizes) / 1000 / 1000, 3)} Mb"
    )
    # sys.exit(1)
    return df_output


def get_all_features_df_from_explicits(
        devices,
        engine_config: EngineConfig,
        filters: Filter,
        useRolled=True,
        add_device_params=False,
        dropFiltered=True,
):
    df_output = pd.DataFrame()
    devices_loc = filters.filter_devices(devices)
    for _, device in devices_loc.iterrows():
        label = generate_id_from_device(device, device_match_columns)
        try:
            if not filters.check_device(device):
                continue

            tmp_df = get_features_df_from_explicits(
                device,
                engine_config,
                filters,
                features=[],
            ...
            if tmp_df.empty:
                logger.info(
                    "No data for {} ages {}".format(
                        device.id, " ".join(map(str, filters.ages))
                    )
                )
                continue
            if add_device_params:
                tmp_df.loc[:, device_match_columns] = device[
                    device_match_columns
                ].values
                for c in standards_match_columns:
...
def remove_doubles(device: pd.Series, engine_config: EngineConfig, filters: Filter):
    explicit_folder, files = get_explicit_files(
        device=device, engine_config=engine_config, filters=filters, useRolled=False
    )
    if len(files) == 0:
        return
    ...
    for age in files_by_age:
        ...
        for f_age in files_by_age[age]:
            ...
            tmp_explicit["Image_basename"] = [
                os.path.basename(x) for x in tmp_explicit["Image_name"]
            ]
            ...
            tmp_explicit = tmp_explicit[
                tmp_explicit["Image_basename"].isin(new_data_Image_basename)
            ]

            # CONCAT
            df_union_explicit = pd.concat([df_union_explicit, tmp_explicit])
        ...
        save_f_name = (
                explicit_folder
                + "\\"
                + files_by_age[age][np.argmin(list(map(len, files_by_age[age])))]
        )
        for f_age in files_by_age[age]:
