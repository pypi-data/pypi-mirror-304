...
from bdm2.constants.global_setup.data import standards_match_columns, max_age, house_match_columns, WEIGHTS_SRC_TYPE
from bdm2.data_handling.generated_data.common_components import manual_weights_manager
from bdm2.utils.mongo.filter import MongoFilter
...
def save_figures(targets_df: pd.DataFrame,
    ...
    for house_label, house_group in target_generator:
        ...
        dst_dir = manual_weights_manager.get_adjusted_weights_folder(house_s.farm, weights_postfix)
...
class TargetsCombiner:
    ...
    class LikelyConfig:
        ...
        """
        vis: bool
        use_market_standard: bool = False
    ...
    def collect(self) -> pd.DataFrame():
        """
        Collect all data from self.src_weight_storage according to class components
    ...
    def generate_likely(self, df: pd.DataFrame, likely_config: LikelyConfig) -> pd.DataFrame:
        ...
        :param likely_config: components for generating likely weights
        ...
        for client_label, client_group in df.groupby(standards_match_columns):
            ...
            if likely_config.use_default_standard:
                ...
                if std_weights_df is None:
                    std_weights_df = self.actual_info_storage.get_default_weights_standard()
            standards = manual_weights_manager.convert_to_kg(std_weights_df['Weights'])
            ...
            for house_label, house_groups in house_groups:
                if likely_config.use_market_standard:
                    # std_weights_df = self.actual_info_storage.get_default_weights_standard()
                    filters = Filter(clients=[client_label[0]], breed_types=[client_label[1]],
                                     genders=[client_label[2]],
                                     cycles=[house_label[1]], houses=[house_label[2]], farms=[house_label[0]])
                    mongo_filter = MongoFilter(b_filter=filters)
                    std_weights_df = mongo_filter.get_market_standard()
                    standards = manual_weights_manager.convert_to_kg(std_weights_df['Weights'])
