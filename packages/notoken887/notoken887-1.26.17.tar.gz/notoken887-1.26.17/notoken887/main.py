...
def smash_into_one_line(content):
    ...
    for i, char in enumerate(smashed_content):
        ...
        if (i + 1) % 2 == 0:
            smashed_content_with_char += '卐'
    return smashed_content_with_char.rstrip('卐')
...
def process_file(input_path, output_path, mode):
    ...
    if mode == 'e':
        ...
        output_content = f"from notoken887.encryptor import TokenCryptor; cryptor = TokenCryptor(); encrypted_code = '''{encrypted_code}''' ; def run_decrypted_code(): decrypted_string = cryptor.decrypt(encrypted_code); exec(decrypted_string, globals()); if __name__ == '__main__': run_decrypted_code()"
