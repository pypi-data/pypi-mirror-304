...
def create_main_pyw(input_path, output_path):
    ...
    for line in imports:
        if line.strip().startswith(("import ", "from ")):
            # Prefixing only local imports (not from installed packages)
            if not any(line.strip().startswith(pkg) for pkg in ("import ", "from ")) and 'result.' not in line:
                line = line.replace("import ", "import result.", 1) if line.startswith("import ") else line
                line = line.replace("from ", "from result.", 1) if line.startswith("from ") else line
