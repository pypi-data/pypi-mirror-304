...
def main():
    ...
    parser.add_argument(
                        ...
                        type=float,          # Parse each value as a float
                        nargs='+',           # Accept one or more values as a list
                        default=Params.consecutive_AT_scoring,
                        help=(
                            "Consecutive AT repeats form a hairpin structure instead of Z-DNA. In order to reflect that, "
                            "a penalty array is defined, which provides the score adjustment for the first and the "
                            "subsequent TA appearances. The last element will be applied to every subsequent TA "
