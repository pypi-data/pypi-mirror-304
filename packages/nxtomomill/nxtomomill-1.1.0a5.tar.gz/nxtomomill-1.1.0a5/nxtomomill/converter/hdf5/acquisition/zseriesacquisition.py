...
def is_z_series_frm_translation_z(
    ...
    :param projection_urls: list of DataUrl pointing to projection
                                     nodes.
    :return: True if the set of projections should be considered as a zseries (according to version 1 of z-series)
    """
    z_values = set()
    for url in projection_urls:
        with EntryReader(url) as entry:
            z_values_tmp, _ = BaseAcquisition.get_translation_z_frm(
...
class ZSeriesBaseAcquisition(BaseAcquisition):
    ...
    def get_z(self, entry):
        ...
        z_array = self._get_translation_z(entry, n_frame=None)[0]
        if z_array is None:
    ...
    @property
    def require_translation_z(self):
        return True
