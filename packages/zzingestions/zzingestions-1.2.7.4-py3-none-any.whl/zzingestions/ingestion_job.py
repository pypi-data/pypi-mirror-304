...
"""
    ...
    def process_parallel_read(self, max_workers:int=None, sleep:int=None) -> None:
        """
        ...
        self.debug_print(f"def PROCESS_PARALLEL_READ >> parameters >> sleep={sleep}")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for key, options in self.__parallel_read_options.items():
                ...
                self.pprint(f"Number of active threads after submit: {threading.active_count()}", lvl=1)
                if sleep:
                    time.sleep(sleep)
            ...
            for future in as_completed(future_options):
                ...
                self.pprint(f"Number of active threads after complete: {threading.active_count()}", lvl=1)
