...
async def entrada_de_notas_34(task):
    """
    ...
    try:
        ...
        if tipo_despesa_work.retorno == True:
        ...
        console.print(f"Selecionando a opção 'Manter Natureza de Operação selecionada'...\n")
        ...
        if not checkbox.get_toggle_state() == 1:
            checkbox.click()
            console.print("A opção 'Manter Natureza de Operação selecionada' selecionado com sucesso... \n")
        ...
        while True:
            console.print(f"Verificando a existencia de fornecedor com múltiplas referencias...\n")
            itens_by_supplier = await is_window_open_by_class("TFrmSelecionaItensFornecedor", "TFrmSelecionaItensFornecedor")
            if itens_by_supplier.IsOpened == True:
                new_app = Application(backend="uia").connect(class_name="TFrmSelecionaItensFornecedor")
                ...
                path_to_png = f"C:\\Users\\{username}\\Downloads\\{nota["nfe"]}.png"
                screenshot.save(path_to_png)
                console.print(f"Print salvo em {path_to_png}...\n")

                console.print(f"Preparando a imagem para maior resolução e assertividade no OCR...\n")
                ...
                console.print(f"Interagindo com os multiplos itens e detectando qual ira ser selecionado para andamento no processo... \n")
                copied_codes_list = []
                count = 0
                last_copied = None
                ...
                while True:
                    ...
                    copied_codes_list.append({"linha": count, "conteudo": window_message})
                ...
                for cod_item, info in extracted_items.items():
                    ...
                    similarity_ratio = difflib.SequenceMatcher(None, item_da_nota, descricao).ratio()
                ...
                if best_match and highest_ratio > 0.7:
                    console.print(f"Melhor semelhança encontrada {best_match}, selecionando... \n")
                    send_keys("^({HOME})")
                    send_keys('{DOWN ' + str(best_match["Linha"]) + '}')
                    send_keys('%o')
                    await worker_sleep(2)
                else:
                    itens_nao_semelhantes.append({"item": item_da_nota, "match": best_match})
                    send_keys('%o')
            else:
                console.print("Não possui a tela de fornecedor com múltiplas referencias, seguindo com o processo...\n")
                break
        ...
        try:
            warning_pop_up_pagamentos = main_window.child_window(class_name="TMessageForm",title="Warning")
        except:
            warning_pop_up_pagamentos = None

        if warning_pop_up_pagamentos.exists():
            console.print("Erro: Warning informando que a Soma dos pagamentos não bate com o valor da nota. ...\n")
            return RpaRetornoProcessoDTO(
                sucesso=False,
            ...
            console.print("Warning informando que a Soma dos pagamentos não bate com o valor da nota não existe ...\n")
        ...
        while i < max_attempts:
            ...
            for window in Desktop(backend='uia').windows():
                if "Rateio" in window.window_text():
                    ...
                    console.print("A janela 'Rateio da Despesas' foi encontrada. Continuando para andamento do processo...\n")
                    break
