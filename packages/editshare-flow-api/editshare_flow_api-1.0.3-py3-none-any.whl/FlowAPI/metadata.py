...
class Metadata(Connection):
    ...
    def get_file_from_hash(self, flow_hash):
        """Fetch a file that matches a hash"""

        return self.getThatReturnsObj("/files?hash={}".format(flow_hash))
    ...
    def find_custom_metadata_field_by_name(self, field_name):
        ...
        url += self.safe_url_string(field_name)
    ...
    def find_custom_metadata_field_by_db_key(self, db_key):
        ...
        url += self.safe_url_string(db_key)
        return self.getThatReturnsObj(url)
    ...
    def createCustomMetadataConfiguration(self, data):
        """Create a new custom metadata template"""
    ...
    def getCustomMetadataConfigurations(self):
        """Fetch all custom metadata templates"""
    ...
    def get_project(self, project_id, include_vendor_data=False):
        """Fetch information about a project"""

        url = "/projects/" + str(project_id)
        if include_vendor_data:
            url += "?vendor_data=true"
    ...
    def findProject(self, name):
        ...
        escaped_name = self.safe_url_string(str(name))
    ...
    def createProjectFolder(self, project_id, parent_id, folder_name):
        """Create a new project folder (bin)"""
    ...
    def ensureProjectFolderExists(self, project_id, folder_name):
        """Ensure a project folder (bin) exists"""
    ...
    def addClipToProjectFolder(self, project_id, parent_folder_id, clip_id):
        """Helper to add a clip to a project"""
    ...
    def createSharedResource(self, name, filename, filesize, resource_type):
        """Create a sharred resource entry"""
    ...
    def send_system_message(self, msg_key, msg_value, msg_origin=""):
        ...
        if msg_origin:
            message_data["origin"] = msg_origin
        self.post("/send_message", message_data)
        return self.lastReturnCode() == 200
    ...
    def delete_log_entry(self, log_entry_id):
        """Delete a log entry"""
        return self.deleteThatReturnsBOOL(
            "delete_log_entry", "/log_entries/" + str(log_entry_id)
        )
