...
class Scan(Connection):
    ...
    def get_job(self, job_uuid):
        ...
        return self.getThatReturnsObj("/jobs/{}".format(job_uuid))

    def get_job_progress(self, job_uuid):
        """Get job progress"""

        return self.getThatReturnsObj("/jobs/{}/progress".format(job_uuid))

    def get_job_results(self, job_uuid):
        """Get job results"""

        return self.getThatReturnsObj("/jobs/{}/results".format(job_uuid))

    def scanAsset(self, job):
        """Start a single asset scan"""

        reply = self.post("/asset", json.dumps(job))
        # print reply
    ...
    def proxyManagmentScan(self):
        ...
        reply = self.post("/proxymanagement", "")
        # print reply
        if self.lastReturnCode() != 200:
            return False
        return json.loads(reply)
    ...
    def scanMediaspace(self, mediaspace, create_proxies=True, scan_type="normal"):
        ...
        return self.scanMediaspaceInner(
            mediaspace, "", True, create_proxies, False, scan_type
        )

    def scan_media_space(
        self,
        mediaspace,
        scan_type="normal",
        create_proxies=True,
        ...
        return self.scanMediaspaceInner(
            mediaspace, "", True, create_proxies, create_editing_proxies, scan_type
        )
    ...
    def scanMediaspaceInner(
        ...
        job = {
            "auto_delete": False,
            "create_proxies": create_proxies,
            "create_editing_proxies": create_editing_proxies,
            "scan_type": scan_type,
        ...
        reply = self.post("/mediaspace", json.dumps(job))
        if self.lastReturnCode() != 200:
            return False
        return json.loads(reply)
    ...
    def scanMediaspaceDirectory(
        ...
        return self.scanMediaspaceInner(
            mediaspace, directory, recursive, create_proxies, False, scan_type
        )
    ...
    def scan_files(
        ...
        *,
        ...
        job = {
            "createproxy": create_streaming_proxies,
            "create_editing_proxy": create_editing_proxies,
    ...
    def getAssetScan(self, job_id):
        ...
        return self.getThatReturnsObj("/asset/" + job_id)

    def getMediaspaceScan(self, job_id):
        """Returns the job status of the scan with job_id"""

        return self.getThatReturnsObj("/mediaspace/" + job_id)

    def getScanResults(self, job_id):
        """Returns the job results"""

        return self.getThatReturnsObj("/jobs/{}/results".format(job_id))
    ...
    def get_workers(self):
        ...
        return self.getThatReturnsObj("/workers").json()
    ...
    def add_worker(self, host: str):
        ...
        return self.post("/workers", json.dumps(add_worker_request))
    ...
    def remove_worker(self, host: str):
        ...
        return self.delete("/workers", json.dumps(remove_worker_request))
