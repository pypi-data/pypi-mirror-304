...
class _Query(_OperationContainer):
    ...
    def query_to_flight_stream(
        ...
        client_v1, client_v2, metadata = self._common.get_commander_and_metadata(args)
        ...
        # Subscribe to logs
        log_stream: Iterable[RunnerInfo] = client_v1.SubscribeLogs(job_id, metadata=metadata)
    ...
    def query_to_json_file(
        ...
        filename = _ensure_parent_dir_exists('filename', filename)
        if file_format == 'json':
            first_line = '[\n'
            last_line = ']'
            line_suffix = ',\n'
    ...
    def query_to_parquet_file(
        ...
        **kwargs,
        ...
        if not filename.lower().endswith('.parquet'):
            raise ValueError('filename should have a .parquet extension')
    ...
    def query_to_csv_file(
        ...
        **kwargs,
        ...
        if not filename.lower().endswith('.csv'):
            raise ValueError('filename should have a .csv extension')
        ...
        pcsv.write_csv(table, filename, **kwargs)
        return filename
