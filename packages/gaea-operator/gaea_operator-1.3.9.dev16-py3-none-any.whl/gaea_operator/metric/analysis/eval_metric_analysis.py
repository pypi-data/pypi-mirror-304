...
class EvalMetricAnalysis(object):
    ...
    def set_labels(self, labels: List):
        ...
        if self.category in ("Image/ImageClassification/MultiTask", ):
            ...
            ## 处理每个属性
            for label in labels:
                if 'parentID' not in label:
                    task_id = label['id']
                    task_name = label['name']
                    self.task_labels[task_id] = {'task_name': task_name, 'labels': []}
                # else pass
            ## 处理每个属性的标签
            for label in labels:
                if 'parentID' in label:
                    parentID = label['parentID']
                    self.task_labels[parentID]['labels'].append(label)
    ...
    def _format_multitask_classification_result(self, metric_result: Dict):
        ...
        task_names = [
                {'id': task_id, 'name': task_item['task_name']}
                for task_id, task_item in self.task_labels.items()]
        multitask_class_schema_metric = MultiAttributeImageClassificationMetric(
                metrics=[total_accuracy], labels=task_names)
