...
@router.channel_post(Command('autodownload'))
...
@router.callback_query(lambda c: c.data in ['split', 'bitrate', 'subtitles'])
async def process_callback(callback_query: types.CallbackQuery):
    logger.debug('🥑 process_callback()')
    action = callback_query.data

    if action == 'split':
        await bot.send_message(callback_query.from_user.id, "Введите YouTube URL для Split:")
        #router.message.register(get_url_for_split)

    elif action == 'bitrate':
        await bot.send_message(callback_query.from_user.id, "Введите YouTube URL для Bitrate:")
        #router.message.register(get_url_for_bitrate)

    elif action == 'subtitles':
        await bot.send_message(callback_query.from_user.id, "Введите YouTube URL для Subtitles:")
        #router.message.register(get_url_for_subtitles)


@router.callback_query(lambda c: c.data == 'split')
...
@router.callback_query(lambda c: c.data == 'bitrate')
async def process_callback_split(callback_query: types.CallbackQuery):
    await bot.edit_message_text(
        chat_id=callback_query.from_user.id,
        message_id=callback_query.message.message_id,
...
@router.callback_query(lambda c: c.data == 'subtitles')
