from ...environment import os, time, datetime,tqdm,np, argparse, torch, tensorboardX
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='datasets', help='datasets path', action = True)
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.val_loader = load_datasets(mode='val', img_size=args.img_size, data_dir=args.data_dir, batch_size=args.batch_size, shuffle=True, transform=None)
        self.test_loader = load_datasets(mode='test', img_size=args.img_size, data_dir=args.data_dir, batch_size=args.batch_size, shuffle=True, transform=None)
        ...
        ## SummaryWriter
        self.writer = tensorboardX.SummaryWriter(log_dir=args.save_folder)
    ...
    def training(self):
        ...
        base_log = "Epoch-Iter: [{:d}/{:d}]-[{:d}/{:d}] || Lr: {:.6f} || Loss: {:.4f}={:.4f}+{:.4f}*{:.4f} || " \
        ...
        for epoch in range(self.args.epochs):
            ...
            for i, (data, mask) in enumerate(tbar):
                ...
                if self.iter_num % self.args.log_per_iter == 0:
                    self.logger.info(base_log.format(epoch + 1, self.args.epochs, self.iter_num % self.iter_per_epoch,
                                                     self.iter_per_epoch, self.optimizer.param_groups[0]['lr'],
                                                     loss_all.item(),loss_softiou.item(), gamma.item(), loss_mse.item(),
                                                     cost_string, eta_string))


                if (self.iter_num % self.args.save_iter_step) == 0 or self.iter_num % self.iter_per_epoch == 0:
                    self.validation()

                tbar.set_description(base_log % (epoch + 1, self.args.epochs, self.iter_num % self.iter_per_epoch,
                                                     self.iter_per_epoch, self.optimizer.param_groups[0]['lr'],
                                                     loss_all.item(),loss_softiou.item(), gamma.item(), loss_mse.item(),
                                                     cost_string, eta_string))
    ...
    def validation(self):
        ...
        for i, (data, labels) in enumerate(tbar):
            ...
            self.eval_PD_FA.update(out_T, labels)

            miou, prec, recall, fmeasure = self.metric.get()
            PD, FA = self.eval_PD_FA.get()

            tbar.set_description(base_log % (self.args.dataset, miou, self.best_miou, fmeasure, self.best_fmeasure, PD, FA))
        ...
        self.logger.info(base_log.format(self.args.dataset, miou, self.best_miou, fmeasure, self.best_fmeasure, PD, FA))
