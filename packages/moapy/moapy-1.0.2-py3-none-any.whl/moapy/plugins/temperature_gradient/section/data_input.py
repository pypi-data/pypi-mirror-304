...
class CompositeIGirderInput(MBaseModel):
    ...
    section: CompositeIGirderSection
...
SectionInput = Annotated[
    ...
    Field(title="Section Input", discriminator="section_type"),
]


class UnSurfacedTypeInput(MBaseModel):
    model_config = ConfigDict(title="Unsurfaced")
    surfacing_type: Literal["unsurfaced"] = Field(default="unsurfaced", description="Surfacing Type")


class WaterProofedTypeInput(MBaseModel):
    model_config = ConfigDict(title="Waterproofed")
    surfacing_type: Literal["waterproofed"] = Field(default="waterproofed", description="Surfacing Type")


class SurfacedTypeInput(MBaseModel):
    model_config = ConfigDict(title="Surfaced")
    surfacing_type: Literal["surfacing"] = Field(default="surfacing", description="Surfacing Type")
    surfacing_thickness: float = Field(..., description="Surfacing Thickness")
...
SurfacingTypeInput = Annotated[
    ...
    Field(title="Surfacing Type", discriminator="surfacing_type"),
]
