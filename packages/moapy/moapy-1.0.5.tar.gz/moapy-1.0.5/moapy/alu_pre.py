...
class AluMaterial(MBaseModel):
    ...
    Alu DB Material
    """
    code: str = dataclass_field(default='AA(A)', description="material code", readOnly=True)
    matl: str = dataclass_field(default='2014-T6', description="material name", enum=enum_to_list(enAluminumMaterial_AA))
    product: str = dataclass_field(default='Extrusions', description="product type", readOnly=True)
