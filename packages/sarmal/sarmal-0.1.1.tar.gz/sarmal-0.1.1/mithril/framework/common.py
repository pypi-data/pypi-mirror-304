...
ReduceType = int | tuple[int, ...] | NoneType | ToBeDetermined
MainValueType = (
    int
    | float
    | tuple
...
class Tensor(BaseData, GenericDataType[DataType]):
    ...
    def match_shapes(self, other: Tensor):
        updates = Updates()
    ...
    def set_value(self, value: DataType | TensorValueType) -> Updates:
        if self._logical_data:
...
class TemplateBase:
    ...
    def __eq__(self, other: TemplateConnectionType):
    ...
    def __ne__(self, other: TemplateConnectionType):
...
class Uniadic:
    ...
    def __eq__(self, other: Uniadic) -> bool:
...
class ShapeNode:
    ...
    def get_shapes(
        self,
        u_keys: dict[ShapeType, str] | None = None,
        v_keys: dict[ShapeType, str] | None = None,
        symbolic=True,
        verbose=False,
...
class Table:
    ...
    def add_column(self, column: Sequence[str | list[str]]):
        for idx, row in enumerate(column[: len(self.headers)]):
            assert isinstance(row, str)
            self.headers[idx].append(row)

        for idx_, row in enumerate(column[idx + 1 :]):
            self.cells[idx_].append(row)
...
def create_shape_repr(
    ...
    solver: ConstraintSolver,
    used_keys: dict[str | int, ShapeType] | None = None,
) -> ShapeRepr:
    if used_keys is None:
        _used_keys: dict[str | int, ShapeType] = {}
    else:
    ...
    for symbol in shp_list:
        ...
        if isinstance(symbol, tuple):
            ...
            symbol_obj = _used_keys.get(symbol := symbol[0], Variadic())
