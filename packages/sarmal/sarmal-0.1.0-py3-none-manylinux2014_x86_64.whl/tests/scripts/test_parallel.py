...
def test_torch_parallel_1():
    ...
    assert sharded_tensor._local_tensor.shape == (2, 128)
    assert replicated_tensor._local_tensor.shape == (8, 128)

    # Apply op to sharded tensor
    sharded_tensor += 1
    np.testing.assert_allclose(
        sharded_tensor._local_tensor.cpu(), (torch.ones(2, 128) + 1)
    )

    # Get full_tensor from sharded_tensor
    full_tensor = sharded_tensor.full_tensor().cpu()
    np.testing.assert_allclose(full_tensor, (torch.ones(8, 128) + 1))

    result_tensor = replicated_tensor + sharded_tensor
    assert result_tensor._local_tensor.shape == (2, 128)
    np.testing.assert_allclose(
        result_tensor.full_tensor().cpu(), (torch.ones(8, 128) + 2)
    )
...
def test_torch_parallel_error_5():
    ...
    assert sharded_tensor._local_tensor.shape == (4, 128)
...
def test_torch_parallel_error_9():
    ...
    assert sharded_tensor._local_tensor.shape == (4, 64)
    assert sharded_tensor.shape == (8, 128)
...
def test_torch_parallel_error_10():
    ...
    assert sharded_tensor._local_tensor.shape == (8, 64)
    assert sharded_tensor.shape == (8, 128)
...
def test_torch_parallel_error_11():
    ...
    assert sharded_tensor._local_tensor.shape == (8, 128)
    assert sharded_tensor.shape == (8, 128)
...
def test_torch_parallel_multi_parallel_1():
    ...
    np.testing.assert_allclose(
        res1["output"].full_tensor(), (left + right).full_tensor()
    )
    np.testing.assert_allclose(
        res2["output"].full_tensor(), (left * right).full_tensor()
    )
...
def test_torch_parallel_multi_parallel_2():
    ...
    assert tensor1._local_tensor.shape == (4, 128)
    assert tensor2._local_tensor.shape == (8, 64)

    tensor3 = backend1.ones([8, 128]) * 5
    tensor4 = backend2.ones([8, 128]) * 5

    assert tensor3._local_tensor.shape == (8, 128)
    assert tensor4._local_tensor.shape == (8, 128)
...
def test_jax_parallel_1():
    if "cuda" in mithril.JaxBackend.get_available_devices():
        ...
        assert sharded_tensor.sharding.shape == (4, 1)
        assert replicated_tensor.sharding.shape == (1, 1)
        ...
        result_tensor = replicated_tensor + sharded_tensor
        assert result_tensor.sharding.shape == (4, 1)
