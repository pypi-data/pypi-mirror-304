...
def check_shapes_semantically(
    shape_1: SemanticShapeType, shape_2: SemanticShapeType
) -> None:
    """
    ...
    """
    mapping: Mapping[str, Uniadic | Variadic] = (
        dict()
    )  # Mapping from items in shape_1 to shape_2.
    reverse_mapping: (
        dict[Uniadic, str] | dict[Variadic, str] | dict[Uniadic | Variadic, str]
    ) = dict()  # Reverse mapping from items in shape_2 to shape_1.
...
def check_single_shape_semantically(list_1, list_2, mapping=None, reverse_mapping=None):
    if mapping is None:
        mapping = {}
