...
class StaticDataStore(GenericDataType[DataType]):
    ...
    def add_static_data(
        ...
        if key in self.unused_keys:
            ...
            assert (isinstance(data, Tensor) and self.is_tensor_type(value)) or (
                isinstance(data, Scalar) and self.is_scalar_type(value)
            )
            ...
            if key not in self._intermediate_non_differentiables:
                ...
                #         _, _updates = self.add_static_data(al_key, value)
                #         updates |= _updates
                # updated_keys |= _updated_keys
        # Finally update cached_data, infer unused keys and
        # return newly added static keys.
        # statics = self._update_cached_data() | {key}
        self.constraint_solver(updates)
