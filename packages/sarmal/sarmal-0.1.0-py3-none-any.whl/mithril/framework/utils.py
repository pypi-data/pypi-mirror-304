...
def infer_all_possible_types(type_def: type | UnionType) -> set[type | UnionType]:
    possible_types = {type_def}
    if type(type_def) is UnionType:
        sub_types = set(type_def.__args__)
...
def find_type(connection) -> type:
    if isinstance(connection, tuple | list):
        element_types = [find_type(elem) for elem in connection]
        if isinstance(connection, tuple):
            return tuple[*element_types]  # type: ignore
        else:
            element_type_set = set(element_types)
            result = element_type_set.pop()
            for typ in element_type_set:
                result |= typ
            return list[result]
    else:
...
def sort_type(type1: type | UnionType):
    """
    ...
    if isinstance(type1, UnionType):
        ...
        and type1.__origin__ is tuple
        or hasattr(type1, "__origin__")
        and type1.__origin__ is list
    ):
