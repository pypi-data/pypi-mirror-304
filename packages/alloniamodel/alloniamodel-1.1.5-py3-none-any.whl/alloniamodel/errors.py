...
def invalid_revision_err(n_versions: Union[None, int] = None):
...
def fewer_revisions_than_data_err(
    revision: Union[int, list, tuple], n_data: int
):
...
def not_read_only_err():
    return ValueError(
        "Can only save a description in this method if the model is"
        " read_only. Please use AleiaModel.save() instead."
    )


def new_model_err():
    return ValueError(
        "Can not update description of a new model. Use AleiaModel.save()"
        " instead on a non read-only model."
    )
...
def limit_arg_err(
    nargs: int,
    number_of_args: Union[int, None] = None,
    min_args: Union[int, None] = None,
    max_args: Union[int, None] = None,
):
    if number_of_args:
        return ValueError(
