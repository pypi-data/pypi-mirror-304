...
fn_tag="tag"
fn_version="src/serialtools/__init__.py"
...
if [ "$build_doc" = 'true' ]; then
	...
	modules="$(find src/serialtools/ -name '*.py' -type f | sed -E 's:src/serialtools/(.*)\.py:serialtools.\1:' | sed 's:/:.:g' | sed 's/serialtools.__init__/serialtools/' | sed 's/^/   /' | sort)"
	newline="$(printf '\n ')"  # the space is required to protect the newline. it does not seem to be inserted into the file.
	sed -i '/^Reference/,/^[A-Z]/ { /^\.\. toctree::/,/^===/ { /^ *serialtools/d } }' 'docs/source/index.rst'
	sed -i "/^Reference/,/^[A-Z]/ { /^\.\. toctree::/,/^$/ { /^$/a \\${modules//$newline/\\$newline}$newline} }" 'docs/source/index.rst'

	# build documentation
	sphinx-apidoc --separate -o 'docs/source' 'src/serialtools'
...
if grep -ERI -ho 'serialtools v[0-9]+\.[0-9]+\.[0-9]+(-[^ ]*)?' docs/ | grep -v "serialtools $version\$"; then
