...
from libs.server.kiln_server.project_api import (
    connect_project_api,
    project_from_id,
)
...
def test_create_and_load_project(client):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "libs.server.kiln_server.project_api.default_project_path",
...
def test_import_project_success(client):
    ...
    ), patch("libs.server.kiln_server.project_api.add_project_to_config") as mock_add:
...
def test_get_project_success(client):
    ...
    with patch(
        "libs.server.kiln_server.project_api.project_from_id",
...
def patched_config(mock_config):
    with patch(
        "libs.server.kiln_server.project_api.Config.shared",
