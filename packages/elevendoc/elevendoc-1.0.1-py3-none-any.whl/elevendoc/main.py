...
def main(
    ...
    Project_description = input(
            "Pour plus de précision dans les dosctrings, le readme, ou le fichier advisory, veuillez entrer une description la plus complète possible de votre projet : (faite 'entrer' pour passer) "
            )
    for dirpath, dirnames, filenames in os.walk(root_dir):
        for filename in filenames:
            ...
            if docstring_bool:
                if filename.endswith(".py"):
                    ...
                    write_changes_function(
                        file_path, tree, docstring_list, function_defs_list, force_bool,verbose
                    )
        ...
        if Readme_bool:
            Readme_generation = send_to_chatgpt(
                Readme_promt_memory, Project_description, False, True, False, model=os.getenv("MODEL_README")
            )
            with open((dirpath + "/Generated_Readme.md"), "w", encoding="utf-8") as file:
...
def run():
    ...
    main(args.folder, args.docstring, args.Readme, args.advisory, args.force, args.verbose)
