    ...
    try:
        ...
        for j, (class_name, function_def) in enumerate(function_defs_list):
            ...
            if class_name:
                ...
                class_pattern = re.compile(rf"class\s+{class_name}\s*(\([\w\s,]*\))?\s*:", re.MULTILINE)
            ...
            pattern = re.compile(r"\)\s*(->\s*[\w\[\], |]+)?\s*:")  # end of function signature
...
def send_to_chatgpt(
    code, Readme_dictionary, description, dockstrings_completion, Readme_completion, advisory_completion, model
):
    """
    Summary:
