...
from dyff.schema.platform import DyffSchemaBaseModel
from dyff.schema.platform import Score as SchemaScore
...
class Score(DyffSchemaBaseModel):
    quantity: Decimal = pydantic.Field(
        description="The numeric quantity associated with the conclusion."
    ...
    def _validate_format(cls, v):
        try:
            SchemaScore._format_quantity(v, Decimal("3.14"), unit="kg")
        except Exception:
    ...
    def _repr_html_(self) -> str:
        ...
        return template.substitute(
            quantity=SchemaScore._format_quantity(
                self.format, self.quantity, unit=self.unit
            ),
            text=self.text,
        )
