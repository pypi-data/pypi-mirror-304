...
class settings(object):
    ...
    def __init__(self,
        ...
        self.pth_resume = f"weight/{segHead}/{data_type}_d{downsample}_1024_best.pth"
        # self.pth_name = f"{data_type}_d{downsample}_1024_fed"
        ...
        if data_type == "FBP":
            self.early_stopping_patience = max(2 * downsample + 5, 7)
            self.lr_patience = max(2 * downsample + 3, 5)
        else:
