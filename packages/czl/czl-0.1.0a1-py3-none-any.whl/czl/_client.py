...
class CzlAI(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous czl-ai client instance.

        This automatically infers the `api_key` argument from the `CZLAI_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("CZLAI_API_KEY")
        if api_key is None:
            raise CzlAIError(
                "The api_key client option must be set either by passing api_key to the client or by setting the CZLAI_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("CZL_AI_BASE_URL")
        if base_url is None:
...
class AsyncCzlAI(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async czl-ai client instance.
