...
# Register the DejaVu Sans Mono font for code-input
LabelBase.register(
    name="DejaVuSansMono",
    fn_regular="assets/fonts/DejaVuSansMono.ttf",  # Regular font
)
...
class MainWidget(BoxLayout):
    ...
    def get_git_log(self):
        ...
        try:
            git_log_output = subprocess.check_output(
                ['git', 'log', '--graph', '--pretty=format:%H - %s - %an - %ad', '--date=local'],
            ...
            for i, commit in enumerate(commit_lines):
                ...
                if len(parts) > 1:
                    ...
                    if len(details_parts) == 3:
                        ...
                        # commit_date = datetime.datetime.strptime(date, "%a %b %d %H:%M:%S %Y")
    ...
    def on_file_click(self, file_name):
        ...
        if file_name.endswith(('.ttf', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.exe', '.dll', '.zip', '.pdf')):
            print(f"{file_name} is a binary file. Cannot display.")
            self.diff_scroll.text = f"Cannot display binary file: {file_name}"
            return
        ...
        try:
            ...
            print(f"Error retrieving file content for {file_name} at commit {commit_hash}: {e}")
            self.diff_scroll.text = f"Error retrieving file content for {file_name} at commit {commit_hash}: {e}"
...
class SelectableRecycleBoxLayout(FocusBehavior, LayoutSelectionBehavior, RecycleBoxLayout):
    ...
    def move_selection(self, direction):
        ''' Moves the selection by direction (-1 for up, 1 for down) '''
...
class FileLabel(Label):
    ...
    def on_selected(self, *args):
        if self.selected:
            ...
            search_text = f"diff --git a/{self.text} b/{self.text}"
...
class TreeLabel(Label, TreeViewNode):
    ...
    def on_touch_up(self, touch):
        ...
        if self.collide_point(*touch.pos):
            self.root_widget.on_file_click(self.file)
            return super().on_touch_up(touch)
...
class siGit(App):
        # Get the directory where this file (likely main.py) is located
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Construct the full path to the main.kv file
    kv_file_path = os.path.join(current_dir, 'main.kv')
