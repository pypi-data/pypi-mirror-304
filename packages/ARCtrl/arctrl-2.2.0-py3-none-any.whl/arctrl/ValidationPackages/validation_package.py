...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2959(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2958(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2957(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2955(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2955))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2957))

            return append(singleton("{"), delay(_arrow2958))

        return join("\n", to_list(delay(_arrow2959)))
...
ValidationPackage_reflection = _expr2962
