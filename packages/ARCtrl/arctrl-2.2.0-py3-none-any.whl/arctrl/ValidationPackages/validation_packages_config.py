...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow3046(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3045(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3044(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3043(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3042(__unit: None=None) -> IEnumerable_1[str]:
                            ...
                            return append(singleton("]"), delay(_arrow3041))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3042))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3043))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3044))

            return append(singleton("{"), delay(_arrow3045))

        return join("\n", to_list(delay(_arrow3046)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3049(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3048(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3048, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3049))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3050
