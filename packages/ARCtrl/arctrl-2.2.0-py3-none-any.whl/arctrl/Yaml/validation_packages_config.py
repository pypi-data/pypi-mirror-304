...
def ValidationPackagesConfig_encoder(validationpackage: ValidationPackagesConfig) -> YAMLElement:
    ...
    def _arrow3060(value: str, validationpackage: Any=validationpackage) -> YAMLElement:
        return string(value)

    def _arrow3061(validationpackage_1: ValidationPackage, validationpackage: Any=validationpackage) -> YAMLElement:
        return ValidationPackage_encoder(validationpackage_1)

    obj_seq: FSharpList[tuple[str, YAMLElement]] = choose(chooser, of_array([try_include("arc_specification", _arrow3060, validationpackage.ARCSpecification), ("validation_packages", resizearray(_arrow3061, validationpackage.ValidationPackages))]))
...
def _arrow3066(value_2: YAMLElement) -> ValidationPackagesConfig:
    def getter(get: IGetters) -> ValidationPackagesConfig:
        def _arrow3064(__unit: None=None, get: Any=get) -> Array[ValidationPackage]:
        ...
        return ValidationPackagesConfig(_arrow3064(), _arrow3065())

    return object(getter, value_2)


ValidationPackagesConfig_decoder: Callable[[YAMLElement], ValidationPackagesConfig] = _arrow3066
...
def ARCtrl_ValidationPackages_ValidationPackagesConfig__ValidationPackagesConfig_toYamlString_Static_71136F3F(whitespace: int | None=None) -> Callable[[ValidationPackagesConfig], str]:
    def _arrow3068(vp: ValidationPackagesConfig, whitespace: Any=whitespace) -> str:
