...
def _arrow2385(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2386(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2387(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2385, ROCrate_decoder_1), map(_arrow2386, ROCrate_decoder_2), map(_arrow2387, ROCrate_decoder_3)]))
...
def _arrow2389(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2390(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2391(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2389, ISAJson_decoder_1), map(_arrow2390, ISAJson_decoder_2), map(_arrow2391, ISAJson_decoder_3)]))
