...
def SparseRowModule_getAllValues(i: IEnumerable_1[tuple[int, str]]) -> IEnumerable_1[str | None]:
    ...
    def _arrow1099(i_1: int, i: Any=i) -> str | None:
        return try_find(i_1, m)

    return initialize(max_by(projection, i, ObjectExpr1098())[0] + 1, _arrow1099)
...
def SparseTable_Create_Z2192E64B(matrix: Any | None=None, keys: FSharpList[str] | None=None, comment_keys: FSharpList[str] | None=None, length: int | None=None) -> SparseTable:
    ...
    return SparseTable(default_arg(matrix, Dictionary([], ObjectExpr1102())), default_arg(keys, empty()), default_arg(comment_keys, empty()), default_arg(length, 0))
...
def SparseTable_AddRow(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    ...
    def _arrow1103(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
        return tuple[0]
    ...
    length: int = (0 if is_empty(values) else (1 + max_by(_arrow1103, values, ObjectExpr1104())[0])) or 0
...
def SparseTable_AddComment(key: str, values: IEnumerable_1[tuple[int, str]], matrix: SparseTable) -> SparseTable:
    if length_1(values) == 0:
        ...
        iterate(action, values)
        def _arrow1105(tuple: tuple[int, str], key: Any=key, values: Any=values, matrix: Any=matrix) -> int:
            return tuple[0]

        class ObjectExpr1106:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        length: int = (0 if is_empty(values) else (1 + max_by(_arrow1105, values, ObjectExpr1106())[0])) or 0
                ...
                if en.System_Collections_IEnumerator_MoveNext():
                    ...
                    if pattern_matching_result == 0:
                        ...
                        if active_pattern_result_1 is not None:
                            ...
                            if vals is not None:
                                def _arrow1109(__unit: None=None, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                    v_2: IEnumerable_1[tuple[int, str]] = vals
                                    k_1: str = key
                                    def _arrow1108(label: str) -> bool:
                                        return k_1 == (prefix_1 + label)

                                    return exists(_arrow1108, labels)
                        ...
                        if pattern_matching_result_1 == 0:
                            ...
                            def _arrow1107(label_1: str, matrix: Any=matrix, remarks: Any=remarks, line_number_1: Any=line_number_1) -> bool:
                                return k_3 == (prefix_1 + label_1)

                            matrix_mut = SparseTable_AddRow(find(_arrow1107, labels), v_3, matrix)
                            remarks_mut = remarks
                            line_number_1_mut = line_number_1 + 1
                            continue
...
def SparseTable_ToRows_759CAFC1(matrix: SparseTable, prefix: str | None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    prefix_1: str = "" if (prefix is None) else (prefix + " ")
    def _arrow1115(__unit: None=None, matrix: Any=matrix, prefix: Any=prefix) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow1111(key: str) -> IEnumerable_1[tuple[int, str]]:
            def _arrow1110(i: int) -> str:
                return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key, i + 1))

            return SparseRowModule_fromValues(cons(prefix_1 + key, initialize_1(matrix.ColumnCount - 1, _arrow1110)))

        def _arrow1114(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow1113(key_1: str) -> IEnumerable_1[tuple[int, str]]:
                def _arrow1112(i_1: int) -> str:
                    return SparseTable__TryGetValueDefault_5BAE6133(matrix, "", (key_1, i_1 + 1))

                return SparseRowModule_fromValues(cons(Comment_wrapCommentKey(key_1), initialize_1(matrix.ColumnCount - 1, _arrow1112)))

            return map(_arrow1113, matrix.CommentKeys)

        return append_1(map(_arrow1111, matrix.Keys), delay(_arrow1114))

    return delay(_arrow1115)
