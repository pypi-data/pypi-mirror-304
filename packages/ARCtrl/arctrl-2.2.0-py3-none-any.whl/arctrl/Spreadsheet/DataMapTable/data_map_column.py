...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    def _arrow1309(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1308(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1307(dc_4: DataContext) -> FSharpList[FsCell]:
                            ...
                            def _arrow1301(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1300(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1299(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1298))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1299))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1300))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1301))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1302))

                    return append(_arrow1295(), delay(_arrow1304))

                return to_list(delay(_arrow1306))

            return map_1(_arrow1307, dc)

        return append(singleton_1(headers), delay(_arrow1308))

    return transpose(to_list(delay(_arrow1309)))
