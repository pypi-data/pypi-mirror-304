...
Init_reflection = _expr3093

def _expr3094() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow3100(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow3099(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3098(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3097(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3096(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3095(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow3095))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow3096))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow3097))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow3098))

        return append(singleton(Clone_get_clone()), delay(_arrow3099))

    return create_git_contract(to_array(delay(_arrow3100)))
