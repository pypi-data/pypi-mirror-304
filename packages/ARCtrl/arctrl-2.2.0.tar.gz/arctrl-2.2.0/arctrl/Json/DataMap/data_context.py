...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2504(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2509(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2511(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2514(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2519(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2523(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2504, DataContext__get_Explication(dc)), try_include("unit", _arrow2509, DataContext__get_Unit(dc)), try_include("objectType", _arrow2511, DataContext__get_ObjectType(dc)), try_include("description", _arrow2514, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2519, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2523, DataContext__get_Label(dc))]))
...
def _arrow2569(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2562(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2564(__unit: None=None) -> OntologyAnnotation | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2562(), _arrow2564(), _arrow2565(), _arrow2566(), _arrow2567(), _arrow2568(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2569)
