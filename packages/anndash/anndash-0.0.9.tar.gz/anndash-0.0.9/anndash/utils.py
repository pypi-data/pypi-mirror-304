...
__all__ = ['fullpath', 'default_env', 'env_dependencies', 'ispublic', 'isproperty', 're_imatch', 'str_match', 'str_imatch',
           ...
           'str_cased_eq', 'col_cased_eq', 'check_columns', 'subset_column_names', 'subset_columns',
           'get_basis_dataframe', 'make_bases_dataframe', 'make_embed_dataframe', 'make_color_dataframe', 'get_ndim',
           'var_index', 'get_basis_keys', 'get_gene_info', 'configure_anndash_keys', 'flatten_obs_array', 'safe_colors']
...
from anndash.hints import (
    ...
    Flags, MatchQ, Columns, REGEX_BASIS, GeneInfo
)
...
def ispublic(member):
    '''Any function, method, or property with `<member>.__name__` starting with an underscore`'_'`.''';
    name = getattr(member, '__name__', '_')
    return name.startswith('_')

def isproperty(member):    
    '''Neither a function or method but with a public name.''';
...
def str_cased_eq(s: str, t: str, i: bool = True) -> bool:
    '''String equality with/without case insensitivy (`i`).''';
    if i: return str(s).casefold() == str(t).casefold()
    return str(s) == str(t)

def col_cased_eq(c: pd.Series, s: str, i: bool = True) -> pd.Series:
    '''Determines the element wise string equality in a Pandas `Series` with dtype in `{'object' | 'category'}` 
    with/without case insensitivy (`i`).'''
...
def get_ndim(adata: ad.AnnData, basis: str | None = None, max_dim: int | None = 10) -> int:
    ndim = 2
...
def var_index(adata: ad.AnnData, col: str, val: str | None) -> int:
    var = adata.var[col]
...
def get_gene_info(adata: ad.AnnData, gene: str, gene_col: str | None = 'gene_symbol') -> GeneInfo | tuple[int, str | int, str]:
