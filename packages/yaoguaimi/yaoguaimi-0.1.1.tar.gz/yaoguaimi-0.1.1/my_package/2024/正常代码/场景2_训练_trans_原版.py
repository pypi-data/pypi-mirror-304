...
def init_weights(m):
    if isinstance(m, nn.Linear):
        nn.init.normal_(m.weight, std=1e-6)  # 正态分布初始化
        nn.init.constant_(m.bias, 0.0)  # 常量初始化（零）
    elif isinstance(m, nn.Conv2d):
        nn.init.xavier_uniform_(m.weight)  # Xavier均匀分布初始化
        if m.bias is not None:
            nn.init.constant_(m.bias, 0.0)
    elif isinstance(m, nn.LayerNorm):
        nn.init.constant_(m.weight, 1.0)  # 常量初始化（一）
        nn.init.constant_(m.bias, 0.0)
    elif hasattr(m, 'weight') and m.weight is not None:
        truncated_normal_(m.weight, std=0.02)  # 截断正态分布初始化
