...
class ResBlock(nn.Module):
    def __init__(self, channel_list, H, W):
        ...
        # 增加通道数
        self.channel_list = [max(ch, 128) for ch in channel_list]  # 例如，提高到128
        ...
        self._relu = nn.ReLU()
        self._dropout = nn.Dropout(0.3)  # 调整Dropout比率
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks=15, channel_list=[128, 128, 128]):
        ...
        self._conv_1 = nn.Conv2d(4 * self.streams, self.channel_list[2], kernel_size=3, padding=1)  # 注意这里的变化
        self._conv_2 = nn.Conv2d(self.channel_list[1], self.streams * self.num_bits_per_symbol, kernel_size=3, padding=1)  # 注意这里的变化
