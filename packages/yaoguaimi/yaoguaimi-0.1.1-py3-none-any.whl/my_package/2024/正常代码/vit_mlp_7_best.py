...
class Neural_receiver(nn.Module):
    ...
    def forward(self, y, template_pilot):
        ...
        def add_gaussian_noise(signal, snr_db_min=5, snr_db_max=30):
            """
            向信号添加指定SNR范围内的加性高斯白噪声。           
            参数:
            signal: 输入信号 (PyTorch张量)。
            snr_db_min: SNR的最小值（分贝）。
            snr_db_max: SNR的最大值（分贝）。
            返回:
            带噪声的信号。
            """
            # 随机选择一个SNR值（以dB为单位）
            ...
            return noise     
        noise = add_gaussian_noise(torch.zeros_like(y), 5, 30) 
