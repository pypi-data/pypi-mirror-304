...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=3):
        ...
        # Set up the downsample layer if the in and out channels differ
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks, base_channels, scene):
        ...
        if scene == 'scene1':
            ...
            for l in range(streams):  # Assuming 'streams' corresponds to layers here
                for t in specific_t_V[l + 1]:  # Adjusting for each specific time slot
                    self.V[0, 0, t, :, :] = 1.6  # Apply the special weights for V
        ...
        for block_id in range(self.num_blocks):
            ...
            self.blocks.add_module(
                ...
                module=ResBlock(in_channels, layer_channels)
            )
