...
excluded_model_types = ['NN_TORCH','FASTAI','KNN','XT']


# In[ ]:


predictor = TabularPredictor(label='score',eval_metric='rmse',problem_type='regression').fit(train_data=X ,presets='best_quality',num_bag_folds=5,excluded_model_types=excluded_model_types,num_stack_levels=2,num_bag_sets=3) #,eval_metric='f1' ,hyperparameters='multimodal',eval_metric='roc_auc', num_bag_folds=8,num_stack_levels=3,num_bag_sets=1,num_bag_sets=2, verbosity = 3,num_stack_levels=3,ag_args_fit={'num_cpus': 20},hyperparameters = {'NN_TORCH': {'num_epochs': 500}, },#auto_stack=True,  num_bag_folds=5, num_bag_sets=3, num_stack_levels=3 ,,num_bag_folds=8,num_stack_levels=4,num_bag_sets=5,auto_stack=True,excluded_model_types=excluded_model_types,hyperparameters='multimodal',
...
predictor=TabularPredictor.load("AutogluonModels/ag-20220919_134607/")
#预测数据
...
test_data['value'].to_csv('赛题1_day11_2.csv',index=False)


# In[ ]:


submision_lgb1=pd.DataFrame()
submision_lgb1['value']=pred_test
#submision_lgb1['value'] = submision_lgb1['value']+1
submision_lgb1['value'][submision_lgb1['value'] >8] =10
...
submision_lgb1['value'].to_csv('赛题1_day11_ALL_2.csv',index=False)
