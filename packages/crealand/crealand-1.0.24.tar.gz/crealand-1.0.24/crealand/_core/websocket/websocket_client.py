...
class WebSocketClient:
    def __init__(self, server_url: str, session_id: str, event_list: list):
...
async def ws_connect_async(server_url, session_id):
    global global_ws_client
    event_list = ['crealand-event-oncall-sdk']
    global_ws_client = WebSocketClient(server_url, session_id, event_list)
    # 尝试连接并订阅事件
...
def exit_thread(sig_num, frame):
    sig_name = signal.Signals(sig_num).name
    logger.info(f'Received the signal {sig_name}.')
...
def ws_connect(server_url, session_id):
    try:
        # 注册退出执行程序
        #atexit.register(exit_thread)
        signal.signal(signal.SIGHUP, exit_thread)
        signal.signal(signal.SIGINT, exit_thread)
        signal.signal(signal.SIGTERM, exit_thread)
        # 获取全局事件循环
        loop = init_event_loop()
        loop.run_until_complete(ws_connect_async(server_url, session_id))
        loop.run_forever()
    except Exception as e:
        logger.error(f'An error occurred: {e}')
