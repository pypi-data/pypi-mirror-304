...
from crealand.apis.constants import AxisType, DestType, FilterStyle
...
class Info:
    ...
    def get_id_in_area(area_id: int, config_ids: list[str]) -> list[int]:
        result = call_api(
            'unity',
            "unity.editableTrigger.getEnterTriggerIDs",
            [area_id, config_ids],
        )
        return result
...
class Camera:
    ...
    def move_to(self, time: float, coordinate: list[float], block: bool):
        ...
        call_api(
            'unity',
            "unity.camera.moveTo",
            [self.get_default_id(), new_time, coordinate, 1 if block else 0],
        )
...
class Motion:
    ...
    def ray_ranging(runtime_id: int, attachment_id: int):
        result =call_api(
            ...
            [runtime_id, {'id':attachment_id}, 20],
        )
        print(result,'***********')
        return result
    ...
    def ptz(runtime_id: int, angle: float, block: bool = False):
        ...
        call_api(
            ...
            [runtime_id, angle, abs(angle) / 30, 1 if  block else 0],
        )
    ...
    def action(runtime_id: int, action: str, block: bool = False):
        call_api(
            ...
            [runtime_id, action, 1 if block else 0],
        )
    ...
    def move_by_point(
        ...
        call_api(
            ...
            [runtime_id, new_time, coordinate, 1 if  block else 0],
        )
