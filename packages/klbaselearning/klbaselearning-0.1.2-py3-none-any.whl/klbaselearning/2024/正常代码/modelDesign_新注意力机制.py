...
class AttentionModule(nn.Module):
    def __init__(self, in_channels):
        ...
        # 防止 in_channels 过小导致的问题
        fc_mid_channels = max(in_channels // 16, 1)  # 确保至少为 1
        self.fc = nn.Sequential(
            nn.Linear(8, 4, bias=False),  # 输入特征数从8降到4
            nn.ReLU(inplace=True),
            nn.Linear(4, 8, bias=False),  # 输出特征数从4升回8
            nn.Sigmoid()
)
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avg_pool(x).view(b, c)  # 将特征图降维到 (b, c)
        y = self.fc(y).view(b, c, 1, 1)  # 通过全连接层后还原维度
        return x * y.expand_as(x)  # 应用注意力权重
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=3, dropout_rate=0.2):
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks, base_channels, scene, dropout_rate=0.2):
        ...
        # Initialize the noise reduction (denoising) module
        #self.denoise_module = ResBlock(4 * self.streams, 4 * self.streams, kernel_size=3, dropout_rate=dropout_rate)
        #self.attention_module = AttentionModule(base_channels)  # 假设基础通道数经过一次升维
        ...
        self.final_conv = nn.Conv2d(final_channels, streams * num_bits_per_symbol, kernel_size=3, padding='same')
    ...
    def forward(self, y, template_pilot):
        ...
        z = z.permute(0, 3, 1, 2)
        #print("Feature map size before AttentionModule:", z.size())

        # Apply the denoise module
