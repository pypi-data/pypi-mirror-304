...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=3, dropout_rate=0.5):
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks, base_channels, scene, dropout_rate=0.5):
        ...
        self.extra_conv1 = nn.Conv2d(base_channels, base_channels * 2, kernel_size=3, padding=1)
        self.extra_conv2 = nn.Conv2d(base_channels * 2, base_channels * 4, kernel_size=3, padding=1)
        self.extra_conv3 = nn.Conv2d(base_channels * 4, base_channels * 8, kernel_size=3, padding=1)

        # Final convolutional layer
        final_channels = base_channels * 8  # Adjusted due to extra convolutional layers
    ...
    def forward(self, y, template_pilot):
        ...
        # Apply initial conv and residual blocks
        z = self.init_conv(z)
        z = self.blocks(z)

        # Apply extra convolutional layers
        z = self.extra_conv1(z)
        z = self.extra_conv2(z)
        z = self.extra_conv3(z)

        # Apply final conv and reshape
        z = self.final_conv(z)
        z = z.permute(0, 2, 3, 1)
