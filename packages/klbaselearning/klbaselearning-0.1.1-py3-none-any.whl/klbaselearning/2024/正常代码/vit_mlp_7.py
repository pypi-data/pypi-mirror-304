...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, emb_size, num_heads, ffn_hidden, dropout_rate, max_len=5000):
        super(TransformerEncoderLayer, self).__init__()
        self.attention = nn.MultiheadAttention(emb_size, num_heads, dropout=dropout_rate)
        self.rel_pos_embedding = RelativePositionEmbedding(emb_size, max_len)
    ...
    def forward(self, src):
        ...
        #rel_pos = self.rel_pos_embedding(seq_length).to(src.device)
        src2 = self.norm1(src)
        attn_output, _ = self.attention(src2, src2 + rel_pos, src2)
...
class Neural_receiver(nn.Module):
    ...
    def forward(self, y, template_pilot):
        ...
        noise_variance =0.0316  # 假设噪声方差，需要根据实际情况调整  10db=0.1,15db=0.0316,,5db=0.3162,30db=0.001
        y = y.to(device)
