...
class DataNormalization(nn.Module):
    def __init__(self):
        ...
        # Assume fixed min-max values for normalization, replace with actual values from your dataset
        self.register_buffer('min_val', torch.tensor([-3.0, -3.0]))  # Min for real and imag parts
        self.register_buffer('max_val', torch.tensor([3.0, 3.0]))    # Max for real and imag parts

    def forward(self, x):
        # Normalize to range [-1, 1]
        return -1 + 2 * (x - self.min_val.view(1, 1, 1, 1, -1)) / (self.max_val - self.min_val).view(1, 1, 1, 1, -1)

class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=5, dropout_rate=0.1):
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks, base_channels, scene, dropout_rate=0.1):
        ...
        self.data_norm = DataNormalization()  # Add normalization layer
