...
# 导入数据  
print("Loading Data ... ")  
train_data = pd.read_csv('X_train_transformed_920.csv')
test_data = pd.read_csv('X_test_transformed_920.csv')
...
train_data = train_data.drop(['msisdn','model_id','NLP'], axis=1)
test_data = test_data.drop(['msisdn','model_id','NLP'], axis=1)
...
params = {'num_leaves': 95,
         ...
         'boosting': 'gbdt',
         'bagging_freq':6,
         'bagging_fraction': 0.82,   # 每次迭代时用的数据比例0.8
         'feature_fraction': 0.82,# 每次迭代中随机选择80％的参数来建树0.8201
         'bagging_seed': 11,
...
for fold_n,(trn_index,val_index) in enumerate(kfolds.split(train_selected,target)):
    ...
    num_round=250000
    clf = lgb.train(params, trn_data, num_round, valid_sets = [trn_data, val_data], verbose_eval=1000, early_stopping_rounds = 12000,categorical_feature=cat)
...
best_features.to_csv('best_features_25W.csv',index=False)
...
submision_lgb1['value'].to_csv('G-submision_lgb1_25w.csv',index=False)
...
predictions_lgb_R.to_csv('G-predictions_lgb_R_25w.csv',index=False)
...
submision_lgb1['value'].to_csv('赛题1_Day13_lgbm_1_0.9.3.csv',index=False)
