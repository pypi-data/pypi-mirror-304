...
c0_c8_data = pd.read_csv('new_改进.csv')
...
df3 = new_train_data.drop(['ID','time','c0'], axis=1)


# In[ ]:


df3  = pd.concat([df3, c0_c8_data], axis=0)
...
#con2_2021_all['y'] = con2_2021_all['y'].fillna(method = "bfill").fillna(method = "pad")
...
for num in ScNumberList:   
   ...
   # print(df5.shape)
    ...
    new_m = Prophet(
               ...
               # holidays_prior_scale=10, #调节节假日模型组件的强度。值越大，该节假日对模型的影响越大，值越小，节假日的影响越小，默认值：10.0。
                changepoint_prior_scale=0.25,#增长趋势模型的灵活度。调节”changepoint”选择的灵活度，值越大，选择的”changepoint”越多，从而使模型对历史数据的拟合程度变强，然而也增加了过拟合的风险。默认值：0.05。
                interval_width=0.9,  #衡量未来时间内趋势改变的程度。表示预测未来时使用的趋势间隔出现的频率和幅度与历史数据的相似度，值越大越相似，  
             ...
             #   mcmc_samples=0,          #mcmc采样，用于获得预测未来的不确定性。若大于0，将做mcmc样本的全贝叶斯推理，如果为0，将做最大后验估计，默认值：0。
   ...
   # print(future)
    ...
    forecast = new_m.predict(df1)    
    forecast["y"] = df1["y"].reset_index(drop = True)
    ...
    outlier_df = df1[outlier_index]
    df6 = df1.copy()
   # df6[(df6['y'] <= forecast["yhat_lower"]) | (df6['y'] >= forecast["yhat_upper"]=np.non #具体数值由于上一次迭代的时候没做记录了，但误差应该在0.0001-0.00
    #df6['y'][df6['y'] <= forecast["yhat_lower"] ] =np.nan
    #df6['y'][df6['y']  >= forecast["yhat_upper"] ] =np.nan
    df6=df6.reset_index(drop=True)
    forecast = forecast.reset_index(drop=True)
    df6.loc[(df6['y'] <= forecast["yhat_lower"]) | (df6['y'] >= forecast["yhat_upper"]), "y"]=np.nan
   # df6 = df1.drop(index=outlier_df.index)
   # print([df5.shape])
    outdf=np.sum(outlier_index)
 #   print("异常值的数量为:",np.sum(outlier_index))

    new_df = new_df.append(df6)

    #outdfdel = len(df1)-len(df6)
    outdfdel = df6['y'].isnull().sum()
  ...
  #  print("mae is",all_mean_error,mean_error)
    print("站点-【%s】，异常值-【%s】,已处理-【%s】"%(num,outdf,outdfdel))


# In[ ]:


new_df.to_csv('new_df_处理异常值_缺失_new.csv',index=False)
