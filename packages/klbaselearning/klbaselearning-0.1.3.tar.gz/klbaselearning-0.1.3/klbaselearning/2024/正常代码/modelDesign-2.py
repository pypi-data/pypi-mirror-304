...
class ResBlock(nn.Module):
    def __init__(self, channel_list, H, W):
        ...
        # 调整通道数以保持性能同时减少计算量
        self.channel_list = [max(ch, 32) for ch in channel_list]  # 减少基础通道数
        ...
        self._relu = nn.ReLU()
        self._dropout = nn.Dropout(0.3)  # 减少Dropout比率以保持网络学习能力
...
class Neural_receiver(nn.Module):
    def __init__(self, subcarriers, timesymbols, streams, num_bits_per_symbol, num_blocks=5, channel_list=[32, 32, 32]):  # 减少残差块数量和通道数
