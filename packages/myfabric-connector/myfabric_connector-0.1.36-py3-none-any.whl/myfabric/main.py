...
def main():
    ...
    try:
        asyncio.run(start_proxy(args.moonraker_url, args.printer_key, args.login, args.password, args.moonraker_login,
...
async def start_proxy(moonraker_url, printer_key, login, password, moonraker_login, moonraker_password):
    ...
    proxy_moonraker_reverb(moonraker_ws, channel_name, bearer)
    # Запуск задач
...
def get_moonraker_token(moonraker_url, username, password):
    ...
    response = requests.get(f"http://{moonraker_url}/access/api_key", headers={
        "Authorization": f'Bearer {bearer}'
    })
    data = response.json()
...
async def proxy_moonraker_reverb(moonraker_url, channel_name, bearer):
    ...
    async with websockets.connect(moonraker_url) as moonraker_ws:
        ...
        await asyncio.gather(
            moonraker_reader(),
            moonraker_writer(),
            handle_moonraker_to_reverb(moonraker_to_reverb_queue, reverb_pusher)
        )


async def handle_moonraker_to_reverb(queue, reverb_pusher):
    logger = logging.getLogger('myfabric')
    channel = reverb_pusher.channels.get('your-channel-name')
    while True:
