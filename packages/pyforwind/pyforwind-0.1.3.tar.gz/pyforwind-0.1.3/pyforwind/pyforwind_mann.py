...
class SWF:
    ...
    def mann_spec(self, F, sigma):
        """ Frequency spectrum of Kaimal wind field as specified by IEC 61400-1. """
        #return 4.*sigma**2*self.L**5*(F/self.V_hub)**4/(1.+F**2*self.L**2/self.V_hub)**(17./6.)
        return (1.+(F*self.L/self.V_hub)**2)**(-5./6.)*(11.-5./(1.+(F*self.L/self.V_hub)**2))

    def mann_coh(self, F, R):
        """ Coherences of Kaimal wind field as specified by IEC 61400-1. """
        R[R==0.] = 1.
        chi = R*np.sqrt(1.+(self.L*F/self.V_hub)**2)/self.L
        cross = (chi/2./(1.+(F*self.L/self.V_hub)**2))**(5./6.)*(11.*kv(5./6., chi)/3.- chi*kv(11./6., chi)/(1.+(F*self.L/self.V_hub)**2))
        coh = cross/self.mann_spec(F, 0.1*self.V_hub)
        return coh
        #return (chi/2./(1.+(F*self.L/self.V_hub)**2))**(5./6.)*(kv(5./6., chi)- chi*kv(1./6., chi)/2.)

    def cov(self, L, sigma):
        """ Temporal correlations calculated from inverse Fourier transform of Kaimal spectrum. """
        mann_spec_vec = np.vectorize(self.mann_spec)
        return np.fft.irfft(mann_spec_vec(self.f, L, sigma))[:self.N_x//2]
    ...
    def gauss_field(self, L, sigma):
        ...
        for ff in range(1, self.N_x//2+1):
            coh_decomp = np.linalg.cholesky(self.mann_coh(self.f[ff], self.R_ij))
            u_hat[:,ff] = coh_decomp*np.sqrt(self.mann_spec(self.f[ff], sigma))@random_phases[:, ff]
