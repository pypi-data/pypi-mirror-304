...
class CollectionMixin(object):
    ...
    def rights_holder(self):
        _rights_holder = self.collection["rights_holder"]
        results = []
        for item in _rights_holder:
    ...
    @property
    def collector(self):
        _collector = self.collection["collector"]
        results = []
    ...
    def basic_classifications(self, value=None):
        if value:
            self.collection["basic_classifications"] = value
        return (
            self.collection["basic_classifications"]
            if "basic_classifications" in self.collection
            and isinstance(self.collection["basic_classifications"], list)
            else []
        )

    def gnd_subjects(self, value=None):
        if value:
            self.collection["gnd_subjects"] = value
        return (
            self.collection["gnd_subjects"]
            if "gnd_subjects" in self.collection
            and isinstance(self.collection["gnd_subjects"], list)
            else []
        )
...
class CollectionConfig(YAMLConfigBase, CollectionMixin):
    ...
    def content(self):
        ...
        if _content:
            for key in ["rights_holder", "collector"]:
                if _content["collection"][key] is None:
                    _content["collection"][key] = []
        return _content
...
class CollectionConfigTemplate(YAMLConfigTemplate, CollectionMixin):
    ...
    def render(self, overwrite=False):
        ...
        if cc.content and cc.basic_classifications():
            self.basic_classifications(value=cc.basic_classifications())
        else:
            # add an empty set to give a hint on how fill it out
            self.basic_classifications(
