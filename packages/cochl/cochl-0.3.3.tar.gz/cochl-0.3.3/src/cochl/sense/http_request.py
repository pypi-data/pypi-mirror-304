...
class HttpResponse:
    ...
    def json(self) -> Optional[Union[dict, list]]:
        ...
        try:
            ...
            raise RequestException(str(err))
...
class HttpRequest:
    ...
    def _get_content_type(file_path: str):
        if not os.path.isfile(file_path):
            raise RequestException('not a file')

        if os.stat(file_path).st_size == 0:
            raise RequestException('an empty file')
    ...
    def _get_or_delete(method: str, url: str, params: Optional[dict] = None, headers: Optional[dict] = None,
        ...
        try:
            ...
            raise RequestException(str(get_addr_info_err))

        except ConnectionRefusedError as conn_refused_err:
            raise RequestException(str(conn_refused_err))

        except TimeoutError as timeout_err:
            raise RequestException(str(timeout_err))

        except ssl.SSLError as ssl_err:
            raise RequestException(str(ssl_err))

        except HTTPException as http_exc:
            raise RequestException(str(http_exc))
    ...
    def put(url: str, data=None, headers: Optional[dict] = None, json: Optional[Union[dict, list]] = None,
        ...
        if isinstance(json, dict) or isinstance(json, list):
            ...
            raise RequestException('data is required')
