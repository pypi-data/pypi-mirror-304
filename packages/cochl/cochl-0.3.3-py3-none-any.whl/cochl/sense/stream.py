...
class StreamAudioType:
    ...
    def get_bytes_per_sec(self) -> int:
        """
        Returns the number of bytes composing 1 second.
        For example, 22,050Hz & f32 combination represents 1 second as 22,050 * 4 = 88,200 bytes.
        """
        return self.sample_rate * self.data_type.byte_size
...
class StreamBuffer:
    ...
    def __init__(self, audio_type: StreamAudioType, session: CreatedSession):
        self.audio_type = audio_type

        bytes_per_sec = audio_type.get_bytes_per_sec()
        self.window_byte_size = int(session.window_size * bytes_per_sec)
        self.window_hop_byte_size = int(session.window_hop * bytes_per_sec)  # number of bytes to hop between windows
...
class StreamClient:
    ...
    def get_buffer(self) -> StreamBuffer:
        return StreamBuffer(self.audio_type, self.session)
