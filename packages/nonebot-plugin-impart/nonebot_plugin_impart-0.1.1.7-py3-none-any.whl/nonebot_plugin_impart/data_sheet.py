...
engine: Engine = create_engine(f"sqlite:///{DATA_PATH}/impart.db")
...
def get_win_probability(userid: int) -> float:
    """传入用户id, 返还数据库中对应的获胜概率"""
    with session() as s:
        return s.query(UserData).filter(UserData.userid == userid).first().win_probability  # type: ignore


def set_win_probability(userid: int, probability_change: float) -> None:
    """传入一个用户id以及需要增加的获胜率, 在数据库内累加, 用这个函数前一定要先判断用户是否在表中"""
    with session() as s:
        current_probability = s.query(UserData).filter(UserData.userid == userid).first().win_probability  # type: ignore
        s.query(UserData).filter(UserData.userid == userid).update(
            {
                UserData.win_probability: round(current_probability + probability_change, 3),
                UserData.last_masturbation_time: int(time.time()),
            }
        )
        s.commit()
