            ...
            if self.ws_connected:
                ...
                for pending_event_id in cur_pending_event_ids:
                    if not self.events[pending_event_id].check_removed():
                        logger.info(f"{self.LOG_TAG}try connect event:{pending_event_id}")
...
class TickEvent:
    ...
    def get_subscribe_dump_data(self):
        logger.info(f"{self.LOG_TAG} subscribe {self.event_id}, {self.ticker}, {self.price}, {self.price_comparison}")
