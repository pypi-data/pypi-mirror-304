...
class YouQuHtmlRpc:
    ...
    def gen_rpc(cls, allure_data_path, allure_html_path, local_ip, report_base_path, dirname):
        ...
        html_a_tag = f"<a href='{report_server_url}' target='_blank'> Click here to view the YouQu test report </a>"
        logger.info(f"html report url: {report_server_url}")
        with open(f"{allure_html_path}/{report_dirname}.html", "w", encoding="utf-8") as f:
            f.write(html_a_tag)
