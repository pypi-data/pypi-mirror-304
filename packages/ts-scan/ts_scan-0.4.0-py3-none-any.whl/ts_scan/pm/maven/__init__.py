...
class MavenScanner(Scanner):
    def __init__(self, excludeDepTypes: t.Optional[str] = None, forward: t.Optional[tuple] = None, **kwargs):
        super().__init__(**kwargs)

        self.__excludeDepTypes = excludeDepTypes.split(',') if excludeDepTypes else ['test']
        self.__forward = [arg for fwd in forward for arg in fwd.split(',')]
    ...
    @staticmethod
    def options() -> Scanner.OptionsType:
        return {
            'excludeDepTypes': {
                'type': str,
                'required': False,
            ...
            'forward': {
                ...
                'required': False,
                'multiple': True,
                'help': 'Forward parameters to the Maven command'
            }
        }
    ...
    def _execute(self, path: Path) -> t.Optional[DependencyScan]:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            if nodes := Tree.from_maven_file(tree_file):
                ...
                scan = MavenScan(module=name,
                                 moduleId=f'{groupId}:{artifactId}' + f':{version}' if version else '',
                                 deps=deps)
    ...
    def _mvn(self, *args, **kwargs) -> subprocess.CompletedProcess:
        return subprocess.run(
            ['mvn', '-f', self.__path] + list(args) + self.__forward,
