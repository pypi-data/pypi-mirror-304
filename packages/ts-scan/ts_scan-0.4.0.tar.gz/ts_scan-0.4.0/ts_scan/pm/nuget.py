...
class NugetScan(DependencyScan):
    ...
    def _process_with_lock_file(self, project_file: Path, depth: int = 0) -> t.List[Dependency]:
        cmd = self.__nuget_command + ["restore", str(project_file), "-UseLockFile"]
        _ = subprocess.check_output(cmd, cwd=self.__path)
    ...
    def _find_global_packages_dir(self) -> Path:
        cmd = self.__nuget_command + ["locals", "global-packages", "-list"]
        result = subprocess.check_output(cmd, cwd=self.__path).decode("utf-8")
    ...
    @staticmethod
    def _determine_nuget_command() -> t.List['str']:
        try:
            cmd = ["nuget"]
            _ = subprocess.check_output(cmd)

        except FileNotFoundError:
            cmd = ["mono", "/usr/local/bin/nuget.exe"]
            output = subprocess.check_output(cmd).decode("utf-8")

            if "No such file or directory" in output:
                raise FileNotFoundError(
                    "Detected mono, but did not find nuget.exe at expected location /usr/local/bin/nuget.exe")
