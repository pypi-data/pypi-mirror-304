...
class FrenchNumeric:
    ...
    RelativeOrdinalRegex = f'(?<relativeOrdinal>prochain[es]?|pr[eé]c[eé]dent[es]?|(l[’\'])?actuel(le)?(\\s+une?)?|(l[’\'])?avant(\\s+|-)derniere?|(ant[eé])?p[eé]nulti[eè]me|derni[eè]r[es]?|suivant[es]?|courant[es]?|cel(le|ui)\\s+d[\'’]avant\\s+l[ae]\\s+derni[èe]re?)'
    ...
    EqualRegex = f'(([ée]ga(l(es)?|ux)|au\\s+nombre)(\\s+([àa]|d[e\'’]))?|(?<!<|>)=)'
    ...
    MoreOrEqualSuffix = f'((et|ou)\\s+(((more|greater|higher|plus(\\s+grand)?|sup[ée]rieure?s?)((?!\\s+([àa]|que))|(\\s+([àa]|que)(?!((\\s+ou\\s+[èe]ga(l(es)?|ux)\\s+[àa])?\\s*\\d+)))))|((a plus|au-dessus)\\s+d[e\'’](?!\\s+than))))'
    ...
    LessOrEqualSuffix = f'((et|ou)\\s+(less|lower|plus petit|moins|inf[ée]rieure?s?)((?!\\s+([àa]|de|que))|(\\s+([àa]|d[e\'’]|que)(?!(\\s*\\d+)))))'
    NumberSplitMark = f'(?![,.](?!\\d+))(?!\\s*\\b(et\\s+({LessRegex}|{MoreRegex})|mais|ou|to)\\b)'
    MoreRegexNoNumberSucceed = f'((bigger|greater|more|plus(\\s+grand)?|sup[ée]rieure?s?)((?!\\s+([àa]|que))|\\s+(([àa]|que)(?!(\\s*\\d+))))|((au-dessus|a plus)\\s+d[e\'’])(?!(\\s*\\d+)))'
    LessRegexNoNumberSucceed = f'((less|lower|plus petit|moins|inf[ée]rieure?s?)((?!\\s+([àa]|d[e\'’]|que))|\\s+(([àa]|d[e\'’]|que)(?!(\\s*\\d+))))|(((en )?dessous)\\s+d[e\'’]|under)(?!(\\s*\\d+)))'
    EqualRegexNoNumberSucceed = f'([èe]ga(l(es)?|ux)((?!\\s+([àa]))|(\\s+([àa]|que)(?!(\\s*\\d+)))))'
