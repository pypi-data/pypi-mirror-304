...
class EventStore:
    ...
    def add_event(self, system_id: str, event: Event):
        ...
        logger.debug(f"Attempting to add event for system {system_id}")
        
        with self._lock:
            logger.debug("Lock acquired")
            system_trace = self.get_or_create_system_trace(system_id)
            logger.debug(f"Got system trace for {system_id}")
            ...
            logger.debug(f"Found partition: {current_partition is not None}")

            if current_partition is None:
                logger.error(f"No partition found for index {event.partition_index}")
                raise ValueError(
            ...
            logger.debug("Event appended to partition")

            # Create a span for the event
            logger.debug("About to create span")
            with tracer.start_as_current_span(event.event_type) as span:
                ...
                logger.debug("Span attributes set")

                for step in event.agent_compute_steps:
                    logger.debug(f"Adding compute step {step.event_order}")
                    span.add_event(
                        "agent_compute",
                        {
