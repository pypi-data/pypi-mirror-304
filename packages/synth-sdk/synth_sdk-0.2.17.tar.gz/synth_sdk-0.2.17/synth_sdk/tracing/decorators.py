                ...
                if log_vars_output:
                    if isinstance(result, dict):
                        for var in log_vars_output:
                            ...
                            elif frame_after and var in local_vars:
                                compute_output[var] = local_vars[var]
                            else:
                                if verbose:
                                    logger.error(f"Variable '{var}' specified in log_vars_output not found in result or local scope")
                                raise ValueError(f"Variable '{var}' specified in log_vars_output not found in result or local scope")
                    elif isinstance(result, (tuple, list)):
                        if len(log_vars_output) > len(result):
                            # Attempt to get remaining variables from local scope
                        ...
                        # Single return value - try to match it with any of the requested output variables
                        for output_var in log_vars_output:
                            if output_var == 'result':
                                compute_output[output_var] = result
                            elif output_var in local_vars:
                                compute_output[output_var] = local_vars[output_var]
                            else:
                                # If the variable isn't found in locals, assume it's meant to capture the return value
                                compute_output[output_var] = result
