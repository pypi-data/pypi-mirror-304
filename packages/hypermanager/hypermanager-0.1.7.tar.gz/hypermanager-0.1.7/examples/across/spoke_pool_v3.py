    ...
    for client, spoke_pool_address in client_config.items():
        ...
        for base_event_config in across_config:
            ...
            try:
                ...
                if df.is_empty():
                    print(f"No events found for {base_event_config.name} on {
                ...
                print(f"Events found for {
                      base_event_config.name} on {client.name}:")
                ...
                df.write_parquet(
                    f"{folder_path}/{base_event_config.name}_{client.name}.parquet"
                )
