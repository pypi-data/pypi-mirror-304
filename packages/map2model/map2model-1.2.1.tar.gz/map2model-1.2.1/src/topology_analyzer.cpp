...
namespace ConverterLib
{
  ...
  TopologyAnalyzer::TopologyAnalyzer(AABB window) : window(window) {}
  //=======================================================================================
  ...
  void TopologyAnalyzer::Initialize(const Parameters &par)
  {
    ...
    converter.ReadData(par.path_geology, "POLYGON", par.constNames,
                       polygonsIdsToRead);
    converter.ReadData(par.path_faults, "LINESTRING", par.constNames,
                       faultsIdsToRead);
    if (par.path_points != "")
    {
      converter.ReadData(par.path_points, "POINT", par.constNames,
                         pointsIdsToRead, pointsIdsToReadString);
    }

    // Initialize the clipping window.
    if (par.clipping_window[0] == par.clipping_window[2] ||
        par.clipping_window[1] ==
            par.clipping_window[3])
    ...
    {
      window = AABB(par.clipping_window[0], par.clipping_window[1],
                    par.clipping_window[2], par.clipping_window[3]);
    }
  ...
  void TopologyAnalyzer::AnalyzeLocalTopology(const Parameters &par)
  {
    ...
    int Nx = (int)((window.BtmRight.X - window.TopLeft.X) /
                   ConverterUtils::CoordToInteger(par.subregion_size_x));
    int Ny = (int)((window.BtmRight.Y - window.TopLeft.Y) /
                   ConverterUtils::CoordToInteger(par.subregion_size_y));
  ...
  void TopologyAnalyzer::WriteUnitFaultIntersectionList(
      ...
      const UnitFaultIntersectionList &unitFaultIntersectionList)
  {
    ...
    size_t counter = 0;
    for (UnitFaultIntersectionList::const_iterator it =
             unitFaultIntersectionList.begin();
    ...
    file.close();
    std::cout << "Wrote " << counter
              << " items to unit-fault intersection list: " << fileName
              << std::endl;
  }
  //=======================================================================================

  void TopologyAnalyzer::WriteFaultIntersectionList(
      const std::string &fileName,
      const FaultIntersectionList &faultIntersectionList)
  {
         ...
         it != faultIntersectionList.end(); ++it)
    {
      ...
      for (std::list<FaultIntersectionData>::const_iterator faultDataIt =
               faultDataList.begin();
           faultDataIt != faultDataList.end(); ++faultDataIt)
      {
        ...
        file << "(" << faultDataIt->first << ", " << faultDataIt->second.first
             << ", " << faultDataIt->second.second << ")";
      }
      file << "}" << std::endl;
      counter++;
    }
    file.close();
    std::cout << "Wrote " << counter
              << " items to fault-fault intersection list: " << fileName
              << std::endl;
  }
  //=======================================================================================

  void TopologyAnalyzer::WritePointPolygonIntersectionList(
      const std::string &fileName,
      const PointPolygonIntersectionList &pointPolygonIntersectionList)
  {
    ...
    // Adding a title.
    file << std::setprecision(16) << "#ID,SiteCode,SiteCommo,"
    ...
    size_t counter = 0;
    for (PointPolygonIntersectionList::const_iterator it =
             pointPolygonIntersectionList.begin();
         it != pointPolygonIntersectionList.end(); ++it)
    {
      ...
      std::string::iterator end_pos =
          std::remove(commodity.begin(), commodity.end(), ' ');
      commodity.erase(end_pos, commodity.end());

      // Write a row.
      file << std::setprecision(16) << std::defaultfloat << it->point->id << ","
           << it->point->site_code << "," << commodity << "," << posX << ","
           << posY << "," << std::setprecision(2) << std::fixed
           << sqrt(it->distanceToContact2) << "," << it->containingPolygon->id
           << "," << it->neighbourPolygon->id << ","
           << it->containingPolygon->code << "," << it->neighbourPolygon->code
           << "," << contactType << "," << faultID << std::endl;
    ...
    file.close();
    std::cout << "Wrote " << counter
              << " items to point polygon intersection list: " << fileName
              << std::endl;
  }

  void TopologyAnalyzer::AnalyzeGlobalTopology(
      const Parameters &par, const std::string &graph_comments)
  {
    ...
    //------------------------------------------------------------------
    {
      ...
      IntersectFaultsWithPolygons(converter.GetFaults(), converter.GetPolygons(),
                                  unitFaultIntersectionList);
    ...
    converter.IdentifyPolygonContactTypes(contacts, par.angleEpsilon,
                                          par.distanceEpsilon);
    converter.SplitMixedPolygonContacts(contacts);

    for (Contacts::const_iterator it = contacts.begin(); it != contacts.end();
         ++it)
    {
      if (it->type == FaultContact)
        fault_contacts.push_back(*it);
      if (it->type == StratigraphicContact)
        stratigraphic_contacts.push_back(*it);
    }
    converter.WriteImage("polygons_with_fault_contacts", windows, 0,
                         &fault_contacts);
    converter.WriteImage("polygons_with_stratigraphic_contacts", windows, 0,
                         &stratigraphic_contacts);
    ...
    IntersectPointsWithPolygons(pointPolygonIntersectionList,
                                converter.GetPoints(), converter.GetPolygons());

    // Finding the closest point contacts.
    FindClosestContactForPoints(pointPolygonIntersectionList, contacts,
                                converter.GetFaults(), converter,
                                par.pointToContactDistanceBuffer);

    std::string fileNamePointPolygon =
        par.path_output + "/point-polygon-intersection.txt";
    WritePointPolygonIntersectionList(fileNamePointPolygon,
                                      pointPolygonIntersectionList);
    ...
    converter.AddPointsToUnitContacts(unit_contacts,
                                      pointPolygonIntersectionList);
    ...
    for (size_t i = 0; i < par.depositListForGraphInfo.size(); i++)
    {
      ...
      // ALL contacts. -----------------------------------------------
      std::string file_graph_all =
          par.path_output + "/graph_all_" + depositName + ".gml";
      graph.WriteGraph(file_graph_all, unit_contacts, "UNIT_NAME", false, 1,
                       par.graph_edge_width_categories,
                       par.graph_edge_direction_type, depositName,
                       graph_comments);

      // Stratigraphic unit contacts. --------------------------------
      UnitContacts unit_contacts_strat = Converter::FilterUnitStratFaultContacts(
          unit_contacts, StratigraphicContact, par.minFractionInMixedContact);

      std::string file_graph_strat =
          par.path_output + "/graph_strat_" + depositName + ".gml";
      graph.WriteGraph(file_graph_strat, unit_contacts_strat, "UNIT_NAME", false,
                       1, par.graph_edge_width_categories,
                       par.graph_edge_direction_type, depositName,
                       graph_comments);

      // Fault unit contacts. ----------------------------------------
      UnitContacts unit_contacts_fault = Converter::FilterUnitStratFaultContacts(
          unit_contacts, FaultContact, par.minFractionInMixedContact);

      std::string file_graph_fault =
          par.path_output + "/graph_fault_" + depositName + ".gml";
      graph.WriteGraph(file_graph_fault, unit_contacts_fault, "UNIT_NAME", false,
                       1, par.graph_edge_width_categories,
                       par.graph_edge_direction_type, depositName,
                       graph_comments);

      // Igneous unit contacts. -------------------------------------
      UnitContacts unit_contacts_ingeous =
          Converter::FilterIgneousUnitContacts(unit_contacts);

      std::string file_graph_ign =
          par.path_output + "/graph_igneous_" + depositName + ".gml";
      graph.WriteGraph(file_graph_ign, unit_contacts_ingeous, "UNIT_NAME", false,
                       1, par.graph_edge_width_categories,
                       par.graph_edge_direction_type, depositName,
                       graph_comments);
    }
    ...
    //----------------------------------------------------------------------------------------
    {
      ...
      converter.FilterContactsByPolygon(contacts, contacts_filtered,
                                        par.partial_graph_polygon_id,
                                        par.partial_graph_depth);
      ...
      graph.WriteGraph(file_graph, unit_contacts, "UNIT_NAME", false, 1,
                       par.graph_edge_width_categories,
                       par.graph_edge_direction_type, graph_comments);
    }
  }
