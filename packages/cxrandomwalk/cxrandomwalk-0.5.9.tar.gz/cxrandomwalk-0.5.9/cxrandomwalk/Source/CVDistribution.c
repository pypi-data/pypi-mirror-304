...
void CVUpdateDistribution(CVDistribution *currentDistribution, CVFloat *probabilities, const CVFloat *data, CVSize count)
{
	if (currentDistribution->count < count) {
		currentDistribution->distribution = realloc(currentDistribution->distribution, count * sizeof(CVFloat));
		currentDistribution->cumulative = realloc(currentDistribution->cumulative, count * sizeof(double));
		if (data) {
			currentDistribution->data = realloc(currentDistribution->data, count * sizeof(CVFloat));
		}
		currentDistribution->tree = realloc(currentDistribution->tree, (count - 1) * sizeof(double));
	}

	currentDistribution->count = count;

	memcpy(currentDistribution->distribution, probabilities, count * sizeof(CVFloat));

	if (data) {
		memcpy(currentDistribution->data, data, count * sizeof(CVFloat));
	}

	double cumValue = 0.0;
	for (CVIndex i = 0; i < count; i++) {
		cumValue += currentDistribution->distribution[i];
		currentDistribution->cumulative[i] = cumValue;
	}

	for (CVIndex i = 0; i < count; i++) {
		currentDistribution->distribution[i] /= cumValue;
		currentDistribution->cumulative[i] /= cumValue;
	}
	...
	currentDistribution->tree = realloc(currentDistribution->tree, (n - 1) * sizeof(double));
	...
	for (m = 0; m < n - 1; m++) {
		...
		CVInteger Kij = im * ipow2(jm + 1) + ipow2(jm) - 1;
		if (Kij < count) {
			currentDistribution->tree[m] = currentDistribution->cumulative[Kij];
		}
		else {
			currentDistribution->tree[m] = 1.0;
		}
	}
}
