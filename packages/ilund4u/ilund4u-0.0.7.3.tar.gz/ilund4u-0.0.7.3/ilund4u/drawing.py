...
class DrawingManager:
    ...
    def plot_hotspots(self, hotspot_ids: list, output_folder: str = "default",
        ...
        try:
            ...
            if keep_temp_data:
                if not os.path.exists(os.path.join(output_folder, "lovis4u_tmp_dirs")):
                    os.mkdir(os.path.join(output_folder, "lovis4u_tmp_dirs"))
                os.system(f"mv {l_parameters.args['output_dir']} "
                          f"{os.path.join(output_folder, 'lovis4u_tmp_dirs', pdf_name)}")
                os.mkdir(os.path.join(output_folder, "lovis4u_tmp_dirs", pdf_name, "gff_files"))
                for gff_file in gff_files:
                    os.system(f"cp {gff_file} {os.path.join(output_folder, 'lovis4u_tmp_dirs', pdf_name, 'gff_files')}")
            else:
