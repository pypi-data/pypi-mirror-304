            ...
            for extentity in func(self, elem):
                if extentity.etype == etype:
                    for citation in self.build_citation(elem):
                        extentity.values.setdefault("has_citation", set()).add(
                            citation.extid
                        )
                        yield citation
                yield extentity
...
class EACCPFImporter:
    ...
    def build_structure(self, elem):
        ...
        desc, desc_format = self.parse_tag_content(elem)
        if desc:
            values = {"description": {desc}, "description_format": {desc_format}}
    ...
    def build_generalcontext(self, elem):
        ...
        if content:
            values = {"content": {content}, "content_format": {content_format}}
    ...
    def parse_relations(self, cpf_description):
        ...
        for cpfrel in self._elem_findall(relations, "eac:cpfRelation"):
            yield from self.build_relation(cpfrel)
        # resourceRelation.
        for rrel in self._elem_findall(relations, "eac:resourceRelation"):
            yield from self.build_resource_relation(rrel)
    ...
    def parse_control(self, control):
        ...
        if record_id is not None and record_id.text and record_id.text.strip():
            ...
            self.record.values["record_id"] = {to_unicode(record_id.text)}
            self.record_visited(record_id, self.record)
        else:
            self.record.extid = self._gen_extid()
            self.import_log.record_warning(
                self._("found no recordId element in control tag, using %s as cwuri")
                % self.record.extid,
                line=control.sourceline,
            )
        for other_record_id in self._elem_findall(control, "eac:otherRecordId"):
            ...
            if other_id:
                ...
                yield extentity
        for elem in control.xpath("eac:sources/eac:source", namespaces=self.namespaces):
            for extentity in self.build_source(elem):
                yield extentity
        for elem in control.xpath(
            "eac:maintenanceHistory/eac:maintenanceEvent", namespaces=self.namespaces
        ):
    ...
    def parse_event_type(self, elem):
        ...
        if event_type:
            ...
            try:
                ...
                self.import_log.record_warning(
                    self._(
                        "eventType %s does not match the PROV-O vocabulary, respective Activity will "  # noqa
