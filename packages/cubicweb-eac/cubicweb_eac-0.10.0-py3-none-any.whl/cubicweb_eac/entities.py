...
class EACResourceRelation(AnyEntity):
    ...
    fetch_attrs, cw_fetch_order = fetch_config(
        ("agent_role", "resource_role", "description")
    )
...
def substitute_xml_prefix(prefix_name, namespaces):
    ...
    try:
        ...
        return prefix_name
    assert prefix in namespaces, "Unknown namespace prefix: {}".format(prefix)
...
class AuthorityRecordEACAdapter(AbstractXmlAdapter):
    ...
    def place_element(self, place):
        ...
        for attr, eac_name in [("role", "placeRole"), ("name", "placeEntry")]:
            ...
            if (
                eac_elt is not None
                and eac_name == "placeEntry"
                and place.equivalent_concept
            ):
                eac_elt.attrib["vocabularySource"] = self.cwuri_url(
                    place.equivalent_concept[0]
                )
