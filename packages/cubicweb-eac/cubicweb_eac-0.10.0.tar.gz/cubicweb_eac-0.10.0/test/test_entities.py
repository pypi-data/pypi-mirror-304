...
class EACExportTC(WebCWTC):
    """Unit tests for EAC-CPF exports."""

    def test_richstring_plain(self):
        with self.admin_access.cnx() as cnx:
            alice = testutils.authority_record(cnx, "Alice")
            desc = "ding\nlikes rabbits"
    ...
    def test_richstring_html_simple(self):
        ...
        self.assertIn(desc, etree.tostring(tag, encoding=str))
    ...
    def test_richstring_html_multiple_elements(self):
        ...
        self.assertIn(etree.tostring(h1, encoding=str), desc[0])
        self.assertIn(etree.tostring(a, encoding=str), desc[1])
    ...
    def test_richstring_markdown(self):
        ...
        self.assertIn(desc_html, etree.tostring(tag, encoding=str))
    ...
    def test_richstring_rest(self):
        ...
        self.assertIn(desc_html, etree.tostring(ptag, encoding=str))
    ...
    def test_richstring_non_none_non_html(self):
        with self.admin_access.cnx() as cnx:
            arecord = testutils.authority_record(cnx, "R")
            mandate = cnx.create_entity(
                "Mandate",
                term="w",
                description=" ",
