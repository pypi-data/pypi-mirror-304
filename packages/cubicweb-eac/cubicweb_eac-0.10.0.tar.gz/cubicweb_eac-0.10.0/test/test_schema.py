...
class SchemaConstraintsTC(WebCWTC):
    ...
    def test_postaladdress_no_required(self):
        ...
        with self.admin_access.cnx() as cnx:
            ar = testutils.authority_record(cnx, "test")
            cnx.create_entity(
                "AgentPlace",
                place_agent=ar,
        ...
        with self.admin_access.cnx() as cnx:
            with self.assertValidationError(cnx) as cm:
                testutils.authority_record(
                    cnx, "Arthur", start_date=date(524, 2, 9), end_date=date(500, 7, 12)
                )
    ...
    def test_on_update_set_end_date_before_start_date(self):
        ...
        with self.admin_access.cnx() as cnx:
            agent = testutils.authority_record(
                cnx, "Arthur", start_date=date(454, 2, 9), end_date=date(475, 4, 12)
            )
            cnx.commit()
            with self.assertValidationError(cnx) as cm:
    ...
    def test_on_update_set_start_date_after_end_date(self):
        ...
        """
        with self.admin_access.cnx() as cnx:
            agent = testutils.authority_record(cnx, "Arthur", end_date=date(476, 2, 9))
            cnx.commit()
            with self.assertValidationError(cnx) as cm:
...
class AuthorityRecordGraphTC(WebCWTC):
    ...
    def test_graph_structure(self):
        ...
        expected = {
            ...
            "NameEntry": {("name_entry_for", "subject"): {"AuthorityRecord"}},
...
class SecurityTC(WebCWTC):
    ...
    def test_agent_kind_relation(self):
        ...
        with self.admin_access.cnx() as cnx:
            record = testutils.authority_record(cnx, "bob", kind="unknown-agent-kind")
            cnx.commit()
    ...
    def test_authority_record_base(self):
        with self.admin_access.cnx() as cnx:
            ...
            testutils.authority_record(
                cnx, "admin record", reverse_function_agent=function
            )
            cnx.commit()
        ...
        with self.new_access("toto").cnx() as cnx:
            ...
            record = testutils.authority_record(
                cnx, "bob", reverse_function_agent=function
            )
            cnx.commit()
