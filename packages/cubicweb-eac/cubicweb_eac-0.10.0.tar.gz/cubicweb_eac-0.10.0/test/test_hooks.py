...
class TernaryRelationDeletionHookTC(WebCWTC):

    def setup_database(self):
        with self.admin_access.cnx() as cnx:
            self.arecord1_eid = testutils.authority_record(cnx, "alice").eid
            self.arecord2_eid = testutils.authority_record(cnx, "bob").eid
            self.arecord3_eid = testutils.authority_record(cnx, "charles").eid
            cnx.create_entity(
                "AssociationRelation",
                entry="alice and bob are friends",
                association_from=self.arecord1_eid,
                association_to=self.arecord2_eid,
            )
            cnx.create_entity(
                "AssociationRelation",
                entry="bob knows charles",
                association_from=self.arecord2_eid,
                association_to=self.arecord3_eid,
            )
            cnx.create_entity(
                "ChronologicalRelation",
                entry="alice is bob's mother",
