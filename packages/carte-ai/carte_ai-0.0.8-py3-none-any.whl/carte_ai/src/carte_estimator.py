...
class BaseCARTEEstimator(BaseEstimator):
    ...
    def _run_step(self, model, data, optimizer):
        ...
        if self.output_dim_ == 1:
            out = out.view(-1).to(torch.float32)  # Reshape output and convert to float32
            target = target.to(torch.float32)  # Convert target to float32
    ...
    def _eval(self, model, ds_eval):
        ...
        with torch.no_grad():
            ...
            target = ds_eval.y
            if self.output_dim_ == 1:
                out = out.view(-1).to(torch.float32)  # Convert output to float32
                target = target.to(torch.float32)  # Convert target to float32
