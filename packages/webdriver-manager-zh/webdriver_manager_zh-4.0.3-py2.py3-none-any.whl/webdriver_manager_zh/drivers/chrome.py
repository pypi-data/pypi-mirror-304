...
from webdriver_manager_zh.core.driver import Driver
...
class ChromeDriver(Driver):
    ...
    def get_latest_release_version(self):
        ...
        if determined_browser_version is not None and version.parse(determined_browser_version) >= version.parse("115"):
            # _latest_release_url: url/latest-patch-versions-per-build.json
            response = self._http_client.get(self._latest_release_url, headers=self.auth_header)
    ...
    def get_url_for_version_and_platform(self, browser_version, platform):
        # _url: url/known-good-versions-with-downloads.json
        response = self._http_client.get(self._url, headers=self.auth_header)
        data = response.json()
            ...
            if compatible_versions:
                ...
                for d in downloads:
                    if d["platform"] == platform:
                        ...
                        # 替换为阿里的url
                        return d["url"].replace('https://storage.googleapis.com/chrome-for-testing-public',
