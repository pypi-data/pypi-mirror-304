...
class TimeHistory:
    ...
    def to_dataframe(self):
        ...
        for i in range(dataset.shape[0]):
            dataset.iloc[i, :len(self.header)] = self.header
            dataset.iloc[i, len(self.header):3 + len(self.header)] = [self.times_past_midnight[i],
                                                                               self.overall_level[i],
                                                                               self.overall_a_weighted_level[i]]
            dataset.iloc[i, len(self.header) + 3:] = self.spectra[i].pressures_decibels
