...
class Controller():
        ...
        for extension in data_resolve['extensions']:
            ...
            if version['service'] == "pypi":
                ...
                if (
                    ...
                    and 'main' not in extension["category"]
                ):
    ...
    def fetch_python_versions(self):
        ...
        if platform.system() == "Windows":
            ...
            for p in list_paths:
                path = p.split(' ')[-1]
                # Make sure the python executable exists (which is not
                # garanteed by call to `py --list-paths`).
                if path and os.path.isfile(path):
                    output = subprocess.check_output(f"{path} --version").decode()
                    version_number = re.findall(r'(?:(\d+\.(?:\d+\.)*\d+))', output)
                    # Special test needed when python 2.7 is installed on the system.
                    # For some reason the process stdout remains empty, while it is
            ...
            for path in list_paths:
                ...
                try:
                    ...
                    if proc.communicate()[1].decode() == "":
                        version_number = re.findall(r'(?:(\d+\.(?:\d+\.)*\d+))', proc.communicate()[0].decode())
                    else:
                        version_number = re.findall(r'(?:(\d+\.(?:\d+\.)*\d+))', proc.communicate()[1].decode())
                    # Special test needed when python 2.7 is installed on the system.
                    # For some reason, the process stdout remains empty.
                    if not version_number:
                        continue
