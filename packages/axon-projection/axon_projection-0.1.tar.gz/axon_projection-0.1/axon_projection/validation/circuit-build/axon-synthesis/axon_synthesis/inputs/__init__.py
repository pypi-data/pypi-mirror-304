...
class Inputs(BasePathBuilder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tuft_number: dict | None = None
    ...
    def load_tuft_params_and_distrs(self, **kwargs):
        ...
        self.update_from_dict("tuft_number_file", "POPULATION_TUFT_NUMBER_FILENAME", kwargs)
        with Path(self.POPULATION_TUFT_NUMBER_FILENAME).open(mode="r", encoding="utf-8") as f:
            number = pd.DataFrame(json.load(f))
        # TODO validate the new tuft_number_file
        # try:
        #     validate_neuron_distribs(distributions)
        # except ValidationError as exc:
        #     msg = "The given tuft distributions are not valid"
