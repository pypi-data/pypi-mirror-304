...
def plot_parliament(total_seats, features, seats, slices=50, additional_rows=5):
    ...
    # Sabit Renk Eşleşmesi Sağla
    # Özelliklerin sayısı fazla olduğunda ek renkler eklemek için bir renk haritası tanımla
    ...
    # Özelliklerle Renkleri Doğrudan Eşle
    ...
    legend_patches = [plt.Rectangle((0, 0), 1, 1, color=feature_colors[features[i]], label=f"{features[i]} ({seats[i]} MV)") for i in range(len(features))]
