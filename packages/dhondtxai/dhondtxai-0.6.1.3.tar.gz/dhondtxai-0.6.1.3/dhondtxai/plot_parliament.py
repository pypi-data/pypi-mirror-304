...
def plot_parliament(total_seats, features, seats, slices=50, additional_rows=5):
    ...
    # Özelliklerin sırasına göre sabit bir renk eşleşmesi sağlayın
    ...
    # Her bir özelliğe sabit bir renk eşlemesi yapın
    feature_colors = {feature: colors[i % len(colors)] for i, feature in enumerate(sorted(features))}
    ...
    legend_patches = [plt.Rectangle((0, 0), 1, 1, color=feature_colors[feature], label=f"{feature} ({seats[features.index(feature)]} MV)") for feature in features]
