...
def generate_token(config, verify_ssl_cert: Union[bool, str, None]=True, validity: Union[int, str, None]='2m'):
    ...
    :param config: Cortex configuration from CortexEnv.get_cortex_profile("str")
    ...
    try:
        jwk = config.get("jwk")
        # this is a bit hacky we should refactor and use the Client object consistently
        if config.get("cert"):
            verify_ssl_cert = write_cert_file(config)  # This may be called independently of the client
...
def write_cert_file(config):
    """
    Helper function to extract the cert attribute from the profile and write to a temporary file.

    Args:
        config: Sensa cluster profile/config

    Yields:
         The name of the temporary file.
    """
    cert_data = config.get('cert')
