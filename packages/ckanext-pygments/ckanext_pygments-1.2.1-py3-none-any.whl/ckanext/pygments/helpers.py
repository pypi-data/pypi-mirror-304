...
def pygments_get_resource_view_cache_size(
    resource_id: str, resource_view_id: str
) -> int:
    return pygments_cache.RedisCache().calculate_view_cache_size(
        resource_id, resource_view_id
    )
...
def pygments_get_preview_lexers_options() -> list[dict[str, str]]:
    return [{"value": opt, "label": opt} for opt in pygment_utils.get_list_of_available_lexers()]
