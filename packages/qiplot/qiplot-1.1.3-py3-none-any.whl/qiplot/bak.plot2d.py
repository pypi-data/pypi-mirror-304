...
class Qiplot:
    def __init__(self, title, size):
        ...
        # create 2d plot box
        self.plo2dw = self.win.addPlot(row=0, col=0, name='p0', title='')
...
def main():
    ...
    print(args.usecols)
    fdata, names = get_xye(args.datafiles[::args.every], args.usecols,
                           args.maxbytes*(1<<20), args.label)
    t1 = time()
...
#    p1.addItem(img)
    ...
    roi = pg.ROI(pos=[ xdata[np.argmax(np.mean(data, axis=0))]-hsize/2, 0],
                             size=[hsize,vsize],
...
#        p2.plot(newx, selected.mean(axis=0), clear=True)
    ...
    qplot.proxy2d = pg.SignalProxy(qplot.plo2dw.scene().sigMouseMoved, rateLimit=60, slot=qplot._mouseMoved)
