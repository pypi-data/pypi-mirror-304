    ...
    while True:
        ...
        if initial_src_balance <= 1:
            base_logger.critical(
                f"Source balance empty. Cannot continue trading.")
            yield EmptySourceBalance(src_token, account.address)
            break

        if not (dest_token := destination_policy()):
            base_logger.critical(f"No viable destination token")
        ...
        base_logger.debug(f"Quote:")
        base_logger.debug(pformat(quote))

        if quote["invalid_amount"]:
            base_logger.warning(f"Quote had invalid amount")
        ...
        if quote["liquidity_source"] == "unavailable":
            base_logger.warning(f"No liquidity source")
        ...
        if (risk_analyis := risk_manager(src_token, dest_token, quote)).reject:
            base_logger.warning(f"Order rejected by risk manager")
        ...
        if src_amount < desired_src_amount:
            base_logger.warning(
                "Not enough liquidity to trade entire source balance")
        ...
        src_token_contract = client_utility.make_token_contract(
            src_w3,
            src_token
        )
        ...
        try:
            ...
            base_logger.error(f"Failed to place order:", exc_info=e)
        ...
        try:
            ...
            base_logger.error(
                f"There was an error submitting this order:", exc_info=e)
        ...
        if count >= config.max_polls:
            base_logger.warning(
                "Source balance not withdrawn after max waiting time")
