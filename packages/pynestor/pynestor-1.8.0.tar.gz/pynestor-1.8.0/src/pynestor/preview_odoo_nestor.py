...
class EnvironementConfig:
    def __init__(self, base_env_vars=None):
        ...
        self.NO_RESET_PASSWORD: bool = (
            base_env_vars.get("NO_RESET_PASSWORD", str(False)).capitalize() == str(True).capitalize()
        )
    ...
    def __repr__(self):
        ...
        res += "\nNO_RESET_PASSWORD = " + str(self.NO_RESET_PASSWORD)
        res += "\nALWAYS_DELETE = " + str(self.NO_RESET_PASSWORD)
        res += "\nALWAYS_RESTORE = " + str(self.NO_RESET_PASSWORD)
        res += "\nNEVER_DELETE_ON_FAIL = " + str(self.NO_RESET_PASSWORD)
...
class PreviewUpScript(ScriptNestor):
    def restore_db(self):
        ...
        self.inst.delete_and_exit_if_failed(
            self.inst.db_restore_from_s3(
                ...
                set_password_to_all=not self.config.NO_RESET_PASSWORD,
                no_reset_password=self.config.NO_RESET_PASSWORD,
            )
        )
    ...
    def run_script(self):
        ...
        if self.config.NEVER_DELETE_ON_FAIL:
            self.inst.exit_if_failed(self.inst.update(self.config.BASE_MODULE_UPDATE))
            self.inst.exit_if_failed(self.inst.wait(up=True, postgres=True))
            self.inst.exit_if_failed(self.inst.start())
        else:
