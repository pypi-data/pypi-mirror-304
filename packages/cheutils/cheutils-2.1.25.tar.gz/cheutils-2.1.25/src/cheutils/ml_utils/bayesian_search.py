...
from hyperopt import fmin, tpe, hp, mix, anneal, rand, space_eval, STATUS_OK
...
from cheutils.ml_utils.model_builder import fit, predict, cross_val_model
    ...
    def __objective(self, params):
        ...
        def optimize():
            if self.cv is not None:
                cv_score = cross_val_model(underlying_model, self.X, self.y, scoring=self.scoring_,
                ...
                #no cross-validation
                fit(underlying_model, self.X, self.y)
                y_pred = predict(underlying_model, self.X)
        ...
        if self.mlflow_log:
            with mlflow.start_run() as run:
                ...
                mlflow.set_tag('model', self.model_option)
                mlflow.log_params(params)
                optimize()
                mlflow.log_metric('mse', min_score)
