...
def randn(*sizes, device=None):
    ...
    """
    u = curl.rand(*sizes, device=device).flatten()
    odd_numel = u.numel() % 2 == 1
    if odd_numel:
        u = curl.cat([u, curl.rand((1,), device=device)])
...
def bernoulli(self):
    ...
    input tensor."""
    return self > curl.rand(self.size(), device=self.device)
...
def weighted_index(self, dim=None):
    ...
    r = curl.rand(max_weight.size(), device=self.device) * max_weight
