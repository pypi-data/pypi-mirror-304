...
def argmax(self, dim=None, keepdim=False, one_hot=True):
    ...
    `input` tensor.
    """
    method = LookupTables().get_config().config["max"]["method"]
...
def max(self, dim=None, keepdim=False, one_hot=True):
    ...
    method = lut_config.config["max"]["method"]
    if dim is None:
        if method in ["log_reduction", "double_log_reduction"]:
            ...
            with lut_config.temp_override({"max": {"method": method}}):
...
def _argmax_helper_pairwise(enc_tensor, dim=None):
    ...
    b = curl.stack([enc_tensor.roll(i + 1, dims=dim) for i in range(row_length - 1)])
...
def _compute_pairwise_comparisons_for_steps(input_tensor, dim, steps):
    ...
    for _ in range(steps):
        ...
        pairwise_max = curl.where(x >= y, x, y)
        enc_tensor_reduced = curl.cat([pairwise_max, remainder], dim=dim)
...
def _max_helper_log_reduction(enc_tensor, dim=None):
    ...
    with lut_config.temp_override({"max": {"method": "pairwise"}}):
...
def _max_helper_double_log_recursive(enc_tensor, dim):
    ...
    if n == 1:
        ...
        full_max_tensor = curl.cat([enc_max_tensor, remainder], dim=dim)
