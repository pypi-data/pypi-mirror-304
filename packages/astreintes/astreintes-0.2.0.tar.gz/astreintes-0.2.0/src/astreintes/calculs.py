...
def _calcul_rotations(df_sites: pd.DataFrame, df_effectifs: pd.DataFrame, params: models.Parametres):
    ...
    if n_mixed > 0:
        ...
        for site in df_sites['nom']:
            ...
            n_respi_mixed = int(np.round(((nr_aga + nr_mixed) / n_aga - nr_respi / n_respi) / (1 / n_respi + 1 / n_aga)))
...
def separation_calendriers(df_sites, df_effectifs=None):
    """
    ...
    df_effectifs['icalendrier'] = -1
    if np.all(df_sites['rotations'] == 1):
        return [{'sites': df_sites, 'effectifs': df_effectifs}]
    df_calendriers = []
    icalendrier = 0
    for i in np.arange(df_sites.shape[0]):
        nr = df_sites.at[i, 'rotations']
        nr_simple, nr_mixed = (np.floor(nr / 2).astype(int), nr % 2)
        ntechs = {
            'aga': _repartition(p=np.r_[np.ones(nr_simple), nr_mixed / 2], n_semaines=df_sites.at[i, 'aga'], max_astreintes=1000),
            'respi': _repartition(
                p=np.r_[np.ones(nr_simple), nr_mixed / 2], n_semaines=df_sites.at[i, 'respi'], max_astreintes=1000
            ),
        }
        df_calendriers.append(
            pd.DataFrame(
                {
                    'n_aga': ntechs['aga'],
                    'n_respi': ntechs['respi'],
                    'site': df_sites.at[i, 'nom'],
                    'type': ['simple'] * nr_simple + ['multi'],
                    'icalendrier': (ical := np.arange(nr_simple + 1) + icalendrier),
                }
            )
        )
        # on répartit les équipes de façon aléatoire
        for specialite in ['aga', 'respi']:
            ie = (
                (df_effectifs['site'] == df_sites.at[i, 'nom'])
                & (df_effectifs['specialite'] == specialite)
                & (df_effectifs['icalendrier'] == -1)
            )
            ie = np.where(ie.values)[0]
            np.random.shuffle(ie)
            for icc, iee in zip(ical, np.split(ie, np.cumsum(ntechs[specialite])[:-1])):
                df_effectifs.loc[iee, 'icalendrier'] = icc
        icalendrier += nr_simple + 1
    df_calendriers = pd.concat(df_calendriers)
    df_calendriers = df_calendriers.loc[(df_calendriers['n_aga'] + df_calendriers['n_respi']) > 0]

    dfs = []
    for _, calendrier in df_calendriers[df_calendriers['type'] == 'simple'].iterrows():
        dfs.append(
            {
                'sites': pd.DataFrame(
                    {
                        'nom': [calendrier.site],
                        'aga': [calendrier.n_aga],
                        'respi': [calendrier.n_respi],
                        'rotations': [1],
                    }
                ),
                'effectifs': df_effectifs[df_effectifs['icalendrier'] == calendrier.icalendrier].copy(),
            }
        )
    df_multi = df_calendriers[df_calendriers['type'] == 'multi']
                ...
                'sites': pd.DataFrame(
                    {
                        'nom': df_multi['site'],
                        'aga': df_multi['n_aga'],
                        'respi': df_multi['n_respi'],
                        'rotations': np.ones(df_multi.shape[0], dtype=int),
                    }
                ),
                'effectifs': df_effectifs[df_effectifs['icalendrier'].isin(df_multi['icalendrier'])].copy(),
            }
        )
    assert np.all(df_effectifs['icalendrier'] > -1)
    return dfs
...
def genere_planning(
    ...
    liste_sites_effectifs = separation_calendriers(df_sites, df_effectifs)
    df_planning = []
    for dse in liste_sites_effectifs:
        # semaine   site  rotation  id_tech specialite
        df_planning.append(_calcul_rotations(dse['sites'], dse['effectifs'], params=params))
    df_planning = pd.concat(df_planning)
