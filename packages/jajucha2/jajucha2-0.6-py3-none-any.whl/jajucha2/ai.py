...
def get_model(name):
    query = "SELECT uuid FROM UserModels WHERE Name = %s"
    cursor.execute(query, (name,))
    rows = cursor.fetchall()

    modeldir = home_dir + "/server/models/usermodels/" + rows[0][0] +".pth"
    net = torch.jit.load(modeldir)
    return net

# Cache for model UUIDs and shapes
...
def get_model_output(input, name):
    ...
    if model_name in ['RESNET18', 'RESNET50', 'RESNET101', 'RESNET152']:
        ...
        frame_resized = cv2.resize(input, (640, 640))
        results = loaded_model(frame_resized)
        return results
