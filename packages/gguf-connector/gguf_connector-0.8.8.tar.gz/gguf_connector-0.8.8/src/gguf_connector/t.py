...
def handle_tensors(path, writer, state_dict, model_arch):
...
# import argparse # dropped with the function below as well
# def parse_args():
    ...
    try:
        ...
        if next(iter(state_dict.values())).dtype == torch.bfloat16:
            ...
            if os.path.isfile(out_path):
                input("Output exists enter to continue or ctrl+c to abort!")
            # handle_tensors(path, writer, state_dict, model_arch)
            handle_tensors(" --src "&path, writer, state_dict, model_arch)
            writer.write_header_to_file(path=out_path)
            writer.write_kv_data_to_file()
            writer.write_tensors_to_file(progress=True)
            writer.close()
