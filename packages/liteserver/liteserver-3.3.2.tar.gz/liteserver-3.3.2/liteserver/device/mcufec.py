...
def open_serdev():
    timeout = 1
    try:
        r = serial.Serial(pargs.tty, pargs.baudrate, timeout=timeout)#, inter_byte_timeout=0.1)#write_timeout=.1) #parity=serial.PARITY_EVEN, rtscts=1)
    except serial.SerialException as e:
        ttyACM = '/dev/ttyACM'
        if pargs.tty.startswith(ttyACM):
            suffix = pargs.tty[-1]
            if suffix in '01':
                pargs.tty = ttyACM + {'0':'1', '1':'0'}[suffix]
                try:
                    r = serial.Serial(pargs.tty, pargs.baudrate, timeout=timeout)
                except:
                    printe('Could not open ttyACM0/1')
                    sys.exit(1)
                printw(f'Was not able to open {pargs.tty}, using {pargs.tty} instead')
        else:
...
class MCUFEC(Device):
    ...
    def __init__(self,name, bigImage=False):
        ...
        self._pars.update({'mean': LDO('R','Average of all samples of ADC channels',
          [0.]*nADCs, units='V')}),
        self._pars.update({'std': LDO('R','Standard deviation of all samples of ADC channels',
          [0.]*nADCs, units='V')}),
