...
def pen_ll_fun(np.ndarray[double] beta, 
               np.ndarray[double, ndim=2] X, 
               np.ndarray[double] y,
               double lam=0, 
               np.ndarray[double] weight_vec=None,
               long max_exp=50):
    cdef double ll_term, reg_term
    cdef np.ndarray[double] betaX
    cdef long i
...
@cython.wraparound(False)   # Deactivate negative indexing.
@cython.cdivision(True)
def pen_ll_fun_grad(np.ndarray[double] beta, 
               np.ndarray[double, ndim=2] X, 
               np.ndarray[double] y,
               double lam=0, 
               np.ndarray[double] weight_vec=None,
               long max_exp=50):
    cdef double ll_term, reg_term
    cdef np.ndarray[double] betaX, z, main_grad_term, other_grad_term
    cdef long i
