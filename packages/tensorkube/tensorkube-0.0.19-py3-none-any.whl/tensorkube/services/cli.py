...
@dev.command()
def list():
    list_instances_in_cloud('aws')


@dev.command()
@click.option('--gpu-type', type=str)
def start(gpu_type: str):
    start_devcontainer('aws', gpu_type)


@dev.command()
def pause():
    pause_devcontainer('aws')


@dev.command()
def purge():
    purge_devcontainer('aws')
