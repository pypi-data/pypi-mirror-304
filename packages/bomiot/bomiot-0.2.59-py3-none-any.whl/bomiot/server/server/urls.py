...
CONFIG = ConfigParser()
CONFIG.read(join(settings.PROJECT_DIR, 'setup.ini'), encoding='utf-8')
...
current_path = [p for p in listdir(settings.PROJECT_DIR) if isdir(p)]
            ...
            if app_mode == 'plugins':
                ...
                if module == project_name:
                    ...
                    for url in find_urls:
                        if exists(join(join(settings.PROJECT_DIR, url), 'urls')):
                            urlpatterns += [
...
if len(filtered_current_path) > 0:
    for module_name in filtered_current_path:
        ...
        if exists_module is not None:
            ...
            app_mode = module_import.mode_return()
            if app_mode == 'plugins':
                if exists(join(join(settings.PROJECT_DIR, module_name), 'urls')):
                    urlpatterns += [
                        path(f'{module_name}/', include(f'{module_name}.urls')),
                    ]
            elif app_mode == 'project':
                if module_name == project_name:
                    project_path = join(settings.PROJECT_DIR, project_name)
