...
def create_file(folder: str):
    ...
    if exists(join(working_space, 'setup.ini')) is False:
        ...
        if folder != 'bomiot':
            ...
            config.write(open(join(working_space, 'setup.ini'), "wt"))
    ...
    config.read(working_path, encoding='utf-8')
    config.set('space', 'name', working_space)
    config.write(open(working_path, "wt"))
