...
with open(join(BASE_DIR, 'workspace.toml'), 'r', encoding='utf-8') as working_space:
    working_space_path = toml.load(working_space)
PROJECT_DIR = working_space_path['space']['name']


CONFIG = ConfigParser()
CONFIG.read(join(BASE_DIR, 'workspace.toml'), encoding='utf-8')
...
current_path = [p for p in listdir(PROJECT_DIR) if isdir(p)]
...
if len(filtered_current_path) > 0:
    for module_name in filtered_current_path:
        ...
        if exists_module is not None:
            ...
            app_mode = module_import.mode_return()
            if app_mode == 'plugins':
                if exists(join(join(PROJECT_DIR, module_name), 'apps')):
...
if PROJECT_NAME in [p for p in listdir(PROJECT_DIR) if isdir(p)]:
...
STATIC_URL = 'static/'
STATIC_ROOT = join(PROJECT_DIR, 'bomiot_static').replace('\\', '/')
STATICFILES_DIRS = [
    join(BASE_DIR, "static").replace('\\', '/'),
]

MEDIA_URL = 'media/'
MEDIA_ROOT = join(PROJECT_DIR, 'media').replace('\\', '/')
...
LOG_PATH = join(PROJECT_DIR, 'logs')
