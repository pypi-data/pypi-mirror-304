...
def create_file(folder: str):
    ...
    working_path = join(join(current_path.parent.parent, 'server'), 'workspace.toml')
    with open(working_path, 'r', encoding='utf-8') as working_space:
        working_space_path = toml.load(working_space)
    working_space_path['space']['name'] = working_space
    with open(working_path, 'w', encoding='utf-8') as user_working_space:
        toml.dump(working_space_path, user_working_space)