...
class AudioToTextRecorder:
    ...
    def _audio_data_worker(audio_queue,
        ...
        def initialize_audio_stream(audio_interface, sample_rate, chunk_size):
            ...
            def validate_device(device_index):
                ...
                try:
                    ...
                    logging.error(f"Validated. Device index {device_info}")
                    return True
            ...
            while not shutdown_event.is_set():
                try:
                    ...
                    logging.error(f"Microphone connection failed: {e}")
            ...
            #             input_device_index=input_device_index,
            #         )
            #         # test read some data from the stream
            #         dummy_read_data = stream.read(chunk_size, exception_on_overflow=False)
            #         logging.error(f"Microphone connected successfully (input_device_index: {input_device_index})")
            #         return stream
        ...
        def verify_continuous_audio(stream, num_test_reads=3):
            """Verify that we can continuously read from the audio stream."""
            try:
                for _ in range(num_test_reads):
                ...
                logging.debug(f"Continuous audio verification failed: {e}")
                return False

        # In the main try-except block of _audio_data_worker:
        try:
            while not shutdown_event.is_set():
                try:
                    data = stream.read(chunk_size, exception_on_overflow=False)
                    ...
                    if e.errno == pyaudio.paInputOverflowed:
                        ...
                        try:
                            ...
                            logging.debug(f"Error closing stream: {close_error}")
                        ...
                        for attempt in range(3):  # Try 3 times
                            if setup_audio():
                                # Verify we can actually read from the stream continuously
                                if verify_continuous_audio(stream):
                                    logging.info("Audio stream reinitialized and verified working")
                                    reinitialized = True
                                    break
                                else:
                                    logging.error("Stream reinitialization failed verification - no audio data")
                                    try:
                                        if stream:
                                            stream.stop_stream()
                                            stream.close()
                            ...
                            logging.error(f"Reinitialization attempt {attempt + 1} failed, waiting before retry...")
                            time.sleep(2)  # Wait between attempts
                        
                        if not reinitialized:
                            logging.error("Failed to reinitialize working audio stream after multiple attempts")
                    ...
                    # Similar reinitialization logic as above
                    try:
                        if stream:
                            stream.stop_stream()
                            stream.close()
