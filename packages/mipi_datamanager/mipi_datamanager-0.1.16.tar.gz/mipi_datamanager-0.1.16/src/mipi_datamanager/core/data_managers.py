...
class DataManager:
    """
    Queries, stores, and combines data from a library of SQL scripts and other data sources. The
    It is always created from a "Base Population" which sets the granularity. Base Populaiton contructors methods are
    all prefixed with `from_`. Additional data can be added using join methods, which are all prefixed with `join_from_`.
    Data joined from SQL will match the granularity of the current populaiton.
    ...
    def from_jinja_repo(cls, inner_path: str,
        ...
        """
        Creates a MiPi DataManager(DM) and sets the attributes of that DM. For frequently used scripts this is the most concise way to create a
        DM. All scripts in the repo are tied to the appropriate connection, frame name, and join key if applicable. Additionally, the repo has a way to automatically
        generate documentation on each Script
    ...
    def from_dataframe(cls, df: pd.DataFrame, frame_name: str = None,
        ...
        Args:
            ...
            frame_name: Name of the frame that is stored in the DM. If None, defaults to `unnamed-frame`
            jinja_repo_source: Coming Soon!
            store_deep_frames: If `True`, each `DM.Frame` stores a full DataFrame for both the `df_query` and `df_target`.
                This is useful for troubleshooting as it provides detailed snapshots of each frame during creation but can be memory-intensive.
            store_deep_base: If `True`, the base frame only `DM.Frame[0]` stores a full DataFrame for both the `df_query` and `df_target`.
            default_format_func_dict: Coming Soon!
    ...
    def from_sql(cls, script_path: str, connection: odbc.Odbc, format_parameters_list: list = None,
        ...
        will be resolved using string formatting. format_parameters_list will be passed into the placeholders in order.

        Args:
            script_path: The absolute or relative path to the SQL script.
            connection: MiPi Connection Object
            format_parameters_list: A list of values to be placed into string format placeholders "{}". Values will be entered into placeholders in
            ...
            store_deep_frames: If `True`, each `DM.Frame` stores a full DataFrame for both the `df_query` and `df_target`.
                This is useful for troubleshooting as it provides detailed snapshots of each frame during creation but can be memory-intensive.
            store_deep_base: If `True`, the base frame `DM.Frame[0]` stores a full DataFrame for both the `df_query` and `df_target`.
            default_format_func_dict:
            dialect: SQL dialect being used in the database, this determines the temp table syntax.
                Currently only 'mysql' is available but more to come
    ...
    def join_from_jinja_repo(self, inner_path: str, how: JoinLiterals = "left", jinja_parameters_dict: dict = None,
                             format_func_dict: dict = None, left_on: str | tuple = None, left_index: str | tuple=None):
    ...
    def join_from_excel(self, excel_path: str, on: str = None, how: JoinLiterals = "left", frame_name: str = None,
                        excel_sheet: str | int = None, format_func_dict: dict = None,
                        left_on: str | tuple = None, right_on: str | tuple = None,
                        left_index: bool = False, right_index: bool = False):
        """
    ...
    def join_from_format_sql(self, script_path: str, connection:odbc.Odbc, on: str | tuple=None, how: JoinLiterals = "left",
        ...
        A method of the DataManager (DM) that runs a query and joins the results to the target population. The DM inserts the
        target records into the subsequent query so that only records are returned.
        The accompanying SQL script can use '{}' optional placeholders to accept parameters. The script must have at
        least one placeholder, the first one is assumed to be for the temp table.
    ...
    def join_from_jinja(self, script_path: str, connection: odbc.Odbc, on: str | tuple = None,
        ...
        A method of the DataManager (DM) that runs a query and joins the results to the target population. The DM inserts the
        target records into the subsequent query so that only records are returned. This method leverages Jinja to insert the
        temp table and any other optional parameters. There must be a jinja tag for {{ MiPiTempTable }} or an alternatively declared temp table name.
        The Jinja script is resolved as per the [Jinja2 official documentation](https://jinja.palletsprojects.com/en/3.1.x/).
    ...
    def _set_column_source_from_frame(self, frame, idx) -> None:
        ...
        for column in frame.query_columns:
            ...
            if (column in self._column_sources
                ...
                self._target_population = self._target_population.rename(columns={column: f"{column}~{frame.name}_{idx}"})
    ...
    def base_population(self) -> pd.DataFrame:
        ...
        or if `store_deep_base = True` during initalization. The meta data for this dataframe is represented in the
        zeroth Frame in `DataManager.frames`. Initially this dataframe defines the granularity and records that are used
        in subsequent queries. This attribute is used as a refrence as the starting point.

        Returns: self._frames[0].df_query

        Raises:
            AttributeError: If `store_deep_base` or `store_deep_frames` is False, indicating that the base population is
            not stored.
    ...
    def duplicated_columns(self) -> list:
        ...
        List of all columns in the target dataframe that contain a "~" character. The when duplicate columns
    ...
    def frames(self) -> com.IndexedDict:
        """
        A dictionary where each entry is a [frame][mipi_datamanager.core.meta.Frame]. A frame represents a snapshot of the data
        that entered and the state of the target population. Frames are stored in an "indexed dictionary".
