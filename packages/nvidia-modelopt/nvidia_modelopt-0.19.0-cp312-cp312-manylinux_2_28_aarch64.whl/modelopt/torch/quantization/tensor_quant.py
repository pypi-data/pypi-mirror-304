...
from .export_onnx import export_fp8, export_int8
...
class DynamicBlockQuantizationFunction(Function):
    ...
    @staticmethod
    def forward(ctx, inputs, block_size, amax, num_bits, scale_bits):
        """Forward method."""
        ctx.save_for_backward(inputs, amax)

        assert inputs.shape[-1] % block_size == 0, (
            "Block size must divide the last dimension of the input tensor. Got input with"
            f" shape: {inputs.shape}."
        )
        assert inputs.is_cuda, "inputs must be a CUDA tensor for dynamic block quantization."
        ...
        if num_bits in mx_format_map:
            ...
            if not cuda_ext_mx:
                raise RuntimeError(
                    f"cuda_ext_mx could not be imported. {mx_format_map[num_bits]} quantization cannot be performed."
                )
            with torch.cuda.device(
                None if inputs.device.index == torch.cuda.current_device() else inputs.device.index
            ):
