...
def tree_sys(supname, name, subname, tree: Tree = None) -> Tree:
    if tree is None:
        treelib_tree = Tree()
        sup_node = treelib_tree.create_node(supname, idgen.next_id(), None, data=SUPER_MODEL_NAME)
    else:
        treelib_tree = tree
        sup_node = find_node_by_tag(tree, supname)
    if isinstance(name, str):
        if not (isinstance(subname, str) or isinstance(subname, list)):
            raise ValueError("name为字符串时，subname必须是字符串或列表.")
        sys_node = treelib_tree.create_node(name, idgen.next_id(), sup_node.identifier, data=SYSTEM_MODEL_NAME)
        if isinstance(subname, str):
            treelib_tree.create_node(subname, idgen.next_id(), sys_node.identifier, data=SUB_MODEL_NAME)
        ...
        for i, sys in enumerate(name):
            sys_node = treelib_tree.create_node(sys, idgen.next_id(), sup_node.identifier, data=SYSTEM_MODEL_NAME)
            sub_list = subname[i]
            if sub_list is None:
                continue
...
if __name__ == '__main__':
    system_tree = tree_sys(supname='insofsys', name=['insoftest'], subname=[['DTIS_511', 'NDT_SNPTC']])
    system_tree = tree_sys(tree=system_tree, supname='insofsys', name=['insofrobot'], subname=[['insoftube', 'insofbend', 'insoflaser']])
    system_tree = tree_sys(tree=system_tree, supname='insofsys', name='insofaiam', subname='insofmining')
    system_tree = tree_sys(tree=system_tree, supname='insofsys', name=['1', '2'], subname=[['insofminin7g', 'insofminin7213'], 'insofminin474g'])
    print(1)
