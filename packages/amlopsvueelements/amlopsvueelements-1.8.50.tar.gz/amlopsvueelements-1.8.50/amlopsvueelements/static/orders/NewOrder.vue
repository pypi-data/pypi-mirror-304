            ...
            >
              <div class="w-full flex gap-x-3">
                ...
                <SelectField
                  ...
                  :options="fuelQuantityUnits"
                  :loading="false"
                  @update:model-value="changeUom($event, index)"
                />
              </div>
            </div>
          </div>
...
import type { IAircraft } from '@/types/order/aircraft.types';
...
const { data: aircrafts, callFetch: fetchAircrafts } = useFetch<
  IAircraft[],
  (id: number) => Promise<IAircraft[]>
>(async (id: number) => {
...
const { callFetch: fetchClients } = useFetch<IClient[], (pageNumber: number) => void>(
...
const { callFetch: fetchOperators } = useFetch<IOperator[], (pageNumber: number) => void>(
  async (pageNumber: number) => {
...
const { data: handlers, callFetch: fetchHandlers } = useFetch<
  IGroundHandler[],
  (id: number) => Promise<IGroundHandler[]>
>(async (id) => {
...
const { callFetch: fetchCurrencies } = useFetch<any[], (pageNumber: number) => void>(
  async (pageNumber: number) => {
...
const { callFetch: fetchFuelCapacity } = useFetch<any[], (pageNumber: number) => any>(
  async (typeId: number) => {
