...
export const useOrderFormStore = defineStore('OrderForm', () => {
  ...
  const mapForm = () => {
    const mappedForm: IMappedOrder = {
      ...
      tails:
        formModel.aircraft && formModel.type?.is_fuel
          ? fuelGroupModel.value.map((el: any, index: number) => {
              return {
                tail_number: airGroupModel.value[index].id,
                fuel_quantity:
                  airGroupModel.value.length === 1
                    ? Number(formModel.fuel_order!.fuel_quantity!)
                    : el.fuel_quantity!,
                fuel_uom: el.fuel_uom!.id! ?? formModel.fuel_order!.fuel_uom!.id!
              };
            })
          : formModel.aircraft && formModel.type?.is_gh
          ? airGroupModel.value.map((el: any) => {
              return {
                tail_number: el.id,
                fuel_quantity: null,
                fuel_uom: null
