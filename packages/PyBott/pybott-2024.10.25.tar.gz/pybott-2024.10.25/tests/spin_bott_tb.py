...
class TestSpinBott(unittest.TestCase):
    ...
    def compute_spin_bott(self, t2, rashba):
        ...
        model = km.get_finite_kane_mele(self.nx, self.ny, self.t1, self.esite, t2, rashba)
        ...
        return spin_bott(lattice_x2, evals, vr_list, sigma, evals[N_sites // 2], self.threshold_psp)
    ...
    def test_spin_bott_t2_above_threshold(self):
        ...
        for t2 in [0.22, 0.5, 0.75, 1.0]:
            ...
            self.assertAlmostEqual(c_sb, 1, delta=1e-6, msg=f"Spin Bott index should be 1 for t2={t2}")
    ...
    def test_spin_bott_t2_below_threshold(self):
        ...
        for t2 in [0.0, 0.1, 0.15, 0.2]:
            c_sb = self.compute_spin_bott(t2, self.rashba)
            self.assertNotAlmostEqual(c_sb, 1, delta=1e-6, msg=f"Spin Bott index should not be 1 for t2={t2}")
    ...
    def test_spin_bott_rashba_below_threshold(self):
        ...
        for rashba in [0.01, 0.05, 0.09]:
            ...
            self.assertAlmostEqual(c_sb, 1, delta=1e-6, msg=f"Spin Bott index should be 1 for rashba={rashba} and t2={t2}")
