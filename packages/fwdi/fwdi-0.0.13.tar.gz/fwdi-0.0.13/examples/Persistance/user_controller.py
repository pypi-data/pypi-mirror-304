...
class UserController(BaseController, metaclass=MetaService):
    ...
    async def get(self, my_data:MyData, current_user: User = Security(JwtService.get_current_active_user, scopes=["user"]),):
    ...
    async def post(self, my_data:MyData, current_user: User = Security(JwtService.get_current_active_user, scopes=["user"]),):
        result_composit_test = self.__composite.run_once()
        return {
            "Controller": self.__class__.__name__,
            "Method 'GET'": "Hello from KSU Flat Web Dependency Injection platform",
