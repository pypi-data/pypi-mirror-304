import sys
from pathlib import Path
sys.path.insert(0,str(Path(sys.path[0]).parent))

from Abstracts.base_random_uuid import BaseRandomGUID
from Abstracts.base_message import BaseMessage
from Abstracts.base_composit import BaseComposit

from examples.generate_uuid_print import GenerateUuidAndPrint
from examples.normal_message import UserNormalMessage
from examples.user_random_id import UserRndGUID

#======= Package library ============================
from fwdi.Application.Abstractions.base_service_collection import BaseServiceCollection
from fwdi.WebApp.web_application import WebApplication
from fwdi.WebApp.web_application_builder import WebApplicationBuilder
#----------------------------------------------------


class ApplicationDI():
    def add(services:BaseServiceCollection):
        services.AddTransient(BaseRandomGUID, UserRndGUID) 
        services.AddTransient(BaseMessage, UserNormalMessage)
        services.AddTransient(BaseComposit, GenerateUuidAndPrint)

def start_web_service():
    server_param = {
        'name':'Python MVC Service',
        'debug':'False'
    }
 
    #====Block created Web Application layer

    builder:WebApplicationBuilder = WebApplication.create_builder(**server_param) # <<<<<<-------------------Maybe any init default workflow in this section
    
    #====/Block=====================================================================
    #==========Init Scopes =========================================================

    scopes = {
        "guest": "Information about current user", 
        "admin": "Administrative access", 
        "user": "All signin users",
        }
    
    builder.add_scope(scopes)
    builder.add_authentification()

    #==========/Init Scopes ========================================================
    #====Block registration all section application services to Dependecy Container
   
    ApplicationDI.add(builder.services)

    #====/Block=====================================================================
    
    app:WebApplication = builder.build() # <<<-----------------------------------------Create all default services in this section

    #WebService.AddEndpoints(app) <<< ---- maybe in new version
    from dependency_injection import WebService
    WebService.AddControllers(app)
    
    kwargs = {
            'host': "0.0.0.0",
            'port': 5000
        }
    
    app.run(**kwargs)

if __name__ == "__main__":
    start_web_service()