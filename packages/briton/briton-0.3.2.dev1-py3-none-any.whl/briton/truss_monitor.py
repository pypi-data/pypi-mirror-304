...
async def start_monitor(briton_interactor: BritonInteractor, predict: Callable, logger: Logger):
    monitor_settings = briton_interactor.monitor_settings()
    if not monitor_settings.start_monitor_thread:
        return

    def on_fail():
        logger.error("Spec dec model is stuck, exiting truss server")
        kill_current_process_and_children()

    async def monitor_fn():
        try:
            return await test_predict(predict, logger)
        except Exception as e:
            return False
