...
class SkillcornerOnDemandClient(FitRequest):
    ...
    def get_physical(self, match_id: int, csv: bool = False):
        ...
        """
        if csv:
            with open('match_physical.csv', 'wb') as f:
                f.write(self.get_match_physical(match_id))
        else:
            response = self.get_match_physical(match_id)
            return response


if __name__ == '__main__':
    client = SkillcornerOnDemandClient(USERNAME, PASSWORD)
    print(client.get_physical(match_id=1429182, csv=True))
